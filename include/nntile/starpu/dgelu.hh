 ...
 *
 * @file include/nntile/starpu/dgelu.hh
...
//! Generic wrapper class for dgelu operation is not defined
template<typename T>
class DGelu;

//! Specialization of wrapper class for add operation via std::tuple
template<typename T>
class DGelu<std::tuple<T>>
{
public:
...
using dgelu_pack_t = OperationPack<
    DGelu,
