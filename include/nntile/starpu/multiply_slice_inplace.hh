/*! @copyright (c) 2022-present Skolkovo Institute of Science and Technology
 *                              (Skoltech), Russia. All rights reserved.
 *                 2023-present Artificial Intelligence Research Institute
 *                              (AIRI), Russia. All rights reserved.
 *
 * NNTile is software framework for fast training of big neural networks on
 * distributed-memory heterogeneous systems based on StarPU runtime system.
 *
 * @file include/nntile/starpu/multiply_slice_inplace.hh
 * StarPU wrappers for in-place multiplication of a tensor and a broadcasted slice
 *
 * @version 1.1.0
 * */

#pragma once

// Compile-time definitions
#include <nntile/defs.h>

// Standard headers
#include <tuple>

// NNTile headers
#include <nntile/starpu/codelet.hh>
#include <nntile/starpu/handle.hh>

namespace nntile::starpu
{

//! Generic wrapper class for multiply_slice_inplace operation is not defined
template<typename T>
class MultiplySliceInplace;

//! Specialization of wrapper class for multiply_slice_inplace operation via std::tuple
template<typename T>
class MultiplySliceInplace<std::tuple<T>>
{
public:
    //! Codelet for the current operation
    CodeletTyped<T> codelet;

    //! Constructor
    MultiplySliceInplace();

    //! Structure for operation arguments
    struct args_t
    {
        Index m;
        Index n;
        Index k;
        Scalar alpha;
        Scalar beta;
        Index axis;
    };

    //! Footprint function for the current operation
    static uint32_t footprint(struct starpu_task *task);

    //! Wrapper for a generic CPU implementation
    static void cpu(void *buffers[], void *cl_args)
        noexcept;

    //! Array of all wrappers for CPU implementations
    static constexpr func_array cpu_funcs = {
        cpu
    };

    //! Wrapper for a generic CUDA implementation
    static void cuda(void *buffers[], void *cl_args)
        noexcept;

    //! Array of all wrappers for CUDA implementations
    static constexpr func_array cuda_funcs = {
#ifdef NNTILE_USE_CUDA
        cuda
#else // NNTILE_USE_CUDA
        nullptr
#endif // NNTILE_USE_CUDA
    };

    //! Submit multiply_slice_inplace task
    void submit(
        Index m,
        Index n,
        Index k,
        Scalar alpha,
        Handle src,
        Scalar beta,
        Handle dst,
        Index axis
    );
};

//! Pack of multiply_slice_inplace operations for different types
using multiply_slice_inplace_pack_t = OperationPack<
    MultiplySliceInplace,
    std::tuple<nntile::fp64_t>,
    std::tuple<nntile::fp32_t>,
    std::tuple<nntile::fp32_fast_tf32_t>,
    std::tuple<nntile::fp32_fast_fp16_t>,
    std::tuple<nntile::fp32_fast_bf16_t>,
    std::tuple<nntile::bf16_t>,
    std::tuple<nntile::fp16_t>
>;

//! Pack of multiply_slice_inplace operations for different types
extern multiply_slice_inplace_pack_t multiply_slice_inplace;

} // namespace nntile::starpu
