...
//!
//! // Build backward graph using grad_reg for accumulation
//! @endcode
class GradientRegistry
{
private:
    // Map from tensor name to its gradient tensor
    std::map<std::string, LogicalGraphTensorNode*> grad_map_;
...
public:
    ...
    //! Mark whether a tensor requires gradient computation
    void set_requires_grad(const LogicalGraphTensorNode& tensor,
                           bool requires = true);
    ...
    //! Check if a tensor requires gradient computation
    bool requires_grad(const LogicalGraphTensorNode& tensor) const;
    ...
    bool has_grad(const LogicalGraphTensorNode& tensor) const;

    //! Get gradient tensor for the given tensor name
    //! @return Pointer to gradient tensor, or nullptr if not registered
    LogicalGraphTensorNode* get_grad(const std::string& tensor_name) const;

    //! Get gradient tensor for the given tensor
    //! @return Pointer to gradient tensor, or nullptr if not registered
    LogicalGraphTensorNode* get_grad(const LogicalGraphTensorNode& tensor) const;
    ...
    //!         accumulation)
    void set_grad(const std::string& tensor_name,
                  LogicalGraphTensorNode* grad_tensor);

    //! Register a gradient tensor for the given tensor
    void set_grad(const LogicalGraphTensorNode& tensor,
                  LogicalGraphTensorNode* grad_tensor);
    ...
    //! Check if this is the first gradient contribution for a tensor
    bool is_first_grad(const LogicalGraphTensorNode& tensor) const;
    ...
    const std::map<std::string, LogicalGraphTensorNode*>& all_grads() const
    {
        return grad_map_;
    }
