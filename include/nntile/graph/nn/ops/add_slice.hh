 ...
 *
 * @file include/nntile/graph/nn/ops/add_slice.hh
 * NNGraph add_slice autograd operation.
 *
 * Forward: output = alpha * src1 + beta * src2 (src1 broadcast along axis)
 * Backward: grad_src1 += alpha * sum_slice(grad_out), grad_src2 += beta *
 * grad_out
 *
 * @version 1.1.0
 * */
...
//! AddSlice op: output = alpha*src1 + beta*src2 (src1 slice broadcast).
//! PyTorch-style.
struct NNAddSliceOp : NNGraph::OpNode
{
    ...
    NNAddSliceOp(NNGraph::TensorNode *src1_,
        ...
        Scalar beta_,
        Index axis_) :
        alpha(alpha_), beta(beta_), axis(axis_), src1(src1_), src2(src2_)
    {
        inputs_ = {src1, src2};
    }
...
NNGraph::TensorNode *add_slice(Scalar alpha,
