 ...
 *
 * @file include/nntile/graph/model/gptneo/gptneo_config_json.hh
 * Parse ``attention_layers`` / HF ``attention_types`` into ``GptneoConfig``.
 *
 * @version 1.1.0
 * */
...
//! Expand HF ``attention_types`` like ``GPTNeoConfig.expand_attention_types``.
    ...
    GptneoConfig &cfg)
{
        ...
        for (auto const &entry : j["attention_layers"])
        {
            if (!entry.is_string())
            {
                throw std::runtime_error(
                    "GptneoConfig JSON: attention_layers entries must be "
                    "strings");
            }
    ...
    for (auto const &group : j["attention_types"])
    {
        ...
        {
            throw std::runtime_error(
                "GptneoConfig JSON: attention_types must be [[types], count], "
                "...");
        }
        ...
        {
            throw std::runtime_error(
                "GptneoConfig JSON: invalid attention_types group");
        }
        ...
        for (auto const &entry : types_json)
        {
            ...
            {
                throw std::runtime_error(
                    "GptneoConfig JSON: attention_types type must be string");
            }
        ...
        {
            throw std::runtime_error(
                "GptneoConfig JSON: attention_types type list is empty");
        }
