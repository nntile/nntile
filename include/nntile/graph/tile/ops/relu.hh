 ...
 *
 * @file include/nntile/graph/tile/ops/relu.hh
 * TileGraph ReLU: dst = relu(src)
 *
 * @version 1.1.0
 * */
...
struct TileReluOp : TileGraph::OpNode
{
    ...
    TileReluOp() = default;
    TileReluOp(TileGraph::TileNode* src_, TileGraph::TileNode* dst_)
        : src(src_), dst(dst_)
    {
        inputs_ = {src};
        outputs_ = {dst};
    ...
    std::string op_name() const override { return "TILE_RELU"; }

    void execute(Runtime& runtime) const override;

    std::shared_ptr<TileGraph::OpNode> clone() const override
    {
        return std::make_shared<TileReluOp>(*this);
    }
};
