...
constexpr int CONTEXT_OOC_SIZE =
    16777216;                     // 16 MiB OOC buffer (unused when OOC=0)
...
static float config_get_float(
    const json &j, const char *key, float default_val)
{
    if (!j.contains(key))
        return default_val;
    const auto &v = j[key];
...
static LlamaConfig load_config(const std::string &path)
{
    ...
    cfg.intermediate_size =
        config_get_int(j, "intermediate_size", DEFAULT_INTERMEDIATE_SIZE);
    cfg.num_hidden_layers =
        config_get_int(j, "num_hidden_layers", DEFAULT_NUM_HIDDEN_LAYERS);
    cfg.num_attention_heads =
        config_get_int(j, "num_attention_heads", DEFAULT_NUM_ATTENTION_HEADS);
    cfg.num_key_value_heads =
        config_get_int(j, "num_key_value_heads", DEFAULT_NUM_KEY_VALUE_HEADS);
    cfg.rms_norm_eps =
        config_get_float(j, "rms_norm_eps", DEFAULT_RMS_NORM_EPS);
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "Error: provide either (--model + --prompt) or "
    ...
    for (int step = 0; step < args.max_tokens; ++step)
    {
        ...
        Index seq_len = is_prefill ? static_cast<Index>(tokens.size())
        ...
        {
            runtime.bind_data(
                input_ids, std::vector<std::int64_t>{tokens.back()});
        }
        ...
        std::int64_t next_id =
            argmax_last_position(logits, config.vocab_size, seq_len);
