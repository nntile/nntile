 ...
 *
 * @file examples/gptneo_generate.cc
 * Autoregressive text generation with a GPT-Neo causal LM.
 *
 * Workflow (two-step):
 *
 *   # Step 1 – Convert HF checkpoint and tokenize the prompt (Python):
 *   python examples/gptneo_generate.py \
...
struct Args
{
    ...
    std::string model;
    std::string prompt;
    std::string output_dir = "/tmp/nntile_gptneo";
    int max_tokens = 32;
};
    ...
    for(int i = 1; i < argc; ++i)
    {
        ...
        else if(arg == "--help" || arg == "-h")
        {
            std::cout
                ...
                << "  --config <path>       config.json (from gptneo_generate.py)\n"
...
static bool run_python_conversion(const Args& args)
{
    ...
    std::cout << "Running: python3 examples/gptneo_generate.py --model "
