 ...
 *
 * @file examples/gptneo_graph_training.cc
 * GPT-Neo causal LM training on the graph API.
 *
 ...
 * with a GPT-Neo tokenizer (e.g. ``--hf-tokenizer gptneo``). Point
 * ``--train-bin`` at the resulting ``train.bin``; ``GptneoConfig.vocab_size``
 * must cover token ids (``--tiny`` uses a 256-id toy vocab).
 *
 * Model path: ``GptneoCausal`` forward takes ``input_ids``, ``position_ids``,
 ...
 * JSON ``GptneoConfig``; otherwise the tiny default applies.
 ...
 *
 * Usage:
 *   ./gptneo_graph_training --train-bin /path/train.bin --seq 8 --batch 2
 *   ./gptneo_graph_training --train-bin data.bin --tiny --output-dir /tmp/out
 *
 * @version 1.1.0
 * */
...
using nntile::examples::save_gptneo_config_json;

static GptneoConfig make_tiny_config()
{
    GptneoConfig c;
    c.vocab_size = 256;
    c.hidden_size = 64;
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (arg == "--help" || arg == "-h")
        {
            std::cout
                ...
                << "  --config <path>       GPT-Neo JSON (optional if --tiny)\n"
        ...
        {
            std::cerr << "gptneo_graph_training: " << arg
                      << " requires a value\n";
            std::exit(EXIT_ERROR);
        }
        if (!arg.empty() && arg[0] == '-')
        {
            std::cerr << "gptneo_graph_training: unknown option " << arg
                      << "\n";
            std::exit(EXIT_ERROR);
        }
        std::cerr << "gptneo_graph_training: unexpected argument " << arg
                  << "\n";
        std::exit(EXIT_ERROR);
    }
    return a;
...
static void init_random_parameter_hints(GptneoCausal &model, std::mt19937 &gen)
{
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "gptneo_graph_training: --train-bin <path> is required\n";
        return EXIT_ERROR;
    }
    ...
    {
        config = load_gptneo_config_json(args.config_path);
    }
    else
    {
        config = make_tiny_config();
    ...
    if (args.warmup_steps < 0)
    {
        std::cerr << "gptneo_graph_training: --warmup-steps must be >= 0\n";
        return EXIT_ERROR;
    }
    if (args.epochs == 0)
    {
        std::cerr << "gptneo_graph_training: --epochs must be >= 1\n";
        return EXIT_ERROR;
    }
    ...
    std::cout << "=== GPT-Neo graph training (setup) ===\n"
        ...
        for (;;)
        {
            ...
            if (logits == nullptr)
            {
                throw std::runtime_error(
                    "gptneo_graph_training: model.forward returned null");
            }
    ...
    if (!args.output_dir.empty())
    {
        ...
        save_gptneo_config_json(config, cfg_path);
