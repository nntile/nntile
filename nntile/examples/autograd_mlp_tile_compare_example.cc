...
static void name_mlp_axis_groups_from_extents(nntile::TensorGraph &tg,
    nntile::Index batch,
    nntile::Index in_dim,
    nntile::Index hid_dim,
    nntile::Index out_dim)
{
    for (nntile::AxisDescriptor *ad : tg.axis_groups())
    {
        if (!ad->name.empty())
        {
            continue;
        ...
        {
            auto *node = static_cast<nntile::TensorGraph::TensorNode *>(
                ad->members[0].first);
        ...
        {
            ad->name =
                "extent_" + std::to_string(static_cast<long long>(ad->extent));
        }
    }
}
...
static void tile_all_axis_groups_heterogeneous(nntile::TensorGraph &tg)
{
    for (nntile::AxisDescriptor *ad : tg.axis_groups())
    {
        if (!ad->is_tiled())
    ...
    const std::vector<float> &ref, const std::vector<float> &other)
{
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        const float denom =
            std::max(std::max(std::abs(ref[i]), std::abs(other[i])), eps);
    ...
    const std::vector<float> &ref, const std::vector<float> &other)
{
    ...
    const double denom = std::max(std::max(ref_norm, other_norm), eps);
    return static_cast<float>(diff_norm / denom);
}

static void bind_same_weights(nntile::Runtime &rt,
    nntile::TensorGraph::TensorNode const *w1,
    nntile::TensorGraph::TensorNode const *w2,
...
int main()
{
    ...
    const std::vector<float> out_ref_v = rt_tensor.get_output<float>(out);
    ...
    const std::vector<float> out_tile_v = rt_tile.get_output<float>(out);
