/*! @copyright (c) 2022-present Skolkovo Institute of Science and Technology
 *                              (Skoltech), Russia. All rights reserved.
 *                 2023-present Artificial Intelligence Research Institute
 *                              (AIRI), Russia. All rights reserved.
 *
 * NNTile is software framework for fast training of big neural networks on
 * distributed-memory heterogeneous systems based on StarPU runtime system.
 *
 * @file nntile/src/tile_graph/logsumexp.cc
 * TileGraph logsumexp operation implementation.
 *
 * @version 1.1.0
 * */

#include "nntile/tile_graph/ops/logsumexp.hh"
#include <stdexcept>
#include <nntile/base_types.hh>
#include <nntile/dtype.hh>
#include <nntile/tile.hh>
#include <nntile/tile/logsumexp.hh>

#include <nntile/runtime.hh>
namespace nntile::tile_graph
{
namespace
{
template<typename T>
void run_lse(Runtime& runtime, TileGraph::TileNode* s, TileGraph::TileNode* d)
{
    nntile::tile::logsumexp<T>(runtime.get_tile<T>(s), runtime.get_tile<T>(d));
}
} // namespace
void logsumexp(TileGraph::TileNode* src, TileGraph::TileNode* dst)
{
    if(!src || !dst)
        throw std::invalid_argument("tile logsumexp: null");
    if(src->graph() != dst->graph())
        throw std::invalid_argument("tile logsumexp: same graph");
    if(src->dtype() != dst->dtype())
        throw std::invalid_argument("tile logsumexp: dtype");
    if(src == dst)
        throw std::invalid_argument("tile logsumexp: distinct");
    src->graph()->add_op(std::make_shared<TileLogsumexpOp>(src, dst));
}
void TileLogsumexpOp::execute(Runtime& runtime) const
{
    DataType dtype = runtime.get_dtype(src);
    switch(dtype)
    {
        case DataType::FP32:
            run_lse<nntile::fp32_t>(runtime, src, dst);
            break;
        case DataType::FP32_FAST_TF32:
            run_lse<nntile::fp32_fast_tf32_t>(runtime, src, dst);
            break;
        case DataType::FP32_FAST_FP16:
            run_lse<nntile::fp32_fast_fp16_t>(runtime, src, dst);
            break;
        case DataType::FP32_FAST_BF16:
            run_lse<nntile::fp32_fast_bf16_t>(runtime, src, dst);
            break;
        case DataType::FP64:
            run_lse<nntile::fp64_t>(runtime, src, dst);
            break;
        case DataType::FP16:
            run_lse<nntile::fp16_t>(runtime, src, dst);
            break;
        case DataType::BF16:
            run_lse<nntile::bf16_t>(runtime, src, dst);
            break;
        case DataType::INT64:
        case DataType::BOOL:
            throw std::runtime_error(
                std::string(dtype_to_string(dtype)) + " not supported for logsumexp");
        default:
            throw std::runtime_error("Unsupported data type for logsumexp");
    }
}
} // namespace nntile::tile_graph
