...
static __global__
void cuda_kernel2(Index nslow, Index nfast, const bool *mask, Scalar val_,
        T *data)
{
    Index griddim_row = (nslow+BLOCK_ROW-1) / BLOCK_ROW;
    ...
    if(threadIdx.x < BLOCK_ROW and global_i < nslow)
    {
    ...
    if(global_i < nslow)
    {
        if(!mask[global_i])
        {
            for(Index global_j = block_j*BLOCK_COL+j;
                    global_j < ::min((block_j+1)*BLOCK_COL, nfast);
                    global_j += BLOCK_COL_STEP)
            {
                data[global_j*nslow+global_i] = val;
            }
        }
    }
}
...
template<typename T>
void cuda(cudaStream_t stream, Index nslow, Index nfast, const bool_t *mask_,
        Scalar val, T *data)
    noexcept
 ...
 *
 * @params[in] nslow: Number of slow (leading) rows of data
 * @params[in] nfast: Number of fast (trailing) columns of data
 * @params[in] mask_: buffer with mask values with nslow entries
 * @params[in] val: value to set if mask element is false
 * @params[inout] data: nslow by nfast matrix, whose elements are updated
 * */
{
    dim3 threads(256);
    using B = typename CUDAComputeType<bool_t>::value;
    auto mask = reinterpret_cast<const B *>(mask_);
    dim3 blocks(((nslow+127)/128) * ((nfast+31)/32));
    (cuda_kernel2<T, 128, 32, 16>)<<<blocks, threads, 0, stream>>>(nslow,
            nfast, mask, val, data);
}

// Explicit instantiation
template
void cuda<fp32_t>(cudaStream_t stream, Index nslow, Index nfast,
        const bool_t *mask, Scalar val, fp32_t *data)
    noexcept;

template
void cuda<fp64_t>(cudaStream_t stream, Index nslow, Index nfast,
        const bool_t *mask, Scalar val, fp64_t *data)
    noexcept;

template
void cuda<bf16_t>(cudaStream_t stream, Index nslow, Index nfast,
        const bool_t *mask, Scalar val, bf16_t *data)
    noexcept;

template
void cuda<fp16_t>(cudaStream_t stream, Index nslow, Index nfast,
