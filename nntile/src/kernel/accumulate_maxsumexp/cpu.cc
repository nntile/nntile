 ...
 * */
{
    ...
    {
        auto src_odd = static_cast<typename T::repr_t>(src_[nelems + i]);
        auto src_even = static_cast<typename T::repr_t>(src_[i]);
        auto dst_odd = static_cast<typename T::repr_t>(dst_[nelems + i]);
        auto dst_even = static_cast<typename T::repr_t>(dst_[i]);
        ...
        if(src_odd != zero)
        {
            ...
            {
                dst_[i] = src_[i];
                dst_[nelems + i] = src_[nelems + i];
            }
            // Otherwise update based on maximum
            else if(dst_even < src_even)
            {
                dst_[nelems + i] = static_cast<T>(src_odd + dst_odd*std::exp(dst_even - src_even));
                dst_[i] = src_[i];
            }
            else
            {
                dst_[nelems + i] = static_cast<T>(dst_odd + src_odd * std::exp(src_even - dst_even));
            }
        }
    }
}
