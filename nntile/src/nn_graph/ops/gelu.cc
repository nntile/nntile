 ...
 *
 * @file nntile/src/nn_graph/gelu.cc
...
NNGraph::TensorNode *NNGeluOp::forward()
{
    if (x == nullptr)
    {
        throw std::invalid_argument("NNGeluOp::forward: x must be non-null");
    }
    NNGraph *graph = x->graph();
    bool out_requires_grad = any_input_requires_grad({x});
    TensorGraph::TensorNode *y_data = graph::tensor::gelu(x->data());
...
void NNGeluOp::backward() const
{
    ...
    if (x != nullptr && x->requires_grad())
    {
        ...
        graph::tensor::gelu_backward(
            x->data(), grad_out->data(), grad_x->data());
    }
}

NNGraph::TensorNode *gelu(NNGraph::TensorNode *x)
{
    if (x == nullptr)
    {
