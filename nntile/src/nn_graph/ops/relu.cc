/*! @copyright (c) 2022-present Skolkovo Institute of Science and Technology
 *                              (Skoltech), Russia. All rights reserved.
 *                 2023-present Artificial Intelligence Research Institute
 *                              (AIRI), Russia. All rights reserved.
 *
 * NNTile is software framework for fast training of big neural networks on
 * distributed-memory heterogeneous systems based on StarPU runtime system.
 *
 * @file nntile/src/nn_graph/relu.cc
 * NNGraph ReLU autograd implementation.
 *
 * @version 1.1.0
 * */

#include "nntile/nn_graph/ops/relu.hh"

#include "nntile/nn_graph/graph_data_node.hh"
#include "nntile/nn_graph/nn_grad_slot_name.hh"
#include "nntile/tensor_graph/ops/clear.hh"
#include "nntile/tensor_graph/ops/relu.hh"
#include "nntile/tensor_graph/ops/relu_backward.hh"

#include <stdexcept>

namespace nntile
{

NNGraph::TensorNode *NNReluOp::forward()
{
    if (x == nullptr)
    {
        throw std::invalid_argument("NNReluOp::forward: x must be non-null");
    }
    NNGraph *graph = x->graph();
    bool out_requires_grad = any_input_requires_grad({x});
    TensorGraph::TensorNode *y_data = tensor_graph::relu(x->data());
    NNGraph::TensorNode *y = graph->tensor(y_data, out_requires_grad);
    outputs_ = {y};
    return y;
}

void NNReluOp::backward() const
{
    NNGraph::TensorNode *out = output();
    if (out == nullptr)
    {
        return;
    }
    NNGraph *graph = out->graph();
    NNGraph::TensorNode *grad_out = out->grad();
    if (grad_out == nullptr)
    {
        return;
    }
    if (x != nullptr && x->requires_grad())
    {
        auto [grad_x, is_first] =
            graph->get_or_create_grad(x, nn_grad_slot_name(x));
        if (is_first)
        {
            tensor_graph::clear(grad_x->data());
        }
        tensor_graph::relu_backward(
            x->data(), grad_out->data(), grad_x->data());
    }
}

NNGraph::TensorNode *relu(NNGraph::TensorNode *x)
{
    if (x == nullptr)
    {
        throw std::invalid_argument("relu: x must be non-null");
    }
    NNGraph *graph = x->graph();
    auto op = std::make_shared<NNReluOp>(x);
    NNGraph::TensorNode *y = op->forward();
    graph->register_op(std::move(op));
    return y;
}

} // namespace nntile
