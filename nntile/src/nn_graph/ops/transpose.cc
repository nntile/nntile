 ...
 *
 * @file nntile/src/nn_graph/transpose.cc
...
NNGraph::TensorNode *NNTransposeOp::forward()
{
    if (src == nullptr)
    {
    ...
    TensorGraph::TensorNode *output_data =
        graph::tensor::transpose(1.0, src->data(), ndim);
    ...
    if (src->requires_grad())
    {
        NNGraph::TensorNode *grad_buf =
            graph->tensor(src->shape(), src->dtype(), false);
        buffers_ = {grad_buf};
    }
    return output;
}
