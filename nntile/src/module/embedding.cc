 ...
 *
 * @file nntile/src/module/embedding.cc
...
Embedding::Embedding(NNGraph *graph,
    ...
    dtype_(dtype)
{
    // Vocab tensor: [num_embeddings, embed_dim] (PyTorch / C-order)
    vocab_tensor_ =
    ...
    dtype_(vocab_tensor != nullptr ? vocab_tensor->dtype() : DataType::FP32)
{
    ...
    if (vocab_tensor->ndim() != 2)
    {
        throw std::invalid_argument(
            ...
            "got " +
            std::to_string(vocab_tensor->ndim()));
    }

    const auto &v_shape = vocab_tensor->shape();
    num_embeddings_ = v_shape.front();
    embed_dim_ = v_shape.back();
...
NNGraph::TensorNode *Embedding::forward(NNGraph::TensorNode *index)
{
    if (index == nullptr)
    {
        throw std::invalid_argument(
...
void Embedding::bind_weight(std::vector<std::uint8_t> data)
{
    ...
    {
        throw std::runtime_error("Embedding::bind_weight: vocab tensor is "
...
{
    const size_t expected = static_cast<size_t>(embed_dim_) * num_embeddings_;
...
{
    std::string result =
        "Embedding(num_embeddings=" + std::to_string(num_embeddings_) +
