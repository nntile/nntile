 ...
 *
 * @file nntile/src/nn_graph/norm_fiber.cc
...
NNGraph::TensorNode *NNNormFiberOp::forward()
{
    if (x == nullptr)
    {
    ...
    if (batch_ndim < 0 || batch_ndim > ndim)
    {
        throw std::invalid_argument(
            ...
            "batch_ndim=" +
            std::to_string(batch_ndim) + ", ndim=" + std::to_string(ndim));
    }
    ...
    TensorGraph::TensorNode *y_data = tensor_graph::norm_fiber(
...
void NNNormFiberOp::backward() const
{
    ...
    {
        throw std::runtime_error("norm_fiber backward is not implemented");
    }
}
    ...
    Scalar alpha)
{
    ...
    NNGraph *graph = x->graph();
    auto op =
        std::make_shared<NNNormFiberOp>(x, axis, batch_ndim, redux, alpha);
