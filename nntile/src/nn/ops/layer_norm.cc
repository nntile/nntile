 ...
 *
 * @file nntile/src/nn_graph/ops/layer_norm.cc
...
void NNLayerNormOp::backward() const
{
    ...
    if (x != nullptr && x->requires_grad())
    {
        ...
        TensorGraph::TensorNode *grad_temp_data = tensor::multiply_fiber(
            1.0, gamma->data(), grad_out->data(), f_axis);
        ...
        tensor::sumprod_slice(grad_temp->data(),
            ...
            0.0);
        tensor::multiply_slice(
            1.0, mean_buf->data(), tmp_y_grad->data(), f_axis);
        tensor::add_inplace(
            1.0, grad_temp->data(), 1.0, tmp_y_grad->data());
        tensor::sum_slice(
            grad_temp->data(), mean_buf->data(), f_axis, redux, inv_l, 0.0);
        tensor::add_slice_inplace(
            -1.0, mean_buf->data(), 1.0, tmp_y_grad->data(), f_axis);
        tensor::multiply_slice(
            1.0, inv_stddev->data(), tmp_y_grad->data(), f_axis);
        tensor::add_inplace(
    ...
    int redux)
{
    ...
    if (beta->ndim() != 1 || beta->shape()[0] != x->shape()[norm_axis])
    {
        throw std::invalid_argument(
    ...
    auto op = std::make_shared<NNLayerNormOp>(
        x, gamma, beta, norm_axis, eps, redux);
