 ...
 *
 * @file nntile/src/nn_graph/concat.cc
...
NNGraph::TensorNode *NNConcatOp::forward()
{
    ...
    const Index f_axis = nn::c_axis_to_fortran(axis, a->ndim());
    TensorGraph::TensorNode *out_data =
        tensor::concat(a->data(), b->data(), f_axis);
