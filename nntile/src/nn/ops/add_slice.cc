 ...
 *
 * @file nntile/src/nn_graph/add_slice.cc
...
NNGraph::TensorNode *NNAddSliceOp::forward()
{
    ...
    TensorGraph::TensorNode *output_data = tensor::add_slice(
        alpha, src1->data(), beta, src2->data(), storage_axis);
    ...
    Index axis)
{
    ...
    NNGraph *graph = src1->graph();
    auto op = std::make_shared<NNAddSliceOp>(src1, src2, alpha, beta, axis);
