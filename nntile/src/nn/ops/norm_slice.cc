 ...
 *
 * @file nntile/src/nn_graph/norm_slice.cc
...
std::vector<Index> norm_slice_output_shape(
    const std::vector<Index> &x_shape, Index axis)
{
...
NNGraph::TensorNode *NNNormSliceOp::forward()
{
    if (x == nullptr)
    {
    ...
    TensorGraph::TensorNode *y_data = tensor::norm_slice(
        alpha, x->data(), beta_fresh, base->data(), storage_axis, redux);
...
void NNNormSliceOp::backward() const
{
    ...
    {
        throw std::runtime_error("norm_slice backward is not implemented");
    }
}
    ...
    NNGraph::TensorNode *x, Index axis, int redux, Scalar alpha)
{
    ...
    NNGraph *graph = x->graph();
    auto op = std::make_shared<NNNormSliceOp>(x, axis, redux, alpha);
