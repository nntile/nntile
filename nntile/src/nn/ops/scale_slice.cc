 ...
 *
 * @file nntile/src/nn_graph/scale_slice.cc
...
NNGraph::TensorNode *NNScaleSliceOp::forward()
{
    if (src == nullptr)
    {
    ...
    const Index f_axis = nn::c_axis_to_fortran(axis, out_ndim);
    TensorGraph::TensorNode *output_data =
        tensor::scale_slice(alpha, src->data(), f_axis, axis_size);
...
void NNScaleSliceOp::backward() const
{
    ...
    if (src != nullptr && src->requires_grad())
    {
        ...
        const Index f_axis =
            nn::c_axis_to_fortran(axis, grad_out->ndim());
