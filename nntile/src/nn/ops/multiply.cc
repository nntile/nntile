 ...
 *
 * @file nntile/src/nn_graph/multiply.cc
...
NNGraph::TensorNode *NNMultiplyOp::forward()
{
    ...
    TensorGraph::TensorNode *z_data =
        tensor::multiply(x->data(), y->data(), alpha);
...
void NNMultiplyOp::backward() const
{
    ...
    if (x != nullptr && x->requires_grad())
    {
        ...
        TensorGraph::TensorNode *grad_x_buf =
            tensor::multiply(grad_out->data(), y->data(), alpha);
        tensor::add_inplace(1.0, grad_x_buf, grad_beta, grad_x->data());
    }
    if (y != nullptr && y->requires_grad())
    {
        ...
        TensorGraph::TensorNode *grad_y_buf =
            tensor::multiply(grad_out->data(), x->data(), alpha);
        tensor::add_inplace(1.0, grad_y_buf, grad_beta, grad_y->data());
    }
}
