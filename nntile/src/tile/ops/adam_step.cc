 ...
 *
 * @file nntile/src/tile_graph/adam_step.cc
 * TileGraph adam step operation implementation.
 *
 * @version 1.1.0
 * */
...
{
    nntile::core::adam_step<T>(runtime.starpu_worker_hint(), num_iter, beta_1, beta_2, eps, lr, weight_decay,
...
TileAdamStepOp::TileAdamStepOp(Index num_iter_, Scalar beta_1_, Scalar beta_2_,
...
{
    auto op = std::make_shared<TileAdamStepOp>(num_iter, beta_1, beta_2, eps, lr,
        weight_decay, grad, first_moment, second_moment, p);
    grad->graph()->add_op(op);
}

void TileAdamStepOp::execute(Runtime& runtime) const
{
    const DataType dtype = runtime.get_dtype(grad);
    switch(dtype)
    {
        case DataType::FP32:
            run_adam<nntile::fp32_t>(runtime, num_iter, beta_1, beta_2, eps, lr,
                weight_decay, grad, first_moment, second_moment, p);
            break;
        case DataType::FP32_FAST_TF32:
            run_adam<nntile::fp32_fast_tf32_t>(runtime, num_iter, beta_1, beta_2,
                eps, lr, weight_decay, grad, first_moment, second_moment, p);
            break;
        case DataType::FP32_FAST_FP16:
            run_adam<nntile::fp32_fast_fp16_t>(runtime, num_iter, beta_1, beta_2,
                eps, lr, weight_decay, grad, first_moment, second_moment, p);
            break;
        case DataType::FP32_FAST_BF16:
            run_adam<nntile::fp32_fast_bf16_t>(runtime, num_iter, beta_1, beta_2,
                eps, lr, weight_decay, grad, first_moment, second_moment, p);
            break;
        case DataType::FP64:
            run_adam<nntile::fp64_t>(runtime, num_iter, beta_1, beta_2, eps, lr,
                weight_decay, grad, first_moment, second_moment, p);
            break;
        case DataType::FP16:
            run_adam<nntile::fp16_t>(runtime, num_iter, beta_1, beta_2, eps, lr,
                weight_decay, grad, first_moment, second_moment, p);
            break;
        case DataType::BF16:
            run_adam<nntile::bf16_t>(runtime, num_iter, beta_1, beta_2, eps, lr,
                weight_decay, grad, first_moment, second_moment, p);
            break;
        case DataType::INT64:
        case DataType::BOOL:
