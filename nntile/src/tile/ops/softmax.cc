 ...
 *
 * @file nntile/src/tile_graph/softmax.cc
...
{
    nntile::core::softmax<T>(runtime.starpu_worker_hint(), runtime.get_tile<T>(m), runtime.get_tile<T>(s), a, runtime.get_tile<T>(d), ax);
}
} // namespace
void softmax(
    TileGraph::TileNode* maxsumexp_n, TileGraph::TileNode* src, Scalar alpha, TileGraph::TileNode* dst, Index axis)
{
    ...
    // maxsumexp vs. src shape rules follow nntile::core::softmax (see
    // src/core/softmax.cc)
