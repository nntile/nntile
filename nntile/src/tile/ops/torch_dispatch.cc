...
void torch_rms_norm(
    ...
    Scalar eps)
{
    auto op = std::make_shared<TileTorchRmsNormOp>(
        input,
        weight,
        out,
        rstd,
...
void TileTorchRmsNormOp::execute(Runtime &runtime) const
{
    auto &in_t = runtime.get_tile<fp32_t>(input);
    auto &out_t = runtime.get_tile<fp32_t>(out);
    auto &rstd_t = runtime.get_tile<fp32_t>(rstd);
...
void torch_rms_norm_backward(
    ...
    bool need_grad_weight)
{
    auto op = std::make_shared<TileTorchRmsNormBackwardOp>(
        grad_out,
        input,
        rstd,
        weight,
...
void TileTorchRmsNormBackwardOp::execute(Runtime &runtime) const
{
    auto &go_t = runtime.get_tile<fp32_t>(grad_out);
    auto &in_t = runtime.get_tile<fp32_t>(input);
    auto &rstd_t = runtime.get_tile<fp32_t>(rstd);
    ...
    core::torch_rms_norm_backward_out(
        runtime.starpu_worker_hint(),
        go_t,
        go_meta,
        in_t,
