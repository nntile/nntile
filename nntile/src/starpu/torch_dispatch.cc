...
std::vector<std::int64_t> trailing_dim_indices(
    ...
    std::int64_t n)
{
    ...
    {
        dims.push_back(input.dim() - n + i);
    }
    return dims;
}

std::vector<std::int64_t> leading_dim_indices(std::int64_t n)
{
    std::vector<std::int64_t> dims;
    dims.reserve(static_cast<size_t>(n));
    for (std::int64_t i = 0; i < n; ++i)
...
at::Tensor reshape_reduced_stat(
    at::Tensor stat,
    const at::Tensor &input,
    std::int64_t normalized_ndim)
{
    if (stat.dim() + normalized_ndim == input.dim())
    {
        std::vector<std::int64_t> sizes = stat.sizes().vec();
        for (std::int64_t i = 0; i < normalized_ndim; ++i)
        {
            sizes.push_back(1);
        }
        return stat.reshape(sizes);
    }
    return stat;
}
...
TorchRmsNorm::TorchRmsNorm():
    codelet("nntile_torch_rms_norm", footprint, cpu_funcs, cuda_funcs)
{
}

void TorchRmsNorm::cpu(void *buffers[], void *cl_args) noexcept
{
#ifndef STARPU_SIMGRID
    try
    {
        ...
        at::Tensor rstd = out_fp32(ifaces[2]->get_ptr<float>(), *args, 1);
        at::Tensor weight;
        if (has_w)
        {
            weight = in_fp32(ifaces[3]->get_ptr<float>(), *args, 1);
        }
        const std::int64_t n =
            static_cast<std::int64_t>(args->iargs[0]);
        std::vector<std::int64_t> dims = trailing_dim_indices(input, n);

        at::AutoDispatchBelowADInplaceOrView guard;
        at::NoGradGuard no_grad;
        at::Tensor variance = at::mean(input * input, dims, true);
        at::Tensor rstd_keep =
            at::rsqrt(variance + static_cast<double>(args->scalars[0]));
        at::Tensor result = input * rstd_keep;
        if (has_w)
        {
            result = result * weight;
        }
        out.copy_(result);
        rstd.copy_(rstd_keep.reshape(rstd.sizes()));
    }
    catch (const std::exception &ex)
    {
        std::fprintf(
            stderr,
            "nntile_torch_rms_norm failed: %s\n",
            ex.what());
        std::abort();
    }
#endif
...
#ifdef NNTILE_USE_CUDA
void TorchRmsNorm::cuda(void *buffers[], void *cl_args) noexcept
{
#ifndef STARPU_SIMGRID
    try
    {
    ...
    catch (const std::exception &ex)
    {
        std::fprintf(
            stderr,
            "nntile_torch_rms_norm CUDA failed: %s\n",
            ex.what());
        std::abort();
    }
#endif
...
uint32_t TorchRmsNorm::footprint(struct starpu_task *task)
{
    return args_footprint(
        reinterpret_cast<args_t *>(task->cl_arg));
}

void TorchRmsNorm::submit(
    int starpu_worker_hint,
    const args_t &meta,
    Handle input,
    Handle weight,
...
)
{
    ...
    if (ret != 0)
    {
        throw std::runtime_error("torch_rms_norm.submit failed");
    }
}

TorchRmsNormBackward::TorchRmsNormBackward():
    codelet(
        "nntile_torch_rms_norm_backward",
        footprint,
        cpu_funcs,
        cuda_funcs)
{
...
void TorchRmsNormBackward::cpu(void *buffers[], void *cl_args) noexcept
{
#ifndef STARPU_SIMGRID
    try
    {
        ...
        auto **ifaces =
            reinterpret_cast<VariableInterface **>(buffers);
        const bool has_w = args->iargs[1] != 0;
        const bool need_gi = args->iargs[2] != 0;
        const bool need_gw = args->iargs[3] != 0;
        Index buf = 0;
        float *grad_out_ptr = ifaces[buf++]->get_ptr<float>();
        float *input_ptr = ifaces[buf++]->get_ptr<float>();
        float *rstd_ptr = ifaces[buf++]->get_ptr<float>();
        ...
        if (has_w)
        {
            weight = in_fp32(ifaces[buf++]->get_ptr<float>(), *args, 3);
        }
        at::Tensor grad_out = in_fp32(grad_out_ptr, *args, 0);
        at::Tensor input = in_fp32(input_ptr, *args, 1);
        at::Tensor rstd = in_fp32(rstd_ptr, *args, 2);
        at::Tensor grad_input;
        at::Tensor grad_weight;
        if (need_gi)
        {
        ...
        const std::int64_t n =
            static_cast<std::int64_t>(args->iargs[0]);
        const std::int64_t axis = input.dim() - n;
        std::vector<std::int64_t> norm_dims =
            trailing_dim_indices(input, n);
        at::AutoDispatchBelowADInplaceOrView guard;
        at::NoGradGuard no_grad;
        at::Tensor rstd_keep = reshape_reduced_stat(rstd, input, n);
        at::Tensor normalized = input * rstd_keep;
        at::Tensor g_hat = has_w ? grad_out * weight : grad_out;
        if (need_gw)
        {
            at::Tensor gw = grad_out * normalized;
            if (axis > 0)
            {
                gw = at::sum(gw, leading_dim_indices(axis), false);
            }
            grad_weight.copy_(gw.reshape(grad_weight.sizes()));
        }
        if (need_gi)
        {
            at::Tensor mean = at::mean(g_hat * input, norm_dims, true);
            at::Tensor rstd2 = rstd_keep * rstd_keep;
            at::Tensor centered = g_hat - input * rstd2 * mean;
            grad_input.copy_(rstd_keep * centered);
        }
    }
    catch (const std::exception &ex)
    {
        std::fprintf(
            stderr,
            "nntile_torch_rms_norm_backward failed: %s\n",
            ex.what());
        std::abort();
    }
#endif
...
#ifdef NNTILE_USE_CUDA
void TorchRmsNormBackward::cuda(void *buffers[], void *cl_args) noexcept
{
#ifndef STARPU_SIMGRID
    try
    {
    ...
    catch (const std::exception &ex)
    {
        std::fprintf(
            stderr,
            "nntile_torch_rms_norm_backward CUDA failed: %s\n",
            ex.what());
        std::abort();
    }
#endif
...
uint32_t TorchRmsNormBackward::footprint(struct starpu_task *task)
{
    return args_footprint(
        reinterpret_cast<args_t *>(task->cl_arg));
}

void TorchRmsNormBackward::submit(
    int starpu_worker_hint,
    const args_t &meta,
    Handle grad_out,
    Handle input,
...
{
    args_t *args = clone_args(meta);
    args->kind = TorchKind::RmsNormBackward;
    args->iargs[1] = has_weight ? 1 : 0;
    args->iargs[2] = need_grad_input ? 1 : 0;
    args->iargs[3] = need_grad_weight ? 1 : 0;
    ...
    starpu_data_handle_t h[6];
    enum starpu_data_access_mode m[6];
    const Index n = static_cast<Index>(handles.size());
    if (n > 6)
    {
        std::free(args);
        throw std::runtime_error(
            "torch_rms_norm_backward.submit: too many handles");
    }
    for (Index i = 0; i < n; ++i)
    {
        m[static_cast<size_t>(i)] = handles[static_cast<size_t>(i)].first;
    ...
    default:
        std::free(args);
        throw std::runtime_error(
            "torch_rms_norm_backward.submit: bad handle count");
    }
    if (ret != 0)
    {
        throw std::runtime_error(
            "torch_rms_norm_backward.submit failed");
    }
}
...
TorchRmsNorm torch_rms_norm;
TorchRmsNormBackward torch_rms_norm_backward;
