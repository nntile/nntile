    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::rope_backward::cpu<T>(args->ncols, args->nrows, sin, cos, dy, dx);
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::rope_backward::cuda<T>(stream, args->ncols, args->nrows, sin, cos,
        dy, dx);
#endif // STARPU_SIMGRID
}
...
template<typename T>
void RopeBackward<std::tuple<T>>::submit(int starpu_worker_hint, Index ncols,
    Index nrows, Index sin_pair0, Handle sin, Handle cos, Handle dy, Handle dx)
