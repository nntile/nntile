 ...
 *
 * @file nntile/src/tensor_graph/relu_backward.cc
    ...
    TensorGraph::TensorNode *x, TensorGraph::TensorNode *dy)
{
    ...
    if (x->graph() != dy->graph())
        throw std::invalid_argument(
            "relu_backward: inputs must belong to same graph");
    if (x->dtype() != dy->dtype())
        throw std::invalid_argument(
            "relu_backward: inputs must have same dtype");
    if (x == dy)
        throw std::invalid_argument(
    ...
    TensorGraph::TensorNode *dx)
{
    ...
    if (x->graph() != dy->graph() || x->graph() != dx->graph())
        throw std::invalid_argument(
            "relu_backward: tensors must belong to same graph");
    if (x->dtype() != dy->dtype() || x->dtype() != dx->dtype())
        throw std::invalid_argument(
            "relu_backward: tensors must have same dtype");
    if (x == dy || x == dx || dy == dx)
        throw std::invalid_argument(
