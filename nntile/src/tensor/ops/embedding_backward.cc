 ...
 *
 * @file nntile/src/tensor_graph/embedding_backward.cc
                        ...
                        int redux)
{
    ...
    validate_embedding_shape_and_merge(
        embed, index, vocab, axis, "embedding_backward");
...
void TensorEmbeddingBackwardOp::lower_to_tile(const LoweringContext& ctx) const
{
    ...
    const Index vocab_b0 =
        uniform_tile_extent_along(*lay_v, vocab_embed_s, "EMBEDDING_BACKWARD");
    const Index embed_axis_bs =
        uniform_tile_extent_along(*lay_e, s_axis, "EMBEDDING_BACKWARD");
    if(embed_axis_bs % vocab_b0 != 0)
    {
        throw std::runtime_error(
            "lower_to_tile EMBEDDING_BACKWARD: embed tile extent along axis "
            "must be divisible by vocab tile extent along embed dim");
    }
