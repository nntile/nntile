...
KVCache::KVCache(const Config &config) : config_(config), cache_len_(0)
{
    ...
    Runtime &runtime, const std::string &prefix) const
{
    ...
    for (Index i = 0; i < config_.num_layers; ++i)
    {
        ...
        else
        {
            ...
            // For FP64 it expects double*. Convert from our dtype-specific
            // buffer.
            switch (config_.dtype)
            {
            case DataType::FP16:
    ...
    Runtime &runtime, const std::string &prefix)
{
    ...
    for (Index i = 0; i < config_.num_layers; ++i)
    {
        ...
        else
        {
            ...
            // For FP64 it returns double. Convert to our dtype-specific
            // buffer.
            switch (config_.dtype)
            {
            case DataType::FP16:
...
void KVCache::append(const float *k_data, const float *v_data, Index seq_len)
{
    ...
    {
        throw std::invalid_argument("KVCache::append: would exceed max_seq");
    }
            ...
            for (Index s = 0; s < seq_len; ++s)
            {
                for (Index b = 0; b < B; ++b)
                {
                    for (Index n = 0; n < N; ++n)
                    {
                        ...
                        std::memcpy(k_dst + dst_offset,
                            k_src + src_offset,
                            static_cast<size_t>(H) * sizeof(float));
                        std::memcpy(v_dst + dst_offset,
                            v_src + src_offset,
                            static_cast<size_t>(H) * sizeof(float));
                    }
                }
            ...
            for (Index s = 0; s < seq_len; ++s)
            {
                for (Index b = 0; b < B; ++b)
                {
                    for (Index n = 0; n < N; ++n)
                    {
                        ...
                        case DataType::FP64:
                        {
                            double *k_dst = reinterpret_cast<double *>(
                                k_buffers_bytes_[layer].data());
                            double *v_dst = reinterpret_cast<double *>(
                                v_buffers_bytes_[layer].data());
                            for (Index h = 0; h < H; ++h)
                            {
                                k_dst[dst_offset + h] =
