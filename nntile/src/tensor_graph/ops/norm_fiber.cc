 ...
 *
 * @file nntile/src/tensor_graph/norm_fiber.cc
...
void TensorNormFiberOp::lower_to_tile(const LoweringContext &ctx) const
{
    ...
    if (lay1 == nullptr)
    {
        throw std::runtime_error(
            "lower_to_tile NORM_FIBER: missing tiling for src1");
    }

    tile_lower::assert_same_elementwise_layout(
        src2, dst, "NORM_FIBER src2/dst");

    const TensorAxisLayout *lay_d = ctx.tiling.find(dst);
    if (lay_d == nullptr)
    {
        throw std::runtime_error(
            "lower_to_tile NORM_FIBER: missing tiling for dst");
    }
