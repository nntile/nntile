 ...
 *
 * @file nntile/src/tensor_graph/gather.cc
...
void TensorGatherOp::lower_to_tile(const LoweringContext &ctx) const
{
    ...
    if (lay_src == nullptr)
    {
        throw std::runtime_error(
            "lower_to_tile GATHER: missing tiling for src");
    }
...
void gather(TensorGraph::TensorNode *src, TensorGraph::TensorNode *dst)
{
    ...
    {
        throw std::invalid_argument("gather: input tensors must be non-null");
    }
