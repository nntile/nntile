...
void bind_data_from_hint_impl(
    Runtime &rt, TensorGraph::TensorNode const *tensor)
{
    if (tensor == nullptr)
    {
        throw std::invalid_argument(
            "Runtime::bind_data_from_hint: tensor must be non-null");
    }
    const std::vector<std::uint8_t> *hint = tensor->get_bind_hint();
    if (hint == nullptr || hint->empty())
    {
        throw std::runtime_error(
            "Runtime::bind_data_from_hint: no bind_hint on tensor '" +
            tensor->name() + "'");
    }
    switch (tensor->dtype())
    {
    ...
    default:
        throw std::runtime_error(
            "Runtime::bind_data_from_hint: unsupported dtype for '" +
            tensor->name() + "'");
    }
}
...
void Runtime::bind_data_from_hint(TensorGraph::TensorNode const *tensor)
{
    bind_data_from_hint_impl(*this, tensor);
}
