 ...
 *
 * @file nntile/tests/tile_graph/multiply.cc
...
TEST_CASE("multiply mixed tile parity", "[graph][tile]")
{
    ...
    b->set_name("b");
    nntile::TensorRef z = nntile::TensorRef::adopt(gt::multiply(a, b, Scalar{0.5f}));
    ...
    nntile::TensorRef zt = nntile::TensorRef::adopt(gt::multiply(at, bt, Scalar{0.5f}));
    ...
    rt_tile.execute();
    rt_tile.wait();
    const std::vector<float> y_tile = rt_tile.get_output<float>(zt);
