 ...
 * Test TileGraph silu backward vs nntile::core (parity).
 *
 * @version 1.1.0
 * */
...
TEST_CASE_METHOD(nntile::test::ContextFixture, "TileGraph silu_backward matches tile", "[graph][tile]")
{
    ...
    tg::silu_backward(Scalar{1.0}, x, dy, Scalar{0.0}, dx);
    Runtime runtime(g);
    runtime.compile();
    ...
    nntile::core::silu_backward<fp32_t>(-1, Scalar{1.0}, tx, tdy, Scalar{0.0}, tdx);
