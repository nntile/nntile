 ...
 *
 * @file nntile/tests/tile_graph/add.cc
 * Test TileGraph add operation against nntile::core::add.
 *
 * @version 1.1.0
 * */
    ...
    const std::vector<Index> &shape, Scalar alpha, Scalar beta)
{
    ...
    nntile::core::Tile<T> src1(shape);
    nntile::core::Tile<T> src2(shape);
    nntile::core::Tile<T> dst(shape);
...
TEST_CASE("TileGraph add structure", "[graph][tile]")
{
    ...
    auto *z = tg::add(alpha, x, beta, y)->set_name("z");
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "TileGraph add matches nntile::core::add",
    "[graph][tile]")
{
