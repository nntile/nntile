 ...
 *
 * @file nntile/tests/tile_graph/add_slice_inplace.cc
 * Test TileGraph add slice inplace vs nntile::core (parity).
 *
 * @version 1.1.0
 * */
...
TEST_CASE_METHOD(nntile::test::ContextFixture, "TileGraph add_slice_inplace", "[graph][tile]")
{
    ...
    { auto A = T1.acquire(STARPU_W), B = T2.acquire(STARPU_W);
      ...
      A.release(); B.release(); }
    nntile::core::add_slice_inplace<fp32_t>(a, T1, b, T2, axis);
