 ...
 *
 * @file nntile/tests/tensor_graph/relu_backward.cc
 * Test TensorGraph relu_backward operation.
 *
 * @version 1.1.0
 * */
...
TEST_CASE("TensorGraph relu_backward structure", "[graph][tensor]")
{
    ...
    nntile::TensorRef dx = nntile::TensorRef::adopt(gt::relu_backward(Scalar{1.0}, x, dy));
    dx->set_name("dx");
    ...
    "TensorGraph relu_backward rejects duplicate tensors", "[graph][tensor]")
{
    ...
    REQUIRE_THROWS_AS(gt::relu_backward(Scalar{1.0}, x, x), std::invalid_argument);
    REQUIRE_THROWS_AS(gt::relu_backward(Scalar{1.0}, x, dy, Scalar{0.0}, x), std::invalid_argument);
    REQUIRE_THROWS_AS(gt::relu_backward(Scalar{1.0}, x, dy, Scalar{0.0}, dy), std::invalid_argument);
}
    ...
    "[graph][tensor]")
{
    ...
    dx_node->set_name("dx");
        gt::relu_backward(Scalar{1.0}, x_node, dy_node, Scalar{0.0}, dx_node);
