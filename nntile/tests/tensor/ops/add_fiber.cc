 ...
 *
 * @file nntile/tests/tensor_graph/add_fiber.cc
...
template <typename T>
void check_add_fiber_vs_tensor_api(const std::vector<Index> &tensor_shape,
    Index axis,
    Index batch_ndim,
    Scalar alpha,
    Scalar beta)
{
    ...
    auto *out_node =
        gt::add_fiber(alpha, fiber_node, beta, tensor_node, axis, batch_ndim);
