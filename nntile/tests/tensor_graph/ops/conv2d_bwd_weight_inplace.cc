 ...
 *
 * @file nntile/tests/tensor_graph/conv2d_bwd_weight_inplace.cc
...
// dY shape from X shape (W,H,C_in,N), dC shape (K_x,K_y,C_in,C_out), padding,
// stride, dilation
...
{
    Index W_out =
        (x_shape[0] + 2 * padding[0] - dilation[0] * (dc_shape[0] - 1) - 1) /
            stride[0] +
        1;
    Index H_out =
        (x_shape[1] + 2 * padding[1] - dilation[1] * (dc_shape[1] - 1) - 1) /
            stride[1] +
        1;
...
TEST_CASE("TensorGraph conv2d_bwd_weight_inplace rejects null tensors",
    "[graph][tensor]")
{
    TensorGraph graph("test");
    auto *x = graph.data({4, 4, 2, 2})->set_name("x");
    ...
    const std::array<Index, 2> &dilation)
{
    ...
    auto dy_shape =
        conv2d_output_shape(x_shape, dc_shape, padding, stride, dilation);
    ...
    auto *dc_node = graph.data(dc_shape, DataType::FP32)->set_name("dc");
    x_node->mark_input(true);
    dy_node->mark_input(true);
    dc_node->mark_input(true);
    dc_node->mark_output(true);

    gt::conv2d_bwd_weight_inplace(
        alpha, x_node, dy_node, beta, dc_node, padding, stride, dilation);
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    ...
    "[graph][tensor]")
{
    const auto [x_shape, dc_shape, alpha, beta, padding, stride, dilation] =
        GENERATE(std::tuple{std::vector<Index>{4, 4, 2, 2},
