 ...
 *
 * @file nntile/tests/tensor_graph/sum_fiber.cc
...
template <typename T>
void check_sum_fiber_vs_tensor_api(const std::vector<Index> &x_shape,
    Index axis,
    Index batch_ndim,
    int redux,
    Scalar alpha,
    Scalar beta)
{
    ...
    std::vector<Index> y_shape =
        sum_fiber_output_shape(x_shape, axis, batch_ndim);
