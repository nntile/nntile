 ...
 *
 * @file nntile/tests/nn_graph/multiply_slice.cc
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "NNGraph multiply_slice structure",
    "[graph][nn_graph]")
{
    ...
    auto *tensor_node =
        g.tensor(tensor_shape, DataType::FP32)->set_name("tensor");
    fill(1.0, tensor_node);
    auto *out =
        multiply_slice(alpha, slice_node, tensor_node, axis)->set_name("out");
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "NNGraph multiply_slice backward",
    "[graph][nn_graph]")
{
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "NNGraph multiply_slice forward matches PyTorch",
    "[graph][nn_graph][pytorch]")
{
    ...
    auto *slice_node =
        g.tensor(slice_sh, DataType::FP32, true)->set_name("slice");
    auto *tensor_node = g.tensor(dst_sh, DataType::FP32)->set_name("tensor");
    fill(1.0, tensor_node);
    auto *out =
        multiply_slice(alpha, slice_node, tensor_node, axis)->set_name("out");
    ...
    runtime.compile();
    runtime.bind_data(slice_node, slice_data);
    runtime.execute();
    runtime.wait();
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "NNGraph multiply_slice backward matches PyTorch",
    "[graph][nn_graph][pytorch]")
{
    const auto [alpha, axis, grad_fill_val] =
        GENERATE(std::tuple{Scalar(1.0), Index(1), Scalar(1.0)},
