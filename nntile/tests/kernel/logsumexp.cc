...
struct TestData
{
    ...
    std::vector<T> maxsumexp;    // Size: 2*num_elems (C-order [2, num_elems])
...
void reference_logsumexp(TestData<T>& data)
{
    ...
    {
        ref_t max_val = static_cast<Y>(data.maxsumexp[i]);
        ref_t sum_val = static_cast<Y>(data.maxsumexp[data.num_elems + i]);
...
void generate_data(TestData<T>& data, Index num_elems, DataGen strategy)
{
    ...
    switch(strategy)
    {
        ...
        case DataGen::PRESET:
            ...
            {
                data.maxsumexp[i] = Y(i + 1);      // max value
                data.maxsumexp[num_elems + i] = Y(10.0 + i); // sum of exponents
        ...
        case DataGen::RANDOM:
            ...
            for(Index i = 0; i < num_elems; ++i)
            {
                ...
                data.maxsumexp[num_elems + i] = dist_sum(gen);   // sum of exponents
