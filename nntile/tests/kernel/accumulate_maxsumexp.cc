...
void reference_accumulate_maxsumexp(TestData<T>& data)
{
    ...
    {
        const ref_t src_max = static_cast<Y>(data.src_init[i * 2]);
        const ref_t src_sumexp = static_cast<Y>(data.src_init[i * 2 + 1]);
        const ref_t dst_max = static_cast<Y>(data.dst_ref[i * 2]);
        const ref_t dst_sumexp = static_cast<Y>(data.dst_ref[i * 2 + 1]);
        ...
        if(src_sumexp != 0.0)
        {
            ...
            if(dst_sumexp == 0.0)
            {
                data.dst_ref[i * 2] = data.src_init[i * 2];
                data.dst_ref[i * 2 + 1] = data.src_init[i * 2 + 1];
            }
            ...
            else if(dst_max < src_max)
            {
                ...
                data.dst_ref[i * 2 + 1] = static_cast<Y>(new_sumexp);
                data.dst_ref[i * 2] = data.src_init[i * 2];
            }
            else
            {
    ...
    switch(strategy)
    {
        ...
        case DataGen::PRESET:
            for(Index i = 0; i < nelems; ++i)
            {
                ...
                data.src_init[i * 2] = src_max;
                const Y src_sumexp = 0.1 + i * 0.1;
                data.src_init[i * 2 + 1] = src_sumexp;

                // Set initial dst values
                const Y dst_max = -1.0 + i * 0.3;
                data.dst_init[i * 2] = dst_max;
                const Y dst_sumexp = 0.05 + i * 0.05;
                data.dst_init[i * 2 + 1] = dst_sumexp;
            }
            break;
        ...
        case DataGen::RANDOM:
            ...
            for(Index i = 0; i < nelems; ++i)
            {
                data.src_init[i * 2] = dist_max(gen);
                data.src_init[i * 2 + 1] = dist_sumexp(gen);
                data.dst_init[i * 2] = dist_max(gen);
                data.dst_init[i * 2 + 1] = dist_sumexp(gen);
            }
            break;
    }
}
...
)
{
    ...
    for(Index i = 0; i < data.nelems; ++i)
    {
        Y dst_max_ref = static_cast<Y>(data.dst_ref[i * 2]);
        Y dst_sumexp_ref = static_cast<Y>(data.dst_ref[i * 2 + 1]);

        REQUIRE(static_cast<Y>(dst[i * 2]) == dst_max_ref);
        REQUIRE_THAT(
            static_cast<Y>(dst[i * 2 + 1]),
