...
void validate_cpu(Index nelems)
{
    ...
    std::cout << "Run starpu::gelutanh_inplace::submit<" << T::short_name << "> restricted to CPU\n";
    gelutanh_inplace.submit<std::tuple<T>>(-1, nelems, data2_handle);
    ...
    std::cout << "OK: starpu::gelutanh_inplace::submit<" << T::short_name << "> restricted to CPU\n";
}
...
void validate_cuda(Index nelems)
{
    ...
    kernel::gelutanh_inplace::cuda<T>(stream, nelems, dev_data);
    ...
    std::cout << "Run starpu::gelutanh_inplace::submit<" << T::short_name << "> restricted to CUDA\n";
    gelutanh_inplace.submit<std::tuple<T>>(-1, nelems, data2_handle);
    starpu_task_wait_for_all();
    data2_handle.unregister();
    // Check result
    ...
    std::cout << "OK: starpu::gelutanh_inplace::submit<" << T::short_name << "> restricted to CUDA\n";
}
#endif // NNTILE_USE_CUDA
