...
void validate_cpu(Index m, Index n, Index k)
{
    ...
    std::vector<T> dst2(m*n*k);
    // Launch low-level kernel
    std::cout << "Run kernel::scale_slice::cpu<" << T::short_name << ">\n";
    kernel::scale_slice::cpu<T>(m, n, k, 0.5, &src[0], &dst[0]);
    ...
    std::cout << "OK: starpu::scale_slice::submit<" << T::short_name << "> restricted to CPU\n";
}
...
void validate_cuda(Index m, Index n, Index k)
{
    ...
    TEST_ASSERT(cuda_err == cudaSuccess);
    std::cout << "Run kernel::scale_slice::cuda<" << T::short_name << ">\n";
    kernel::scale_slice::cuda<T>(stream, m, n, k, 0.5, dev_src, dev_dst);
    ...
    std::cout << "OK: starpu::scale_slice::submit<" << T::short_name << "> restricted to CUDA\n";
}
#endif // NNTILE_USE_CUDA
