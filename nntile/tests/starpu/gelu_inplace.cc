 ...
 *
 * @file tests/starpu/gelu_inplace.cc
...
void validate_cpu(Index nelems)
{
    ...
    std::cout << "Run kernel::gelu_inplace::cpu<" << T::short_name << ">\n";
    kernel::gelu_inplace::cpu<T>(nelems, &data[0]);
    // Check by actually submitting a task
    VariableHandle data2_handle(&data2[0], sizeof(T)*nelems);
    gelu_inplace.restrict_where(STARPU_CPU);
    std::cout << "Run starpu::gelu_inplace::submit<" << T::short_name << "> restricted to CPU\n";
    gelu_inplace.submit<std::tuple<T>>(-1, nelems, data2_handle);
    ...
    std::cout << "OK: starpu::gelu_inplace::submit<" << T::short_name << "> restricted to CPU\n";
}
...
void validate_cuda(Index nelems)
{
    ...
    TEST_ASSERT(cuda_err == cudaSuccess);
    std::cout << "Run kernel::gelu_inplace::cuda<" << T::short_name << ">\n";
    kernel::gelu_inplace::cuda<T>(stream, nelems, dev_data);
    ...
    std::cout << "Run starpu::gelu_inplace::submit<" << T::short_name << "> restricted to CUDA\n";
    gelu_inplace.submit<std::tuple<T>>(-1, nelems, data2_handle);
    starpu_task_wait_for_all();
    data2_handle.unregister();
    // Check result
    ...
    std::cout << "OK: starpu::gelu_inplace::submit<" << T::short_name << "> restricted to CUDA\n";
}
#endif // NNTILE_USE_CUDA
