 ...
 *
 * @file nntile/tests/model/gpt2/gpt2_mlp.cc
 * Tests for Gpt2MLP.
 *
 * @version 1.1.0
 * */
...
TEST_CASE(
    "Gpt2MLP tests skipped (GPT2_DATA_DIR undefined)", "[model][gpt2]")
{
...
constexpr char gpt2_mlp[] = "gpt2_mlp";
...
TEST_CASE("Gpt2MLP forward builds output", "[model][gpt2]")
{
    MlpFixtureSpec fx;
    if (!skip_unless_fixture_ready(mlp_fixture_stem::gpt2_mlp, fx))
    {
        SKIP("Missing or invalid gpt2_mlp.json / .safetensors.");
    }
    NNGraph g("gpt2_mlp");
    Gpt2MLP mlp(&g, "mlp", fx.config);
...
TEST_CASE("Gpt2MLP load from safetensors roundtrip", "[model][gpt2][io]")
{
    ...
    const std::string data_path =
        mlp_fixture_safetensors_path(std::string(GPT2_DATA_DIR), fx);

    NNGraph g1("load_graph");
    Gpt2MLP mlp1(&g1, "mlp", fx.config);
    mlp1.load(data_path);

    const std::string save_path =
        "/tmp/nntile_gpt2_mlp_roundtrip.safetensors";
    mlp1.save(save_path);
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "Gpt2MLP matches PyTorch reference",
    "[model][gpt2]")
{
    MlpFixtureSpec fx;
    if (!skip_unless_fixture_ready(mlp_fixture_stem::gpt2_mlp, fx))
    {
        SKIP("GPT-2 MLP fixture pair not found.");
    }
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "Gpt2MLP backward matches PyTorch reference",
    "[model][gpt2]")
{
    MlpFixtureSpec fx;
    if (!skip_unless_fixture_ready(mlp_fixture_stem::gpt2_mlp, fx))
