 ...
 *
 * @file nntile/tests/model/gpt2/gpt2_causal.cc
 * Tests for Gpt2Causal.
 *
 * @version 1.1.0
 * */
...
TEST_CASE(
    "Gpt2Causal tests skipped (GPT2_DATA_DIR undefined)", "[model][gpt2]")
{
...
constexpr char gpt2_causal[] = "gpt2_causal";
...
TEST_CASE("Gpt2Causal forward builds output", "[model][gpt2]")
{
    CausalFixtureSpec fx;
    if (!skip_unless_fixture_ready(causal_fixture_stem::gpt2_causal, fx))
    {
        SKIP("Missing or invalid gpt2_causal.json / .safetensors.");
    }
    NNGraph g("gpt2_causal");
    Gpt2Causal model(&g, "model", fx.config);
    auto *input_ids =
...
TEST_CASE("Gpt2Causal load from safetensors roundtrip", "[model][gpt2][io]")
{
    ...
    Gpt2Causal model1(&g1, "model", fx.config);
    model1.load(data_path);

    const std::string save_path =
        "/tmp/nntile_gpt2_causal_roundtrip.safetensors";
