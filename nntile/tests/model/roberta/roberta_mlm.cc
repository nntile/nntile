 ...
 *
 * @file nntile/tests/model/roberta/roberta_mlm.cc
 * Tests for RobertaMlm.
 *
 * @version 1.1.0
 * */
...
TEST_CASE(
    "RobertaMlm tests skipped (ROBERTA_DATA_DIR undefined)", "[model][roberta]")
{
...
inline std::string mlm_fixture_path(
...
TEST_CASE("RobertaMlm forward builds logits", "[model][roberta]")
{
    MlmFixtureSpec fx;
    if (!try_load_mlm_fixture_spec(
            std::string(ROBERTA_DATA_DIR), "roberta_mlm", fx))
    {
        SKIP("Missing roberta_mlm fixture.");
    }
...
TEST_CASE("RobertaMlm load from safetensors roundtrip", "[model][roberta][io]")
{
    ...
    const std::string data_path = mlm_fixture_path(
        std::string(ROBERTA_DATA_DIR), fx);

    NNGraph g1("load_graph");
    RobertaMlm model1(&g1, "model", fx.config);
    model1.load(data_path);

    const std::string save_path =
        "/tmp/nntile_roberta_mlm_roundtrip.safetensors";
    ...
    "RobertaMlm forward matches PyTorch reference", "[model][roberta]")
{
    ...
    {
        SKIP("RoBERTa MLM fixture not found.");
    }
    const std::string full_path = mlm_fixture_path(dir, fx);
