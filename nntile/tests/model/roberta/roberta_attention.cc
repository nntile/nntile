 ...
 *
 * @file nntile/tests/model/roberta/roberta_attention.cc
 * Tests for BertAttention with RoBERTa reference weights.
 *
 * @version 1.1.0
 * */
...
TEST_CASE(
    "RobertaAttention tests skipped (ROBERTA_DATA_DIR undefined)",
    "[model][roberta]")
{
...
inline std::string attention_fixture_path(
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "RobertaAttention forward matches PyTorch reference",
    "[model][roberta]")
{
    constexpr char stem[] = "roberta_attention";
    AttentionFixtureSpec fx;
    const std::string dir = std::string(ROBERTA_DATA_DIR);
    if (!try_load_attention_fixture_spec(dir, stem, fx))
    {
        SKIP("RoBERTa attention fixture not found.");
    }
    const std::string full_path = attention_fixture_path(dir, fx);
    std::ifstream st(full_path);
    if (!st.good())
    {
        SKIP("RoBERTa attention safetensors not found.");
    }
