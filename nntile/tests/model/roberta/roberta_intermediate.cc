 ...
 *
 * @file nntile/tests/model/roberta/roberta_intermediate.cc
 * Tests for BertIntermediate with RoBERTa reference weights.
 *
 * @version 1.1.0
 * */
...
TEST_CASE(
    "RobertaIntermediate tests skipped (ROBERTA_DATA_DIR undefined)",
    "[model][roberta]")
{
...
inline bool try_load_mlp_fixture_spec(
    const std::string &data_dir, const char *stem, MlpFixtureSpec &out)
{
    out = {};
    out.stem = stem;
    ...
    "[model][roberta]")
{
    ...
    {
        NNGraph g("roberta_intermediate_ref");
