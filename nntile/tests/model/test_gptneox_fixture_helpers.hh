 ...
 *
 * @file nntile/tests/model/test_gptneox_fixture_helpers.hh
...
inline void prepare_gptneox_config(
    nntile::model::gptneox::GptneoxConfig &cfg)
{
...
{
    nntile::NNGraph::TensorNode *sin = nullptr;
    nntile::NNGraph::TensorNode *cos = nullptr;
    ...
    GptneoxRopeInputs &out)
{
    ...
    const Index rope_dim =
        nntile::model::gptneox::gptneox_rope_dim(config);
    ...
    out.cos = g.tensor({n_batch, n_seq, half}, nntile::DataType::FP32)
                  ->set_name("rope_cos");
    safetensors_nntile_layout::read_tensor_from_safetensors(
...
inline void bind_rope_inputs(
    nntile::Runtime &runtime, const GptneoxRopeInputs &rope)
{
    if(rope.sin == nullptr)
    {
        return;
