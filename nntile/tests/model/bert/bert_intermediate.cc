 ...
 *
 * @file nntile/tests/model/bert/bert_intermediate.cc
 * Tests for BertIntermediate.
 *
 * @version 1.1.0
 * */
...
TEST_CASE(
    "BertIntermediate tests skipped (BERT_DATA_DIR undefined)", "[model][bert]")
{
...
constexpr char bert_intermediate[] = "bert_intermediate";
...
TEST_CASE("BertIntermediate forward builds output", "[model][bert]")
{
    MlpFixtureSpec fx;
    if (!skip_unless_fixture_ready(mlp_fixture_stem::bert_intermediate, fx))
    {
        SKIP("Missing or invalid bert_intermediate.json / .safetensors.");
    }
...
TEST_CASE("BertIntermediate load from safetensors roundtrip", "[model][bert][io]")
{
    MlpFixtureSpec fx;
    if (!skip_unless_fixture_ready(mlp_fixture_stem::bert_intermediate, fx))
    {
    ...
    const std::string save_path =
        "/tmp/nntile_bert_intermediate_roundtrip.safetensors";
    mlp1.save(save_path);
    ...
    "[model][bert]")
{
    ...
    if (!skip_unless_fixture_ready(mlp_fixture_stem::bert_intermediate, fx))
    {
        SKIP("BERT MLP fixture pair not found.");
    }
