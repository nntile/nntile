 ...
 *
 * @file nntile/tests/model/gptneo/gptneo_mlp.cc
 * Tests for GptneoMLP.
 *
 * @version 1.1.0
 * */
...
TEST_CASE(
    "GptneoMLP tests skipped (GPTNEO_DATA_DIR undefined)", "[model][gptneo]")
{
...
constexpr char gptneo_mlp[] = "gptneo_mlp";
...
TEST_CASE("GptneoMLP forward builds output", "[model][gptneo]")
{
    MlpFixtureSpec fx;
    if (!skip_unless_fixture_ready(mlp_fixture_stem::gptneo_mlp, fx))
    {
        SKIP("Missing or invalid gptneo_mlp.json / .safetensors.");
    }
    NNGraph g("gptneo_mlp");
    GptneoMLP mlp(&g, "mlp", fx.config);
...
TEST_CASE("GptneoMLP load from safetensors roundtrip", "[model][gptneo][io]")
{
    MlpFixtureSpec fx;
    if (!skip_unless_fixture_ready(mlp_fixture_stem::gptneo_mlp, fx))
    {
    ...
    const std::string data_path =
        mlp_fixture_safetensors_path(std::string(GPTNEO_DATA_DIR), fx);

    NNGraph g1("load_graph");
    GptneoMLP mlp1(&g1, "mlp", fx.config);
    mlp1.load(data_path);

    const std::string save_path =
        "/tmp/nntile_gptneo_mlp_roundtrip.safetensors";
    mlp1.save(save_path);
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "GptneoMLP matches PyTorch reference",
    "[model][gptneo]")
{
    MlpFixtureSpec fx;
    if (!skip_unless_fixture_ready(mlp_fixture_stem::gptneo_mlp, fx))
    {
        SKIP("GPT-Neo MLP fixture pair not found.");
    }
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "GptneoMLP backward matches PyTorch reference",
    "[model][gptneo]")
{
    MlpFixtureSpec fx;
    if (!skip_unless_fixture_ready(mlp_fixture_stem::gptneo_mlp, fx))
