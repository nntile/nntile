 ...
 *
 * @file nntile/tests/model/gptneo/gptneo_model.cc
 * Tests for GptneoModel.
 *
 * @version 1.1.0
 * */
...
TEST_CASE(
    "GptneoModel tests skipped (GPTNEO_DATA_DIR undefined)", "[model][gptneo]")
{
...
constexpr char gptneo_model[] = "gptneo_model";
...
void model_backward_compare_ref(const ModelFixtureSpec &fx)
{
    const std::string full_path =
        model_fixture_safetensors_path(std::string(GPTNEO_DATA_DIR), fx);
    ...
    std::vector<float> grad_wpe_result;
    {
        ...
        GptneoModel model(&g, "model", fx.config);
        ...
        model.wpe_vocab_tensor()->grad()->mark_output(true);
        ...
        grad_wpe_result =
            runtime.get_output<float>(model.wpe_vocab_tensor()->grad());
    }
...
TEST_CASE("GptneoModel forward builds output", "[model][gptneo]")
{
    ModelFixtureSpec fx;
    if (!skip_unless_fixture_ready(model_fixture_stem::gptneo_model, fx))
    {
        SKIP("Missing or invalid gptneo_model.json / .safetensors.");
    }
    NNGraph g("gptneo_model");
    GptneoModel model(&g, "model", fx.config);
    auto *input_ids =
...
TEST_CASE("GptneoModel load from safetensors roundtrip", "[model][gptneo][io]")
{
    ModelFixtureSpec fx;
    if (!skip_unless_fixture_ready(model_fixture_stem::gptneo_model, fx))
    {
    ...
    GptneoModel model1(&g1, "model", fx.config);
    model1.load(data_path);

    const std::string save_path =
        "/tmp/nntile_gptneo_model_roundtrip.safetensors";
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "GptneoModel forward matches PyTorch reference", "[model][gptneo]")
{
    ModelFixtureSpec fx;
    if (!skip_unless_fixture_ready(model_fixture_stem::gptneo_model, fx))
    {
        SKIP("GPT-Neo model fixture not found.");
    }
    const std::string full_path =
        model_fixture_safetensors_path(std::string(GPTNEO_DATA_DIR), fx);
    SafeTensorsReader reader(full_path);
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "GptneoModel backward matches PyTorch reference",
    "[model][gptneo]")
{
    ModelFixtureSpec fx;
    if (!skip_unless_fixture_ready(model_fixture_stem::gptneo_model, fx))
