 ...
 *
 * @file nntile/tests/model/gptneo/gptneo_decoder.cc
 * Tests for GptneoDecoder.
 *
 * @version 1.1.0
 * */
...
TEST_CASE(
    "GptneoDecoder tests skipped (GPTNEO_DATA_DIR undefined)", "[model][gptneo]")
{
...
constexpr char gptneo_decoder[] = "gptneo_decoder";
...
void decoder_forward_compare_ref(const DecoderFixtureSpec &fx)
{
    const std::string full_path =
        decoder_fixture_safetensors_path(std::string(GPTNEO_DATA_DIR), fx);
    ...
    std::vector<float> result;
    {
        ...
        auto *output = decoder.forward(input, mask);
...
TEST_CASE("GptneoDecoder forward builds output", "[model][gptneo]")
{
    DecoderFixtureSpec fx;
    if (!skip_unless_fixture_ready(decoder_fixture_stem::gptneo_decoder, fx))
    {
        SKIP("Missing or invalid gptneo_decoder.json / .safetensors.");
    }
...
TEST_CASE("GptneoDecoder load from safetensors roundtrip", "[model][gptneo][io]")
{
    ...
    GptneoDecoder decoder1(&g1, "decoder", fx.config);
    decoder1.load(data_path);

    const std::string save_path =
        "/tmp/nntile_gptneo_decoder_roundtrip.safetensors";
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "GptneoDecoder forward matches PyTorch reference", "[model][gptneo]")
{
    DecoderFixtureSpec fx;
    if (!skip_unless_fixture_ready(decoder_fixture_stem::gptneo_decoder, fx))
    {
        SKIP("GPT-Neo decoder fixture not found.");
    }
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "GptneoDecoder backward matches PyTorch reference",
    "[model][gptneo]")
{
    DecoderFixtureSpec fx;
    if (!skip_unless_fixture_ready(decoder_fixture_stem::gptneo_decoder, fx))
