 ...
 *
 * @file nntile/tests/model/test_gptneo_fixture_helpers.hh
 * Shared JSON, position_ids, and attention-mask helpers for GPT-Neo graph model
 * tests.
 *
 * @version 1.1.0
 * */
...
inline void prepare_gptneo_config(nntile::model::gptneo::GptneoConfig &cfg)
{
    ...
    std::vector<std::int64_t> &pos_data)
{
        ...
        info.shape[1] != n_seq)
    {
        throw std::runtime_error(
            "GPT-Neo test fixture: position_ids shape mismatch");
    }
    ...
    std::vector<std::uint8_t> &mask_bytes)
{
    ...
    {
        throw std::runtime_error(
            "GPT-Neo test fixture: attn_mask shape mismatch");
    }
    ...
    if (info.dtype == nntile::DataType::BOOL)
    {
        ...
        {
            throw std::runtime_error(
                "GPT-Neo test fixture: BOOL attn_mask byte size mismatch");
        }
    ...
    if (info.dtype == nntile::DataType::FP32)
    {
        ...
        {
            throw std::runtime_error(
                "GPT-Neo test fixture: F32 attn_mask byte size mismatch");
        }
    ...
    throw std::runtime_error(
        "GPT-Neo test fixture: attn_mask must be BOOL or F32");
}
