...
using nntile::test::safetensors_nntile_layout::copy_safetensors_to_nntile;

TEST_CASE(
    "copy_safetensors_to_nntile copies 2-D C-order payload",
    "[model][layout]")
{
    const std::vector<std::int64_t> shape{2, 3};
    const std::vector<float> raw{0.f, 1.f, 2.f, 3.f, 4.f, 5.f};
    std::vector<float> out;
    copy_safetensors_to_nntile(
...
TEST_CASE(
    "copy_safetensors_to_nntile copies 3-D C-order payload",
    "[model][layout]")
{
