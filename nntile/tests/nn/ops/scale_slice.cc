 ...
 *
 * @file nntile/tests/nn_graph/scale_slice.cc
    ...
    "[graph][nn_graph]")
{
    ...
    auto *out = scale_slice(alpha, src, axis, axis_size)->set_name("out");
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "NNGraph scale_slice forward matches PyTorch",
    "[graph][nn_graph][pytorch]")
{
    ...
    auto *src = g.tensor(src_shape, DataType::FP32, true)->set_name("src");
    auto *out = scale_slice(alpha, src, axis, axis_size)->set_name("out");
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "NNGraph scale_slice backward matches PyTorch",
    "[graph][nn_graph][pytorch]")
{
    const auto [alpha, axis, grad_fill_val] =
        GENERATE(std::tuple{Scalar(2.0), Index(1), Scalar(1.0)},
