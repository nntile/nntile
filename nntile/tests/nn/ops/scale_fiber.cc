 ...
 *
 * @file nntile/tests/nn_graph/scale_fiber.cc
    ...
    "[graph][nn_graph]")
{
    ...
    auto *out = scale_fiber(alpha, src, dst_shape, axis, batch_ndim_none)
                    ->set_name("out");
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "NNGraph scale_fiber backward",
    "[graph][nn_graph]")
{
    const auto [alpha, axis, grad_fill_val] =
        GENERATE(std::tuple{Scalar(2.0), Index(1), Scalar(1.0)},
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "NNGraph scale_fiber forward matches PyTorch",
    "[graph][nn_graph][pytorch]")
{
    const auto [alpha, axis] = GENERATE(std::tuple{Scalar(1.0), Index(1)},
    ...
    auto *src = g.tensor(fiber_shape, DataType::FP32, true)->set_name("src");
    auto *out = scale_fiber(alpha, src, dst_shape, axis, batch_ndim_none)
                    ->set_name("out");
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "NNGraph scale_fiber backward matches PyTorch",
    "[graph][nn_graph][pytorch]")
{
    const auto [alpha, axis, grad_fill_val] =
        GENERATE(std::tuple{Scalar(2.0), Index(1), Scalar(1.0)},
