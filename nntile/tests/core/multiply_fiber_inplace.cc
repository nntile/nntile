 ...
 *
 * @file tests/core/multiply_fiber_inplace.cc
...
void check(Scalar alpha, Index axis)
{
    ...
    starpu::multiply_fiber_inplace.submit<std::tuple<T>>(-1, m, n, k, alpha, src,
            dst);
    multiply_fiber_inplace<T>(-1, alpha, src, dst2, axis);
