 ...
 *
 * @file tests/core/silu_backward.cc
 * Backward SiLU on Tile<T>
 *
 * @version 1.1.0
 * */
...
void validate()
{
    ...
    Tile<T> x({3, 2}), dy({3, 2}), dx({3, 2}), dxr({3, 2});
    ...
    starpu::silu_backward.submit<std::tuple<T>>(-1, x.nelems, 1.0f, x, dy, 1.0f, dx);
    silu_backward<T>(-1, 1.0f, x, dy, 1.0f, dxr);
