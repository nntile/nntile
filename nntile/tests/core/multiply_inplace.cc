 ...
 *
 * @file tests/core/multiply_inplace.cc
...
void validate()
{
    ...
    Tile<T> src1({}), dst1({}), dst1_copy({}), src2({4, 3, 2}),
        dst2({4, 3, 2}), dst2_copy({4, 3, 2});
    ...
    starpu::multiply_inplace.submit<std::tuple<T>>(-1, 1, 1.0, src1, dst1);
    multiply_inplace<T>(-1, 1.0, src1, dst1_copy);
    dst1_local.acquire(STARPU_R);
    dst1_copy_local.acquire(STARPU_R);
    TEST_ASSERT(Y(dst1_local[0]) == Y(dst1_copy_local[0]));
    dst1_local.release();
    dst1_copy_local.release();
    starpu::multiply_inplace.submit<std::tuple<T>>(-1, src2.nelems, 1.0, src2, dst2);
    multiply_inplace<T>(-1, 1.0, src2, dst2_copy);
