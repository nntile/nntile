 ...
 *
 * @file tests/core/relu_backward.cc
...
void validate()
{
    ...
    Tile<T> x({4, 3}), dy({3, 4}), dx({3, 4}), dxr({3, 4});
    ...
    starpu::relu_backward.submit<std::tuple<T>>(-1, x.nelems, x, dy, dx);
    relu_backward<T>(-1, x, dy, dxr);
