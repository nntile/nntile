 ...
 *
 * @file tests/core/norm_slice_inplace.cc
...
void check()
{
    ...
    // Check axis=0
    {
        starpu::norm_slice_inplace.submit<std::tuple<T>>(-1, 1, 20, 3, alpha, src, beta, dst[0]);
        norm_slice_inplace<T>(-1, alpha, src, beta, dst2[0], 0);
    ...
    // Check axis=1
    {
        starpu::norm_slice_inplace.submit<std::tuple<T>>(-1, 3, 5, 4, alpha, src, beta, dst[1]);
        norm_slice_inplace<T>(-1, alpha, src, beta, dst2[1], 1);
    ...
    // Check axis=2
    {
        starpu::norm_slice_inplace.submit<std::tuple<T>>(-1, 12, 1, 5, alpha, src, beta, dst[2]);
        norm_slice_inplace<T>(-1, alpha, src, beta, dst2[2], 2);
...
void validate()
{
    ...
    Tile<T> dst[3] = {Tile<T>({2, 4, 5}), Tile<T>({2, 3, 5}),
        Tile<T>({2, 3, 4})};
    Tile<T> empty({});
    TEST_THROW(norm_slice_inplace<T>(-1, 1.0, src, 1.0, empty, 0));
    TEST_THROW(norm_slice_inplace<T>(-1, 1.0, empty, 1.0, empty, 0));
    TEST_THROW(norm_slice_inplace<T>(-1, 1.0, src, 1.0, dst[0], -1));
    TEST_THROW(norm_slice_inplace<T>(-1, 1.0, src, 1.0, dst[0], 3));
    TEST_THROW(norm_slice_inplace<T>(-1, 1.0, src, 1.0, src, 0));
    TEST_THROW(norm_slice_inplace<T>(-1, 1.0, src, 1.0, dst[0], 1));
    TEST_THROW(norm_slice_inplace<T>(-1, 1.0, src, 1.0, dst[2], 1));
}
