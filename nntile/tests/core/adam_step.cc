...
void validate()
{
    ...
    Tile<T> grad({4, 3, 2}), m({2, 3, 4}), v({2, 3, 4}), p({2, 3, 4});
    Tile<T> m_ref({4, 3, 2}), v_ref({2, 3, 4}), p_ref({2, 3, 4});
    ...
    starpu::adam_step.submit<std::tuple<T>>(-1, num_iter, p.nelems, beta1, beta2,
            eps, lr, wd, grad, m, v, p);
    adam_step<T>(-1, num_iter, beta1, beta2, eps, lr, wd, grad, m_ref, v_ref,
            p_ref);
