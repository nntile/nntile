 ...
 *
 * @file tests/core/gelutanh_backward.cc
 * Backward approximate GeLU (tanh) on Tile<T>
 *
 * @version 1.1.0
 * */
...
void validate()
{
    ...
    Tile<T> x({4, 3, 2}), dy({4, 3, 2}), dx({4, 3, 2}), dxr({4, 3, 2});
    ...
    starpu::gelutanh_backward.submit<std::tuple<T>>(-1, x.nelems, 1.0f, x, dy, 1.0f, dx);
    gelutanh_backward<T>(-1, 1.0f, x, dy, 1.0f, dxr);
