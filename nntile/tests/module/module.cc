 ...
 *
 * @file nntile/tests/module/module.cc
...
class DummyModule final : public Module
{
  ...
  public:
    DummyModule(NNGraph *graph, const std::string &name) : Module(graph, name)
    {
    }
    ...
    NNGraph::TensorNode *forward(NNGraph::TensorNode *input)
    {
        ...
        output_tensor_ = graph()
                             ->tensor(output_shape,
                                 input->dtype(),
                                 graph()->requires_grad(input))
                             ->set_name(tensor_name("output"));
...
TEST_CASE("Module RegisterParameterAndBuffer", "[module]")
{
    ...
    auto *param = g.tensor({2, 3}, DataType::FP32, true)->set_name("param");
    auto *buffer = g.tensor({4}, DataType::FP32, true)->set_name("buffer");
...
TEST_CASE("Module RegisterNullPointers", "[module]")
{
    ...
    REQUIRE_THROWS_AS(
        module.register_parameter("weight", nullptr), std::invalid_argument);
    REQUIRE_THROWS_AS(
        module.register_buffer("running", nullptr), std::invalid_argument);
    REQUIRE_THROWS_AS(
        module.register_module("child", nullptr), std::invalid_argument);
}
...
TEST_CASE("Module RecursiveParametersAndModules", "[module]")
{
    ...
    REQUIRE(
        std::find(params.begin(), params.end(), parent_param) != params.end());
    REQUIRE(
        std::find(params.begin(), params.end(), child_param) != params.end());

    auto named_params = parent.named_parameters_recursive();
    REQUIRE(named_params.size() == 2);
    REQUIRE(std::any_of(named_params.begin(),
        named_params.end(),
        [&](const auto &entry)
        {
