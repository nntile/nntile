 ...
 *
 * @file nntile/tests/module/activation.cc
    ...
    "[module][pytorch]")
{
    const auto [shape, act_type, grad_fill_val] = GENERATE(
        ...
        std::tuple{
            std::vector<Index>{1, 8}, ActivationType::RELU, Scalar(-1.0)},
        std::tuple{std::vector<Index>{6}, ActivationType::GELU, Scalar(1.0)},
        std::tuple{
            std::vector<Index>{2, 4}, ActivationType::SILU, Scalar(-1.0)});
    ...
    auto *input = g.tensor(shape, DataType::FP32, true)->set_name("input");
