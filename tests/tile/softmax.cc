...
void check()
{
    ...
    // Check axis=0
    {
        starpu::softmax::submit<T>(1, 20, 3, maxsumexp[0], dst);
        softmax<T>(maxsumexp[0], dst2, 0);
    ...
    // Check axis=1
    {
        starpu::softmax::submit<T>(3, 5, 4, maxsumexp[1], dst);
        softmax<T>(maxsumexp[1], dst2, 1);
        dst_local.acquire(STARPU_R);
        dst2_local.acquire(STARPU_R);
        for(Index i = 0; i < dst.nelems; ++i)
        {
    ...
    // Check axis=2
    {
        starpu::softmax::submit<T>(12, 1, 5, maxsumexp[2], dst);
        softmax<T>(maxsumexp[2], dst2, 2);
        dst_local.acquire(STARPU_R);
        dst2_local.acquire(STARPU_R);
        for(Index i = 0; i < dst.nelems; ++i)
        {
...
void validate()
{
    ...
    Tile<T> maxsumexp[3] = {Tile<T>({2, 4, 5}), Tile<T>({2, 3, 5}),
        Tile<T>({2, 3, 4})};
    TEST_THROW(softmax<T>(empty, empty, 0));
    TEST_THROW(softmax<T>(maxsumexp[0], dst, 1));
    TEST_THROW(softmax<T>(maxsumexp[0], dst, 2));
    TEST_THROW(softmax<T>(dst, dst, 0));
    TEST_THROW(softmax<T>(maxsumexp[2], dst, 0));
    TEST_THROW(softmax<T>(maxsumexp[1], dst, 2));
    TEST_THROW(softmax<T>(maxsumexp[0], dst, -1));
    TEST_THROW(softmax<T>(maxsumexp[0], dst, 3));
}
...
int main(int argc, char **argv)
{
    ...
    // Init codelet
    starpu::softmax::init();
    starpu::softmax::restrict_where(STARPU_CPU);
    // Launch all tests
    validate<fp32_t>();
    validate<fp64_t>();
    return 0;
