 ...
 *
 * @file tests/tile/sqrt_inplace.cc
 * sqrt_inplace operation on Tile<T>
 *
 * @version 1.1.0
 * */
...
void validate()
{
    ...
    starpu::sqrt_inplace::submit<T>(1, tile1);
    sqrt_inplace<T>(tile1_copy);
    tile1_local.acquire(STARPU_R);
    tile1_copy_local.acquire(STARPU_R);
    TEST_ASSERT(Y(tile1_local[0]) == Y(tile1_copy_local[0]));
    tile1_local.release();
    tile1_copy_local.release();
    starpu::sqrt_inplace::submit<T>(tile2.nelems, tile2);
    sqrt_inplace<T>(tile2_copy);
...
int main(int argc, char **argv)
{
    ...
    // Init codelet
    starpu::sqrt_inplace::init();
    starpu::sqrt_inplace::restrict_where(STARPU_CPU);
    // Launch all tests
    validate<fp32_t>();
    validate<fp64_t>();
    return 0;
