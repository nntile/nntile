 ...
 *
 * @file tests/tile/dgelu.cc
 * Derivative of GeLU operation on Tile<T>
 *
 * @version 1.1.0
 * */
...
void validate()
{
    ...
    tile1_local.release();
    tile1_copy_local.release();
    starpu::dgelu::submit<T>(tile2.nelems, tile2);
    dgelu<T>(tile2_copy);
...
int main(int argc, char **argv)
{
    ...
    // Init codelet
    starpu::dgelu::init();
    starpu::dgelu::restrict_where(STARPU_CPU);
    // Launch all tests
    validate<fp32_t>();
    validate<fp64_t>();
    return 0;
