...
using namespace nntile::core::kernel::embedding_backward;
...
struct TestData
{
    ...
    std::vector<nntile::core::int64_t> index_init;
    ...
    switch(strategy)
    {
        case DataGen::PRESET:
            for(Index i = 0; i < data.m * data.n; ++i)
            {
                ...
                nntile::core::int64_t::repr_t idx = (i * 7 + 3) % data.vocab_size;
        ...
        case DataGen::RANDOM:
            std::mt19937 gen(42);
            std::uniform_int_distribution<nntile::core::int64_t::repr_t>
...
void verify_results(
    ...
    const std::vector<nntile::core::int64_t>& index,
...
void run_cpu_test(TestData<T>& data)
{
    ...
    std::vector<nntile::core::int64_t> index_cpu(data.index_init);
...
void run_cuda_test(TestData<T>& data)
{
    ...
    CUDA_CHECK(
        cudaMalloc(
            &dev_index,
            sizeof(nntile::core::int64_t) * data.index_init.size()
        ),
    ...
    std::vector<nntile::core::int64_t> index_cuda(data.index_init);
