...
using namespace nntile::core::kernel::hypot_inplace;
...
void reference_hypot_inplace(TestData<T>& data)
{
    ...
    for(Index i = 0; i < data.num_elems; ++i)
    {
        ...
        else
        {
            if(beta == zero)
            {
                result = std::fabs(alpha * src_val);
            }
            else
            {
                result = std::hypot(alpha * src_val, beta * dst_val);
            }
        }
