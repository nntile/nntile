...
using namespace nntile::core::kernel::hypot;
...
void reference_hypot(TestData<T>& data)
{
    ...
    for(Index i = 0; i < data.num_elems; ++i)
    {
        ...
        else
        {
            if(beta == zero)
            {
                result = std::fabs(alpha * src1_val);
            }
            else
            {
                result = std::hypot(alpha * src1_val, beta * src2_val);
            }
        }
...
void generate_data(TestData<T>& data, Index num_elems, DataGen strategy)
{
    ...
    switch(strategy)
    {
        ...
        case DataGen::PRESET:
            ...
            {
                Y src1_val = 2 * i + 1 - num_elems;
                Y src2_val = num_elems - i;
                data.src1_init[i] = src1_val;
                data.src2_init[i] = src2_val;
                Y dst_val = -1;
...
void run_cuda_test(TestData<T>& data)
{
    ...
    CUDA_CHECK(
        cudaMalloc(&dev_src1, sizeof(T) * data.num_elems),
        "cudaMalloc dev_src1"
    );
    CUDA_CHECK(
        cudaMalloc(&dev_src2, sizeof(T) * data.num_elems),
        "cudaMalloc dev_src2"
    );
    CUDA_CHECK(
...
)
{
    ...
    const Scalar alpha = GENERATE(1.0, -1.5, 2.0);
    const Scalar beta = GENERATE(1.0, -1.5, 2.0);
