 ...
 * Per-element scaling of a broadcasted slice
 *
 * @version 1.1.0
 * */
...
using namespace nntile::core::kernel::scale_slice;
...
// Reference implementation of the scale slice operation
template<typename T>
void reference_scale_slice(TestData<T>& data)
{
    using Y = typename T::repr_t;
    ...
    for(Index i0 = 0; i0 < data.m; ++i0)
    {
        for(Index i1 = 0; i1 < data.n; ++i1)
        {
            ...
            for(Index i2 = 0; i2 < data.k; ++i2)
            {
                ...
                ref_t dst_val = data.alpha * src_val;
...
// Catch2-based tests
TEMPLATE_TEST_CASE(
    "Scale Slice Kernel Verification",
    "[scale_slice]",
    fp64_t,
    fp32_t,
    fp16_t,
...
)
{
    ...
    auto data = get_test_input_data<T>(m, n, k, alpha, strategy);

    // Compute reference outputs for verification
    reference_scale_slice(data);
...
TEMPLATE_TEST_CASE(
    "Scale Slice Kernel Benchmark",
    "[scale_slice][!benchmark]",
    fp64_t,
    fp32_t,
    fp16_t,
    bf16_t
