 ...
 *
 * @file tests/kernel/relu_backward.cc
 * Backward ReLU operation
 *
 * @version 1.1.0
 * */
...
using namespace nntile::core::kernel::relu_backward;
...
// Reference implementation of the backward ReLU operation
template<typename T>
void reference_relu_backward(TestData<T>& data)
{
    using Y = typename T::repr_t;
    if (data.nelems == 0)
    {
...
void run_cpu_test(TestData<T>& data)
{
    ...
    {
        BENCHMARK(
            "[kernel][relu_backward][cpu][nelems=" +
...
void run_cuda_test(TestData<T>& data)
{
    ...
    {
        BENCHMARK(
            "[kernel][relu_backward][cuda][nelems=" +
            std::to_string(data.nelems) +
            "]"
        )
        {
...
// Catch2-based tests
TEMPLATE_TEST_CASE(
    "ReLU Backward Kernel Verification",
    "[relu_backward]",
    fp64_t,
    fp32_t,
    fp16_t,
...
TEMPLATE_TEST_CASE(
    "ReLU Backward Kernel Benchmark",
    "[relu_backward][!benchmark]",
    fp64_t,
    fp32_t,
    fp16_t,
    bf16_t
