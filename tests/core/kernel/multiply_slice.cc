...
using namespace nntile::core::kernel::multiply_slice;
...
void reference_multiply_slice(TestData<T>& data)
{
    ...
    for(Index i2 = 0; i2 < data.n; ++i2)
    {
        ...
        {
            ref_t src_val = static_cast<Y>(data.src_init[i2 * data.m + i1]);
            src_val *= alpha_r;
    ...
    switch(strategy)
    {
        ...
        case DataGen::PRESET:
            for(Index i2 = 0; i2 < data.n; ++i2)
            {
                for(Index i1 = 0; i1 < data.m; ++i1)
                {
                    Y src_val = static_cast<Y>(2.0 * (i1 + i2) + 1.0);
                    data.src_init[i2 * data.m + i1] = src_val;
                }
            }
            for(Index i2 = 0; i2 < data.n; ++i2)
            {
                for(Index i0 = 0; i0 < data.k; ++i0)
                {
                    ...
                    {
                        Index dst_idx = (i2 * data.k + i0) * data.m + i1;
                        Y dst_val = static_cast<Y>(1.0 + i0 + i1 * data.k +
                            i2 * data.k * data.m);
