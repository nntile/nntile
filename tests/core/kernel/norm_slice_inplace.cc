...
using namespace nntile::core::kernel::norm_slice_inplace;
...
void reference_norm_slice_inplace(TestData<T>& data)
{
    ...
    for(Index i2 = 0; i2 < data.n; ++i2)
    {
        for(Index i1 = 0; i1 < data.m; ++i1)
        {
            ...
            ref_t dst_val = static_cast<Y>(data.dst_init[i2 * data.m + i1]);
            ref_t result = std::hypot(beta_r * dst_val, norm_val);
            ...
            for(Index i0 = 0; i0 < data.k; ++i0)
            {
                for(Index i1 = 0; i1 < data.n; ++i1)
                {
                    for(Index i2 = 0; i2 < data.m; ++i2)
                    {
                        ...
                        Y src_val = 1.0 + i0 + i1 * data.k +
                            i2 * data.k * data.n;
                        data.src_init[src_idx] = src_val;
                    }
                }
            }
