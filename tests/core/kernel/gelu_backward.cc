...
using namespace nntile::core::kernel::gelu_backward;
...
void reference_gelu_backward(TestData<T>& data)
{
    ...
    constexpr ref_t one = 1.0, mone = -1.0, pt5 = 0.5;
    const ref_t f1 = mone / std::sqrt(2.0), f2 = one / std::sqrt(2*pi);

    for(Index i = 0; i < data.nelems; ++i)
    {
        ref_t x = static_cast<Y>(data.x[i]);
        ref_t dy = static_cast<Y>(data.dy[i]);
        ref_t exp_x = std::exp(-pt5 * x * x);
        ref_t y = std::erfc(f1 * x);
        ref_t z = static_cast<Y>(data.dx_init[i]);
        z += (x*f2*exp_x + pt5*y) * dy;
        data.dx_ref[i] = static_cast<T>(static_cast<Y>(z));
    }
}
