    ...
    "NNGraph multiply_slice structure", "[graph][nn_graph]")
{
    ...
    auto* out = multiply_slice(alpha, src, "out", axis, axis_size);

    REQUIRE(out != nullptr);
    REQUIRE(out->has_producer());
    REQUIRE(out->shape() == (std::vector<Index>{dim_2, dim_4}));
    REQUIRE(g.num_ops() == 1);
    REQUIRE(g.tensor_graph().ops()[0]->op_name() == "MULTIPLY_SLICE");
}
    ...
    "NNGraph multiply_slice forward matches PyTorch", "[graph][nn_graph][pytorch]")
{
    ...
    auto out_pt = (alpha * src_pt.unsqueeze(static_cast<int64_t>(axis))
    ...
    "NNGraph multiply_slice backward matches PyTorch", "[graph][nn_graph][pytorch]")
{
    ...
    auto out_pt = alpha * src_pt.unsqueeze(static_cast<int64_t>(axis)).expand({dim_2, dim_4});
