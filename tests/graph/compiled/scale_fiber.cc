 ...
 *
 * @file tests/graph/compiled/scale_fiber.cc
 * Test for compiled graph scale_fiber operation.
 *
 * @version 1.1.0
 * */
...
TEST_CASE_METHOD(
    GraphTestFixture,
    "CompiledGraph ScaleFiber vs Tensor",
    "[graph][verification]")
{
    ...
    auto run_tensor_direct = [](std::map<std::string, std::vector<float>>& inputs,
        ...
        nntile::tensor::TensorTraits y_traits({4, 3}, {4, 3});
        nntile::tensor::Tensor<T> y(y_traits);

        write_tensor(x, inputs["x"]);
        write_tensor(y, inputs["y"]);
        nntile::tensor::scale_fiber<T>(1.5f, x, y, 0, 0);
