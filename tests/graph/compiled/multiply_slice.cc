 ...
 *
 * @file tests/graph/compiled/multiply_slice.cc
 * Test for compiled graph multiply_slice operation.
 *
 * @version 1.1.0
 * */
...
TEST_CASE_METHOD(
    GraphTestFixture,
    "CompiledGraph MultiplySlice vs Tensor",
    "[graph][verification]")
{
    auto build_graph = [](LogicalGraph& g) {
        auto& x = g.tensor({1, 6}, "x", DataType::FP32);
        auto& y = g.tensor({4, 1, 6}, "y", DataType::FP32);
        multiply_slice(2.0f, x, y, 0);
    };
    ...
    auto run_tensor_direct = [](std::map<std::string, std::vector<float>>& inputs,
        ...
        nntile::tensor::TensorTraits x_traits({1, 6}, {1, 6});
        nntile::tensor::Tensor<T> x(x_traits);
        nntile::tensor::TensorTraits y_traits({4, 1, 6}, {4, 1, 6});
        nntile::tensor::Tensor<T> y(y_traits);

        write_tensor(x, inputs["x"]);
        write_tensor(y, inputs["y"]);
        nntile::tensor::multiply_slice<T>(2.0f, x, y, 0);
