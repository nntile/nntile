 ...
 *
 * @file tests/graph/compiled/multiply_fiber_inplace.cc
 * Test for compiled graph multiply_fiber_inplace operation.
 *
 * @version 1.1.0
 * */
...
TEST_CASE_METHOD(
    GraphTestFixture,
    "CompiledGraph MultiplyFiberInplace vs Tensor",
    "[graph][verification]")
{
    auto build_graph = [](LogicalGraph& g) {
        auto& x = g.tensor({4, 1}, "x", DataType::FP32);
        auto& y = g.tensor({4, 6}, "y", DataType::FP32);
        multiply_fiber_inplace(2.0f, x, y, 0, 0);
    };
    ...
    auto run_tensor_direct = [](std::map<std::string, std::vector<float>>& inputs,
        ...
        nntile::tensor::TensorTraits x_traits({4, 1}, {4, 1});
        ...
        nntile::tensor::multiply_fiber_inplace<T>(2.0f, x, y, 0);
