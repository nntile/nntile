 ...
 *
 * @file tests/graph/compiled/add_slice.cc
 * Test for compiled graph add_slice operation.
 *
 * @version 1.1.0
 * */
...
TEST_CASE_METHOD(
    GraphTestFixture,
    "CompiledGraph AddSlice vs Tensor",
    "[graph][verification]")
{
    auto build_graph = [](LogicalGraph& g) {
        auto& x = g.tensor({4, 6}, "x", DataType::FP32);
        auto& y = g.tensor({1, 6}, "y", DataType::FP32);
        add_slice(x, y, "z", 2.0f, 3.0f, 0);
    };
    ...
    auto run_tensor_direct = [](std::map<std::string, std::vector<float>>& inputs,
        ...
        nntile::tensor::TensorTraits y_traits({1, 6}, {1, 6});
        ...
        nntile::tensor::add_slice<T>(2.0f, x, 3.0f, y, z, 0);
