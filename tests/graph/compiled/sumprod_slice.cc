 ...
 *
 * @file tests/graph/compiled/sumprod_slice.cc
 * Test for compiled graph sumprod_slice operation.
 *
 * @version 1.1.0
 * */
...
TEST_CASE_METHOD(
    GraphTestFixture,
    "CompiledGraph SumProdSlice vs Tensor",
    "[graph][verification]")
{
    ...
    auto run_tensor_direct = [](std::map<std::string, std::vector<float>>& inputs,
        ...
        nntile::tensor::Tensor<T> x1(x_traits);
        nntile::tensor::Tensor<T> x2(x_traits);
        nntile::tensor::TensorTraits y_traits({4}, {4});
        nntile::tensor::Tensor<T> y(y_traits);

        write_tensor(x1, inputs["x1"]);
        write_tensor(x2, inputs["x2"]);
        nntile::tensor::sumprod_slice<T>(1.0f, x1, x2, 0.0f, y, 1, 0);
