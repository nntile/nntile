 ...
 *
 * @file tests/graph/compiled/adam_step.cc
 * Test for compiled graph adam_step operation.
 *
 * @version 1.1.0
 * */
...
TEST_CASE_METHOD(
    GraphTestFixture,
    "CompiledGraph AdamStep vs Tensor",
    "[graph][verification]")
{
    ...
    auto run_tensor_direct = [](std::map<std::string, std::vector<float>>& inputs,
        ...
        nntile::tensor::adam_step<T>(10, 0.9f, 0.999f, 1e-8f, 0.001f, 0.0f, grad, first_moment, second_moment, p);
