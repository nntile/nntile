 ...
 *
 * @file tests/graph/model/t5/t5_decoder_block.cc
...
inline std::string decoder_block_fixture_safetensors_path(
    const std::string &data_dir, const DecoderBlockFixtureSpec &spec)
{
    ...
    const char *stem, DecoderBlockFixtureSpec &fx)
{
    ...
    std::ifstream st(decoder_block_fixture_safetensors_path(dir, fx));
    return st.good();
}
...
void decoder_block_forward_compare_ref(const DecoderBlockFixtureSpec &fx)
{
    const std::string full_path =
        decoder_block_fixture_safetensors_path(std::string(T5_DATA_DIR), fx);
