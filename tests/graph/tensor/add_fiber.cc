    ...
    Scalar beta)
{
    ...
    auto* out_node = add_fiber(alpha, fiber_node, beta, tensor_node, "out",
                               axis, batch_ndim);
    ...
    tensor::add_fiber<T>(alpha, fiber_t, beta, tensor_t, out_t, axis, batch_ndim);
...
TEST_CASE("TensorGraph add_fiber structure", "[graph][tensor]")
{
    ...
    auto* out = add_fiber(alpha_one, fiber, beta_one, tensor, "out",
