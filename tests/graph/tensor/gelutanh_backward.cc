 ...
 *
 * @file tests/graph/tensor/gelutanh_backward.cc
 * Test TensorGraph gelutanh_backward operation against nntile::tensor::gelutanh_backward.
 *
 * @version 1.1.0
 * */
...
template<typename T>
void check_gelutanh_backward_vs_tensor_api(
    const std::vector<Index>& shape)
{
    ...
    // --- TensorGraph path ---
    TensorGraph graph("gelutanh_backward_test");
    ...
    gelutanh_backward(x_node, dy_node, dx_node);
    ...
    tensor::gelutanh_backward<T>(x_t, dy_t, dx_t);
...
TEST_CASE("TensorGraph gelutanh_backward structure", "[graph][tensor]")
{
    ...
    auto* dx = gelutanh_backward(x, dy, "dx");
...
TEST_CASE("TensorGraph gelutanh_backward rejects duplicate tensors", "[graph][tensor]")
{
    TensorGraph graph("test");
    auto* x = graph.data({4, 5}, "x");
    auto* dy = graph.data({4, 5}, "dy");

    REQUIRE_THROWS_AS(gelutanh_backward(x, x, "dx"), std::invalid_argument);
    REQUIRE_THROWS_AS(gelutanh_backward(x, dy, x), std::invalid_argument);
    REQUIRE_THROWS_AS(gelutanh_backward(x, dy, dy), std::invalid_argument);
}

TEST_CASE_METHOD(nntile::test::ContextFixture,
    "TensorGraph gelutanh_backward matches tensor::gelutanh_backward", "[graph][tensor]")
{
    ...
    check_gelutanh_backward_vs_tensor_api<nntile::fp32_t>(shape);
}
