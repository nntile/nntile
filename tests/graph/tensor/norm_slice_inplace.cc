    ...
    Scalar beta)
{
    ...
    norm_slice_inplace(alpha, src_node, beta, dst_node, axis, redux);
    ...
    tensor::TensorTraits dst_traits(dst_sh, dst_sh);
    ...
    tensor::norm_slice_inplace<T>(alpha, src_t, beta, dst_t, axis, redux);
...
TEST_CASE("TensorGraph norm_slice_inplace structure", "[graph][tensor]")
{
    ...
    norm_slice_inplace(alpha_one, src, beta_one, dst, axis_0, redux_none);
...
TEST_CASE("TensorGraph norm_slice_inplace rejects duplicate tensors", "[graph][tensor]")
{
    ...
    REQUIRE_THROWS_AS(
        norm_slice_inplace(alpha_one, src, beta_one, src, axis_0, redux_none),
        std::invalid_argument);
}

TEST_CASE_METHOD(nntile::test::ContextFixture,
    "TensorGraph norm_slice_inplace matches tensor::norm_slice_inplace", "[graph][tensor]")
{
