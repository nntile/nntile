 ...
 * Test TensorGraph sumprod_slice operation against nntile::tensor::sumprod_slice.
 *
 * @version 1.1.0
 * */
...
template<typename T>
void check_sumprod_slice_vs_tensor_api(
    const std::vector<Index>& src_shape,
    Index axis,
    int redux,
    Scalar alpha,
...
TEST_CASE("TensorGraph sumprod_slice rejects duplicate tensors", "[graph][tensor]")
{
    TensorGraph graph("test");
    auto* src1 = graph.data({dim_2, dim_4}, "src1");
    auto* src2 = graph.data({dim_2, dim_4}, "src2");
