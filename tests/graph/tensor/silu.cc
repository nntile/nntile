 ...
 *
 * @file tests/graph/tensor/silu.cc
 * Test TensorGraph silu operation against nntile::tensor::silu.
 *
 * @version 1.1.0
 * */
    ...
    const std::vector<Index>& shape)
{
    ...
    // --- TensorGraph path ---
    TensorGraph graph("silu_test");
    auto* src_node = graph.data(shape, "src", DataType::FP32);
    src_node->mark_input(true);

    auto* dst_node = gt::silu(src_node, "dst");
...
TEST_CASE("TensorGraph silu structure", "[graph][tensor]")
{
...
TEST_CASE_METHOD(nntile::test::ContextFixture,
    "TensorGraph silu matches nntile::tensor::silu", "[graph][tensor]")
{
    ...
    check_silu_vs_tensor_api<nntile::fp32_t>(shape);
}

TEST_CASE_METHOD(nntile::test::ContextFixture,
    "TensorGraph silu tiled matches untiled", "[graph][tensor]")
{
    const auto shape = GENERATE(
    ...
    {
        TensorGraph graph("silu_untiled");
        auto* src_node = graph.data(shape, "src", DataType::FP32);
        src_node->mark_input(true);
        auto* dst_node = gt::silu(src_node, "dst");
        dst_node->mark_output(true);
