 ...
 * Test TensorGraph copy_intersection operation against
 * nntile::core::tensor::copy_intersection.
 *
 * @version 1.1.0
 * */
...
template <typename T>
void check_copy_intersection_vs_tensor_api(const std::vector<Index> &src_shape,
    ...
    const std::vector<Index> *basetile_dst_ptr = nullptr)
{
    ...
    const std::vector<Index> basetile_src =
        basetile_src_ptr
            ? *basetile_src_ptr
            : std::vector<Index>(src_shape.begin(), src_shape.end());
    const std::vector<Index> basetile_dst =
        basetile_dst_ptr
            ? *basetile_dst_ptr
    ...
    // --- Reference: single-tile nntile::core::tensor with same values as src_data
    nntile::core::tensor::TensorTraits ref_src_traits(src_shape, src_shape);
    nntile::core::tensor::TensorTraits ref_dst_traits(dst_shape, dst_shape);
    std::vector<int> distr1(1, distr_rank_single);
    nntile::core::tensor::Tensor<T> ref_src(ref_src_traits, distr1);
    nntile::core::tensor::Tensor<T> ref_dst(ref_dst_traits, distr1);
    {
        auto tile = ref_src.get_tile(0);
        auto loc = tile.acquire(STARPU_W);
        for (Index i = 0; i < src_nelems; ++i)
        {
            loc[static_cast<size_t>(i)] =
                static_cast<Y>(src_data[static_cast<size_t>(i)]);
        }
        loc.release();
    }
    {
    ...
    nntile::core::tensor::copy_intersection<T>(
        ref_src, src_offset, ref_dst, dst_offset);
    ...
    std::vector<float> tensor_result(static_cast<size_t>(dst_nelems));
    {
        ...
        for (Index i = 0; i < dst_nelems; ++i)
        {
            tensor_result[static_cast<size_t>(i)] =
                static_cast<float>(loc[static_cast<size_t>(i)]);
        }
        loc.release();
    }
...
TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph copy_intersection matches nntile::core::tensor::copy_intersection",
    "[graph][tensor]")
{
    ...
    check_copy_intersection_vs_tensor_api<nntile::core::fp32_t>(
    ...
    "[graph][tensor]")
{
    ...
    check_copy_intersection_vs_tensor_api<nntile::core::fp32_t>(
        src_shape, dst_shape, src_offset, dst_offset, nullptr, nullptr);
}
