 ...
 * Test TensorGraph gelu operation against nntile::core::tensor::gelu.
 *
 * @version 1.1.0
 * */
...
template <typename T>
void check_gelu_vs_tensor_api(const std::vector<Index> &shape)
{
    ...
    auto *dst_node = gt::gelu(src_node);
    ...
    nntile::core::tensor::gelu<T>(src, dst);
...
TEST_CASE("TensorGraph gelu rejects duplicate tensors", "[graph][tensor]")
{
    TensorGraph graph("test");
    auto *src = graph.data({4, 5})->set_name("src");

    REQUIRE_THROWS_AS(gt::gelu(src, src), std::invalid_argument);
}

TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph gelu matches nntile::core::tensor::gelu",
    "[graph][tensor]")
{
    ...
    check_gelu_vs_tensor_api<nntile::core::fp32_t>(shape);
}

TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph gelu tiled matches untiled",
    "[graph][tensor]")
{
    ...
    {
        TensorGraph graph("gelu_untiled");
        auto *src_node = graph.data(shape, DataType::FP32)->set_name("src");
        src_node->mark_input(true);
        auto *dst_node = gt::gelu(src_node);
