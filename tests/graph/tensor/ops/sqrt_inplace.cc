 ...
 * Test TensorGraph sqrt_inplace operation against
 * nntile::core::tensor::sqrt_inplace.
 *
 * @version 1.1.0
 * */
...
template <typename T>
void check_sqrt_inplace_vs_tensor_api(const std::vector<Index> &shape)
{
    ...
    nntile::core::tensor::sqrt_inplace<T>(dst);
...
TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph sqrt_inplace matches nntile::core::tensor::sqrt_inplace",
    "[graph][tensor]")
{
    ...
    check_sqrt_inplace_vs_tensor_api<nntile::core::fp32_t>(shape);
}

TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph sqrt_inplace tiled matches untiled",
    "[graph][tensor]")
{
    ...
    {
        TensorGraph graph("sqrt_inplace_untiled");
        auto *dst_node = graph.data(shape, DataType::FP32)->set_name("dst");
        dst_node->mark_input(true);
        dst_node->mark_output(true);
        gt::sqrt_inplace(dst_node);
    ...
    {
        TensorGraph graph("sqrt_inplace_tiled");
        auto *dst_node = graph.data(shape, DataType::FP32)->set_name("dst");
        dst_node->mark_input(true);
        dst_node->mark_output(true);
        gt::sqrt_inplace(dst_node);
