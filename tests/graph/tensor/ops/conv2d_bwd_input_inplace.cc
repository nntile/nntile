 ...
 * nntile::core::tensor::conv2d_bwd_input_inplace.
 *
 * @version 1.1.0
 * */
    ...
    const std::array<Index, 2> &dilation)
{
    ...
    auto dy_shape =
        conv2d_output_shape(dx_shape, kernel_shape, padding, stride, dilation);

    const Index dy_nelems = std::accumulate(
        dy_shape.begin(), dy_shape.end(), Index(1), std::multiplies<>());
    const Index kernel_nelems = std::accumulate(kernel_shape.begin(),
        kernel_shape.end(),
        Index(1),
        std::multiplies<>());
    ...
    auto *kernel_node =
        graph.data(kernel_shape, DataType::FP32)->set_name("kernel");
    auto *dx_node = graph.data(dx_shape, DataType::FP32)->set_name("dx");
    ...
    gt::conv2d_bwd_input_inplace(
        alpha, dy_node, kernel_node, beta, dx_node, padding, stride, dilation);
    ...
    nntile::core::tensor::TensorTraits kernel_traits(kernel_shape, kernel_shape);
    nntile::core::tensor::TensorTraits dx_traits(dx_shape, dx_shape);
    std::vector<int> distr(1, distr_rank_single);

    nntile::core::tensor::Tensor<T> dy_t(dy_traits, distr);
    nntile::core::tensor::Tensor<T> kernel_t(kernel_traits, distr);
    nntile::core::tensor::Tensor<T> dx_t(dx_traits, distr);
    ...
    nntile::core::tensor::conv2d_bwd_input_inplace<T>(
...
TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph conv2d_bwd_input_inplace matches "
    "nntile::core::tensor::conv2d_bwd_input_inplace",
    "[graph][tensor]")
{
    const auto [dx_shape,
        ...
        padding,
        stride,
        dilation] = GENERATE(std::tuple{std::vector<Index>{4, 4, 2, 2},
                                 std::vector<Index>{2, 2, 2, 2},
                                 1.0,
                                 0.0,
                                 std::array<Index, 2>{0, 0},
                                 std::array<Index, 2>{1, 1},
                                 std::array<Index, 2>{1, 1}},
        std::tuple{std::vector<Index>{5, 5, 2, 2},
            std::vector<Index>{3, 3, 2, 2},
            1.0,
            1.0,
            std::array<Index, 2>{1, 1},
            std::array<Index, 2>{1, 1},
    ...
    check_conv2d_bwd_input_inplace_vs_tensor_api<nntile::core::fp32_t>(
