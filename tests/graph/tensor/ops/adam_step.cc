 ...
 * Test TensorGraph adam_step operation against nntile::core::tensor::adam_step.
 *
 * @version 1.1.0
 * */
...
template <typename T>
void check_adam_step_vs_tensor_api(const std::vector<Index> &shape,
    Index num_iter,
    Scalar beta_1,
    Scalar beta_2,
    Scalar eps,
    ...
    Scalar weight_decay)
{
    ...
    std::vector<float> graph_first =
        runtime.get_output<float>(first_moment_node);
    std::vector<float> graph_second =
        runtime.get_output<float>(second_moment_node);
    ...
    nntile::core::tensor::adam_step<T>(num_iter,
        beta_1,
        beta_2,
        eps,
        lr,
...
TEST_CASE("TensorGraph adam_step structure", "[graph][tensor]")
{
    ...
    gt::adam_step(
        1, 0.9, 0.999, 1e-8, 0.001, 0.0, grad, first_moment, second_moment, p);
...
TEST_CASE("TensorGraph adam_step rejects null tensors", "[graph][tensor]")
{
    ...
    REQUIRE_THROWS_AS(gt::adam_step(1,
                          0.9,
                          0.999,
                          1e-8,
                          0.001,
    ...
    REQUIRE_THROWS_AS(
        gt::adam_step(
            1, 0.9, 0.999, 1e-8, 0.001, 0.0, grad, nullptr, second_moment, p),
        std::invalid_argument);
}

TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph adam_step matches nntile::core::tensor::adam_step",
    "[graph][tensor]")
{
    const auto [shape,
        num_iter,
        ...
        eps,
        lr,
        weight_decay] = GENERATE(std::tuple{std::vector<Index>{dim_4, dim_5},
                                     Index(1),
                                     0.9,
                                     0.999,
                                     1e-8,
                                     ...
                                     0.0},
        std::tuple{
            std::vector<Index>{6}, Index(2), 0.9, 0.999, 1e-8, 0.01, 0.01},
        std::tuple{
            std::vector<Index>{2, 3}, Index(1), 0.95, 0.99, 1e-6, 0.001, 0.0});

    check_adam_step_vs_tensor_api<nntile::core::fp32_t>(
        shape, num_iter, beta_1, beta_2, eps, lr, weight_decay);
}

TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph adam_step tiled matches untiled",
    "[graph][tensor]")
{
