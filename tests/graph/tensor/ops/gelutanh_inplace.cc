 ...
 *
 * @file tests/graph/tensor/gelutanh_inplace.cc
 * Test TensorGraph gelutanh_inplace operation against
 * nntile::core::tensor::gelutanh_inplace.
 *
 * @version 1.1.0
 * */
...
template <typename T>
void check_gelutanh_inplace_vs_tensor_api(const std::vector<Index> &shape)
{
    ...
    // --- TensorGraph path ---
    TensorGraph graph("gelutanh_inplace_test");
    ...
    nntile::core::tensor::gelutanh_inplace<T>(dst);
...
TEST_CASE("TensorGraph gelutanh_inplace structure", "[graph][tensor]")
{
    ...
    gt::gelutanh_inplace(dst);

    REQUIRE(graph.num_data() == 1);
    REQUIRE(graph.num_ops() == 1);

    const auto &ops = graph.ops();
    REQUIRE(ops[0]->op_name() == "GELUTANH_INPLACE");
...
TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph gelutanh_inplace matches nntile::core::tensor::gelutanh_inplace",
    "[graph][tensor]")
{
    ...
    check_gelutanh_inplace_vs_tensor_api<nntile::core::fp32_t>(shape);
}

TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph gelutanh_inplace tiled matches untiled",
    "[graph][tensor]")
{
