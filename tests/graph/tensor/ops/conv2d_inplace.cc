...
template <typename T>
void check_conv2d_inplace_vs_tensor_api(const std::vector<Index> &x_shape,
    const std::vector<Index> &c_shape,
    Scalar alpha,
    Scalar beta,
    const std::array<Index, 2> &padding,
    ...
    const std::array<Index, 2> &dilation)
{
    ...
    auto y_shape =
        conv2d_output_shape(x_shape, c_shape, padding, stride, dilation);
    ...
    auto *c_node = graph.data(c_shape, DataType::FP32)->set_name("c");
    ...
    gt::conv2d_inplace(
        alpha, x_node, c_node, beta, y_node, padding, stride, dilation);
    ...
    runtime.bind_data(c_node, c_data);
