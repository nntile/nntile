 ...
 * Test TensorGraph relu operation against nntile::core::tensor::relu.
 *
 * @version 1.1.0
 * */
...
void check_relu_vs_tensor_api(const std::vector<Index> &shape)
{
    ...
    nntile::core::tensor::relu<T>(src, dst);
...
TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph relu matches nntile::core::tensor::relu",
    "[graph][tensor]")
{
    ...
    check_relu_vs_tensor_api<nntile::core::fp32_t>(shape);
}

TEST_CASE_METHOD(nntile::core::test::ContextFixture,
    "TensorGraph relu tiled matches untiled",
    "[graph][tensor]")
{
    ...
    {
        TensorGraph graph("relu_untiled");
        auto *src_node = graph.data(shape, DataType::FP32)->set_name("src");
        src_node->mark_input(true);
        auto *dst_node = gt::relu(src_node);
