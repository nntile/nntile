 ...
 * Test TensorGraph silu_backward operation against nntile::tensor::silu_backward.
 *
 * @version 1.1.0
 * */
...
template<typename T>
void check_silu_backward_vs_tensor_api(
    const std::vector<Index>& shape)
{
    ...
    // --- TensorGraph path ---
    TensorGraph graph("silu_backward_test");
    ...
    silu_backward(x_node, dy_node, dx_node);
    ...
    tensor::silu_backward<T>(x_t, dy_t, dx_t);
...
TEST_CASE("TensorGraph silu_backward rejects duplicate tensors", "[graph][tensor]")
{
    TensorGraph graph("test");
    auto* x = graph.data({4, 5}, "x");
    auto* dy = graph.data({4, 5}, "dy");

    REQUIRE_THROWS_AS(silu_backward(x, x, "dx"), std::invalid_argument);
    REQUIRE_THROWS_AS(silu_backward(x, dy, x), std::invalid_argument);
    REQUIRE_THROWS_AS(silu_backward(x, dy, dy), std::invalid_argument);
}

TEST_CASE_METHOD(nntile::test::ContextFixture,
    "TensorGraph silu_backward matches tensor::silu_backward", "[graph][tensor]")
{
