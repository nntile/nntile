        ...
        Index axis)
{
    ...
    // Perform tensor-wise and tile-wise scale_slice operations
    scale_slice<T>(-1.5, src, dst, axis);
    if(mpi_rank == mpi_root)
    {
        tile::scale_slice<T>(-1.5, src_single.get_tile(0),
...
void validate()
{
    ...
    TEST_THROW(scale_slice<T>(1.0, B, A, -1));
    TEST_THROW(scale_slice<T>(1.0, B, A, 2));
    TEST_THROW(scale_slice<T>(1.0, B, A, 0));
    TEST_THROW(scale_slice<T>(1.0, B, A, 1));
    TEST_THROW(scale_slice<T>(1.0, C, A, 0));
    TEST_THROW(scale_slice<T>(1.0, C, A, 1));
}
