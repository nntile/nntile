        ...
        Index axis)
{
    ...
    // Perform tensor-wise and tile-wise softmax operations
    softmax<T>(src, dst, axis);
    if(mpi_rank == mpi_root)
    {
        tile::softmax<T>(src_single.get_tile(0),
...
void validate()
{
    ...
    TEST_THROW(softmax<T>(B, A, 0));
    TEST_THROW(softmax<T>(D, D, 0));
    TEST_THROW(softmax<T>(C, A, 0));
    TEST_THROW(softmax<T>(C, A, -1));
    TEST_THROW(softmax<T>(C, A, 2));
    TEST_THROW(softmax<T>(E, A, 0));
    TEST_THROW(softmax<T>(F, A, 0));
    TEST_THROW(softmax<T>(C, A, 1));
    TEST_THROW(softmax<T>(G, A, 0));
    TEST_THROW(softmax<T>(G, A, 1));
}
