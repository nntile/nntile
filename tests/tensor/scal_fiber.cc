        ...
        Index axis)
{
    ...
    scal_fiber<T>(-1.0, src1, 0.5, src2, dst, axis, 0);
    if(mpi_rank == mpi_root)
    {
        tile::scal_fiber<T>(-1.0, src1_single.get_tile(0), 0.5,
...
void validate()
{
    ...
    TEST_THROW(scal_fiber<T>(1.0, A, 0.0, A, C, 0, 0));
    TEST_THROW(scal_fiber<T>(1.0, B, 0.0, A, C, -1, 0));
    TEST_THROW(scal_fiber<T>(1.0, B, 0.0, A, B, 2, 0));
    TEST_THROW(scal_fiber<T>(1.0, B, 0.0, A, B, 0, 0));
    TEST_THROW(scal_fiber<T>(1.0, B, 0.0, A, A, 1, 0));
    TEST_THROW(scal_fiber<T>(1.0, C, 0.0, A, A, 0, 0));
    TEST_THROW(scal_fiber<T>(1.0, C, 0.0, A, B, 1, 0));
}
