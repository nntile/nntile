 ...
 *
 * @file tests/tensor/scal_fiber.cc
        ...
        Index axis)
{
    ...
    std::vector<Index> src_shape{shape[axis]}, src_basetile{basetile[axis]};
    ...
    // Perform tensor-wise and tile-wise scal_fiber operations
    scal_fiber<T>(-1.0, src, dst, axis, 0);
    if(mpi_rank == mpi_root)
    {
        tile::scal_fiber<T>(-1.0, src_single.get_tile(0), dst_single.get_tile(0), axis, 0);
    }
    // Compare results
...
void validate()
{
    ...
    TEST_THROW(scal_fiber<T>(1.0, A, C, 0, 0));
    TEST_THROW(scal_fiber<T>(1.0, B, C, -1, 0));
    TEST_THROW(scal_fiber<T>(1.0, B, B, 2, 0));
    TEST_THROW(scal_fiber<T>(1.0, B, B, 0, 0));
    TEST_THROW(scal_fiber<T>(1.0, B, A, 1, 0));
    TEST_THROW(scal_fiber<T>(1.0, C, A, 0, 0));
    TEST_THROW(scal_fiber<T>(1.0, C, B, 1, 0));
}
...
int main(int argc, char **argv)
{
    ...
    // Init codelet
    starpu::scal_fiber::init();
    starpu::subcopy::init();
    starpu::copy::init();
    starpu::scal_fiber::restrict_where(STARPU_CPU);
