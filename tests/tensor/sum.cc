...
void check(const std::vector<Index> &shape, const std::vector<Index> &basetile)
{
    ...
    TensorTraits dst_single_traits({}, {});
    ...
    sum<T>(alpha, src, beta, dst);
    if(mpi_rank == mpi_root)
    {
        tile::sum<T>(alpha, src_single.get_tile(0), beta,
                dst_single.get_tile(0));
    }
    // Compare results
...
void validate()
{
    ...
    Tensor<T> A(trA, dist0000), B(trB, dist0),
        ...
        G(trG, dist00);
    TEST_THROW(sum<T>(1.0, A, 1.0, C));
    TEST_THROW(sum<T>(1.0, A, 1.0, B));
    TEST_THROW(sum<T>(1.0, A, 1.0, D));
}
