...
void check(const std::vector<Index> &shape, const std::vector<Index> &basetile)
{
    ...
    // Perform tensor-wise and tile-wise hypot operations
    hypot<T>(-1.0, src1, 0.5, src2, dst);
    if(mpi_rank == mpi_root)
    {
        tile::hypot<T>(-1.0, src1_single.get_tile(0), 0.5,
...
void validate()
{
    ...
    TEST_THROW(hypot<T>(1.0, A, 0.0, B, C));
    TEST_THROW(hypot<T>(1.0, A, 0.0, C, B));
    TEST_THROW(hypot<T>(1.0, B, 0.0, C, A));
    TEST_THROW(hypot<T>(1.0, B, 0.0, A, C));
    TEST_THROW(hypot<T>(1.0, C, 0.0, B, A));
    TEST_THROW(hypot<T>(1.0, C, 0.0, A, B));
    TEST_THROW(hypot<T>(1.0, A, 0.0, A, B));
}
