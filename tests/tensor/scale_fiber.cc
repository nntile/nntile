...
// Helper function to run Tensor test and verify results
template<typename T, bool run_bench>
void run_tensor_test(TestData<T>& data)
{
    std::vector<T> dst_tensor(data.dst_init);
    std::vector<T> src_tensor(data.src_init);

    if constexpr (run_bench)
    {
        BENCHMARK(
            "[tensor][scale_fiber][m=" +
        ...
        {
            // Placeholder for tensor test
            scale_fiber_async<T>(data.alpha, src_tensor, dst_tensor, 0, data.batch);
        };
    }
    else
    {
        // Placeholder for tensor test
        scale_fiber_async<T>(data.alpha, src_tensor, dst_tensor, 0, data.batch);
        verify_results(data, src_tensor, dst_tensor);
    }
}

// Catch2-based tests
TEMPLATE_TEST_CASE(
    "Scale Fiber Tensor Verification",
    "[scale_fiber]",
    fp64_t,
    fp32_t,
    fp16_t,
...
)
{
    ...
    SECTION("tensor")
    {
        run_tensor_test<T, false>(data);
    }
}

// Catch2-based benchmarks
TEMPLATE_TEST_CASE(
    "Scale Fiber Tensor Benchmark",
