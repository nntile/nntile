...
void validate_cpu(Index nelems)
{
    ...
    std::cout << "Run starpu::dgelutanh::submit<T> restricted to CPU\n";
    ...
    std::cout << "OK: starpu::dgelutanh::submit<T> restricted to CPU\n";
}
...
void validate_cuda(Index nelems)
{
    ...
    std::cout << "Run starpu::dgelutanh::submit<T> restricted to CUDA\n";
    dgelutanh::submit<T>(nelems, data2_handle);
    starpu_task_wait_for_all();
    data2_handle.unregister();
    // Check result
    ...
    std::cout << "OK: starpu::dgelutanh::submit<T> restricted to CUDA\n";
}
#endif // NNTILE_USE_CUDA
