...
void validate_cpu(Index nelems)
{
    ...
    std::cout << "Run starpu::drelu::submit<" << T::short_name << "> restricted to CPU\n";
    drelu.submit<std::tuple<T>>(nelems, data2_handle);
    ...
    std::cout << "OK: starpu::drelu::submit<" << T::short_name << "> restricted to CPU\n";
}
...
void validate_cuda(Index nelems)
{
    ...
    std::cout << "Run starpu::drelu::submit<" << T::short_name << "> restricted to CUDA\n";
    drelu.submit<std::tuple<T>>(nelems, data2_handle);
    starpu_task_wait_for_all();
    data2_handle.unregister();
    // Check result
    ...
    std::cout << "OK: starpu::drelu::submit<" << T::short_name << "> restricted to CUDA\n";
}
#endif // NNTILE_USE_CUDA
