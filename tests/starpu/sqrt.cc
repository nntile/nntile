...
void validate_cpu(Index nelems)
{
    ...
    std::cout << "Run kernel::sqrt::cpu<T>\n";
    kernel::sqrt::cpu<T>(nelems, &data[0]);
    // Check by actually submitting a task
    VariableHandle data2_handle(&data2[0], sizeof(T)*nelems, STARPU_RW);
    sqrt::restrict_where(STARPU_CPU);
    std::cout << "Run starpu::sqrt::submit<T> restricted to CPU\n";
    sqrt::submit<T>(nelems, data2_handle);
    ...
    std::cout << "OK: starpu::sqrt::submit<T> restricted to CPU\n";
}
