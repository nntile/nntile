...
void validate_cpu(Index m, Index n)
{
    ...
    std::cout << "Run starpu::transpose::submit<T> restricted to CPU\n";
    ...
    std::cout << "OK: starpu::transpose::submit<T> restricted to CPU\n";
}
...
void validate_cuda(Index m, Index n)
{
    ...
    std::cout << "Run starpu::transpose::submit<T> restricted to CUDA\n";
    transpose::submit<T>(m, n, 0.5, src_handle, dst2_handle);
    starpu_task_wait_for_all();
    dst2_handle.unregister();
    // Check result
    ...
    std::cout << "OK: starpu::transpose::submit<T> restricted to CUDA\n";
}
#endif // NNTILE_USE_CUDA
