...
void validate_cpu(Index m, Index n, Index k)
{
    ...
    std::cout << "Run starpu::add_slice::submit<T> restricted to CPU\n";
    ...
    std::cout << "OK: starpu::add_slice::submit<T> restricted to CPU\n";
}
...
void validate_cuda(Index m, Index n, Index k)
{
    ...
    std::cout << "Run starpu::add_slice::submit<T> restricted to CUDA\n";
    add_slice::submit<T>(m, n, k, 0.5, src_handle, -0.5, dst2_handle);
    starpu_task_wait_for_all();
    dst2_handle.unregister();
    // Check result
    ...
    std::cout << "OK: starpu::add_slice::submit<T> restricted to CUDA\n";
}
#endif // NNTILE_USE_CUDA
