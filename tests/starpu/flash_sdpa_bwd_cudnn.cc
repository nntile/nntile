...
void validate_cuda(Index seq, Index head, Index batch)
{
    ...
    kernel::flash_sdpa_fwd_cudnn::execute_graph<T>(
        handle, fwd_graph, dev_K, dev_Q, dev_mask, dev_lse, dev_V, dev_O);
    CUDA_CHECK(cudaStreamSynchronize(stream), "sync fwd");
    ...
    std::vector<T> O_staging(total);
    std::vector<fp32_t> lse_host(seq * batch);
    ...
    CUDA_CHECK(cudaMemcpy(O_staging.data(), dev_O, sizeof(T) * total, cudaMemcpyDeviceToHost), "copy O");
    CUDA_CHECK(cudaMemcpy(lse_host.data(), dev_lse, sizeof(fp32_t) * seq * batch, cudaMemcpyDeviceToHost), "copy lse");

    // StarPU buffers
    std::vector<T> K_starpu = K;
    std::vector<T> Q_starpu = Q;
    std::vector<T> V_starpu = V;
    std::vector<T> O_starpu = O_staging;
    ...
    std::vector<fp32_t> lse_starpu = lse_host;
    ...
    auto compare = [&](const std::vector<T> &ref, const std::vector<T> &got)
    {
        for(Index i = 0; i < total; ++i)
        {
            float r = static_cast<float>(static_cast<Y>(ref[i]));
            float g = static_cast<float>(static_cast<Y>(got[i]));
            const float tol = 5e-2f + 1e-2f * std::abs(r);
            TEST_ASSERT(std::abs(r - g) <= tol);
        }
    };
...
int main(int argc, char **argv)
{
    ...
    validate_cuda<fp16_t>(64, 32, 2);
    validate_cuda<bf16_t>(64, 32, 2);
#else
    (void)argc;
    (void)argv;
#endif
