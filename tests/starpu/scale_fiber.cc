...
using namespace nntile::starpu::scale_fiber;
...
// Helper function to run StarPU test and verify results
template<typename T, bool run_bench>
void run_starpu_test(TestData<T>& data)
{
    std::vector<T> dst_starpu(data.dst_init);
    std::vector<T> src_starpu(data.src_init);

    if constexpr (run_bench)
    {
        BENCHMARK(
            "[starpu][scale_fiber][m=" +
        ...
        {
            ScaleFiber<T> scale_fiber(data.m, data.n, data.k, data.batch, data.alpha);
            scale_fiber.submit(&src_starpu[0], &dst_starpu[0]);
        };
    }
    else
    {
        ScaleFiber<T> scale_fiber(data.m, data.n, data.k, data.batch, data.alpha);
        scale_fiber.submit(&src_starpu[0], &dst_starpu[0]);
        verify_results(data, src_starpu, dst_starpu);
    }
}

// Catch2-based tests
TEMPLATE_TEST_CASE(
    "Scale Fiber StarPU Verification",
    "[scale_fiber]",
    fp64_t,
    fp32_t,
    fp16_t,
...
)
{
    ...
    SECTION("starpu")
    {
        run_starpu_test<T, false>(data);
    }
}

// Catch2-based benchmarks
TEMPLATE_TEST_CASE(
    "Scale Fiber StarPU Benchmark",
