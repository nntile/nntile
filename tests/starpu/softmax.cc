 ...
 *
 * @file tests/starpu/softmax.cc
 * Softmax operation on a StarPU buffer
 *
 * @version 1.0.0
...
void validate_cpu(Index m, Index n, Index k)
{
    ...
    std::cout << "Run kernel::softmax::cpu<T>\n";
    kernel::softmax::cpu<T>(m, n, k, &maxsumexp[0], &dst[0]);
    ...
    std::cout << "Run starpu::softmax::submit<T> restricted to CPU\n";
    softmax::submit<T>(m, n, k, maxsumexp_handle, dst2_handle);
    ...
    std::cout << "OK: starpu::softmax::submit<T> restricted to CPU\n";
}
...
void validate_cuda(Index m, Index n, Index k)
{
    ...
    cuda_err = cudaMemcpy(dev_dst, &dst[0], sizeof(T)*m*n*k,
            cudaMemcpyHostToDevice);
    TEST_ASSERT(cuda_err == cudaSuccess);
    std::cout << "Run kernel::softmax::cuda<T>\n";
    kernel::softmax::cuda<T>(stream, m, n, k, dev_maxsumexp, dev_dst);
    ...
    std::cout << "Run starpu::softmax::submit<T> restricted to CUDA\n";
    softmax::submit<T>(m, n, k, maxsumexp_handle, dst2_handle);
    starpu_task_wait_for_all();
    dst2_handle.unregister();
    // Check result
    ...
    std::cout << "OK: starpu::softmax::submit<T> restricted to CUDA\n";
}
#endif // NNTILE_USE_CUDA
