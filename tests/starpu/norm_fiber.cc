...
void validate_cpu(Index m, Index n, Index k, Index batch, Scalar alpha, Scalar beta)
{
    ...
    std::cout << "Run kernel::norm_fiber::cpu<" << T::short_name << ">\n";
    kernel::norm_fiber::cpu<T>(m, n, k, batch, alpha, &src1[0], beta, &src2[0], &dst[0]);
    // Check by actually submitting a task
    VariableHandle src2_handle(&src2[0], sizeof(T)*k*batch);
    ...
    std::cout << "OK: starpu::norm_fiber::submit<" << T::short_name << "> restricted to CPU\n";
}
