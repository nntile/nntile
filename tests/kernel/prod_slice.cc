...
void reference_prod_slice(TestData<T>& data)
{
    ...
    for(Index i2 = 0; i2 < data.n; ++i2)
    {
        ...
        {
            const ref_t src_val = alpha_r * static_cast<Y>(data.src[i2 * data.m + i1]);
            for(Index i0 = 0; i0 < data.k; ++i0)
            {
                Index dst_idx = (i2 * data.k + i0) * data.m + i1;
                ref_t result = static_cast<Y>(data.dst[dst_idx]) * src_val;
...
void generate_data(TestData<T>& data, DataGen strategy)
{
    ...
    data.dst.resize(data.m * data.n * data.k);
    ...
    switch(strategy)
    {
        ...
        case DataGen::PRESET:
            for(Index i2 = 0; i2 < data.n; ++i2)
            {
                for(Index i1 = 0; i1 < data.m; ++i1)
                {
                    data.src[i2 * data.m + i1] = Y{2.0 * (i1 + i2) + 1.0};
                }
            }
            for(Index i2 = 0; i2 < data.n; ++i2)
            {
                for(Index i0 = 0; i0 < data.k; ++i0)
                {
                    ...
                    {
                        Index dst_idx = (i2 * data.k + i0) * data.m + i1;
                        data.dst[dst_idx] = Y{1.0 + i0 + i1 * data.k + i2 * data.k * data.m};
                    }
                }
            }
            break;
        ...
        case DataGen::RANDOM:
            ...
            for(Index i = 0; i < data.dst.size(); ++i)
            {
                data.dst[i] = dist(gen);
            }
            break;
...
void run_cuda_test(TestData<T>& data)
{
    ...
    CUDA_CHECK(cudaMalloc(&dev_src, sizeof(T) * data.src.size()),
               "cudaMalloc dev_src");
    CUDA_CHECK(cudaMalloc(&dev_dst, sizeof(T) * data.dst.size()),
               "cudaMalloc dev_dst");

    std::vector<T> dst_cuda(data.dst);

    CUDA_CHECK(cudaMemcpy(dev_src, &data.src[0], sizeof(T) * data.src.size(),
                          cudaMemcpyHostToDevice), "cudaMemcpy dev_src");
    CUDA_CHECK(cudaMemcpy(dev_dst, &data.dst[0], sizeof(T) * data.dst.size(),
    ...
    else
    {
        ...
        CUDA_CHECK(cudaMemcpy(&dst_cuda[0], dev_dst, sizeof(T) * data.dst.size(),
