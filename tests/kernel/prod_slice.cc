...
void reference_prod_slice(TestData<T>& data)
{
    ...
    for(Index i2 = 0; i2 < data.n; ++i2)
    {
        ...
        {
            const ref_t src_val = alpha_r * static_cast<Y>(data.src_init[i2 * data.m + i1]);
            for(Index i0 = 0; i0 < data.k; ++i0)
            {
                Index dst_idx = (i2 * data.k + i0) * data.m + i1;
                ref_t result = static_cast<Y>(data.dst_init[dst_idx]) * src_val;
    ...
    switch(strategy)
    {
        ...
        case DataGen::PRESET:
            for(Index i2 = 0; i2 < data.n; ++i2)
            {
                for(Index i1 = 0; i1 < data.m; ++i1)
                {
                    data.src_init[i2 * data.m + i1] = Y{2.0 * (i1 + i2) + 1.0};
                }
            }
            for(Index i2 = 0; i2 < data.n; ++i2)
            {
                for(Index i0 = 0; i0 < data.k; ++i0)
                {
                    ...
                    {
                        Index dst_idx = (i2 * data.k + i0) * data.m + i1;
                        data.dst_init[dst_idx] = Y{1.0 + i0 + i1 * data.k + i2 * data.k * data.m};
                    }
                }
            }
            break;
