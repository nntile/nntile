...
void reference_prod_fiber(TestData<T>& data)
{
    ...
    {
        const ref_t src_val = alpha_r * static_cast<Y>(data.src[i2]);
        for(Index i1 = 0; i1 < data.n; ++i1)
        {
            for(Index i0 = 0; i0 < data.m; ++i0)
            {
                Index dst_idx = (i1 * data.k + i2) * data.m + i0;
                ref_t result = src_val * static_cast<Y>(data.dst[dst_idx]);
        ...
        case DataGen::PRESET:
            ...
            for(Index i2 = 0; i2 < data.k; ++i2)
            {
                for(Index i1 = 0; i1 < data.n; ++i1)
                {
                    ...
                    {
                        Index dst_idx = (i1 * data.k + i2) * data.m + i0;
                        data.dst[dst_idx] = Y{1.0 + i0 + i1 * data.m + i2 * data.m * data.n};
                    }
                }
            }
            break;
