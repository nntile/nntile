...
template<typename T>
void generate_data(TestData<T> &data, Index seq, Index batch, DataGen strategy)
{
    using repr_t = typename T::repr_t;
    using lse_t = typename fp32_t::repr_t;
    data.seq = seq;
    ...
    data.src_attn_init.resize(data.nelems);
    data.dst_attn_init.resize(data.nelems);
    data.dst_attn_ref.resize(data.nelems);
    ...
    switch(strategy)
    {
        case DataGen::PRESET:
            ...
            {
                const lse_t base = lse_t(-3.0f) + lse_t(0.25f) * lse_t(i % 17);
                const bool dst_active = ((i + 1) % 3) != 0;
                const bool src_active = ((i + 2) % 4) != 0;
                data.dst_lse_init[i] = dst_active ? base : neg_inf;
                data.src_lse_init[i] = src_active ? (base + lse_t(0.5f)) : neg_inf;

                const repr_t dst_val =
                        dst_active ? repr_t(0.1f * (i % 11) - 1.0f) : repr_t(0);
                const repr_t src_val =
                        src_active ? repr_t(0.2f * ((i + 3) % 13) - 0.5f) : repr_t(0);

                data.dst_attn_init[i] = static_cast<T>(dst_val);
                data.src_attn_init[i] = static_cast<T>(src_val);
            }
            break;
        case DataGen::RANDOM:
        {
            ...
            for(Index i = 0; i < data.nelems; ++i)
            {
                ...
                data.dst_lse_init[i] = dst_has_mass ? dist_lse(gen) : neg_inf;
                data.src_lse_init[i] = src_has_mass ? dist_lse(gen) : neg_inf;

                const repr_t dst_val =
                        dst_has_mass ? repr_t(dist_val(gen)) : repr_t(0);
                const repr_t src_val =
                        src_has_mass ? repr_t(dist_val(gen)) : repr_t(0);
...
template<typename T>
TestData<T> get_test_input_data(Index seq, Index batch, DataGen strategy)
{
    TestData<T> data;
    generate_data(data, seq, batch, strategy);
...
void reference_accumulate(TestData<T> &data)
{
    ...
    {
        const Index offset = s * data.batch;
        for(Index b = 0; b < data.batch; ++b)
        {
            const Index idx = offset + b;
            const lse_t old_lse = static_cast<lse_t>(data.dst_lse_ref[idx]);
            const lse_t incoming_lse = static_cast<lse_t>(data.src_lse_init[idx]);
            ...
            const ref_t dst_val = static_cast<repr_t>(data.dst_attn_ref[idx]);
            const ref_t src_val = static_cast<repr_t>(data.src_attn_init[idx]);
            const ref_t updated = dst_weight * dst_val + src_weight * src_val;

            data.dst_lse_ref[idx] = new_lse;
            data.dst_attn_ref[idx] = static_cast<T>(static_cast<repr_t>(updated));
        }
    }
}
...
void run_cuda_test(TestData<T> &data)
{
    ...
    CUDA_CHECK(cudaMalloc(&dev_src_attn, sizeof(T) * data.nelems),
            "cudaMalloc dev_src_attn");
    CUDA_CHECK(cudaMalloc(&dev_dst_lse, sizeof(fp32_t) * data.nelems),
            "cudaMalloc dev_dst_lse");
    CUDA_CHECK(cudaMalloc(&dev_dst_attn, sizeof(T) * data.nelems),
    ...
    CUDA_CHECK(cudaMemcpy(dev_src_attn, src_attn.data(),
            sizeof(T) * data.nelems, cudaMemcpyHostToDevice),
    ...
    if constexpr(run_bench)
    {
        BENCHMARK(
                ...
                + "][batch=" + std::to_string(data.batch) + "]")
        {
            cuda<T>(stream, data.seq, data.batch,
    ...
    else
    {
        ...
        CUDA_CHECK(cudaMemcpy(dst_attn.data(), dev_dst_attn,
                sizeof(T) * data.nelems, cudaMemcpyDeviceToHost),
                "cudaMemcpy dst_attn");
        ...
        fp32_t, fp16_t, bf16_t)
{
    ...
    auto data = get_test_input_data<T>(seq, batch, strategy);
        ...
        fp32_t, fp16_t, bf16_t)
{
    ...
    auto data = get_test_input_data<T>(seq, batch, DataGen::PRESET);
