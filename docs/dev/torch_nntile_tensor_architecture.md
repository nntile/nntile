...
| D9 | Convolution public `*.out` only | StarPU conv codelets call **only** public ATen `*_out` (no hidden `raw_cudnn_*` / undeclared CUDA leaves). Cudnn forward uses real `cudnn_convolution.out` into the StarPU blob. Transpose forward (`cudnn_convolution_transpose.out`) and CUDA bwd (`convolution_backward.out`) are **autogen**: cuDNN (or the CUDA composite) then `copy_` into the blob. Unused bwd grads need same-dtype throwaways; autogen cannot copy into an undefined tensor. Extra D2D of conv outputs/grads vs CUDA’s `empty_cuda` + in-place raw write. | If PyTorch exports a public dgrad/wgrad `*_out` that writes into a caller buffer, switch to that. Do **not** `dlsym` / link hidden `raw_*` symbols. See [torch_starpu_kernels.md](torch_starpu_kernels.md). |
