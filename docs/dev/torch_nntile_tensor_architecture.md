...
| D6 | libnntile required | `torch_nntile` builds always define `TORCH_NNTILE_USE_LIBNNTILE` and link `libnntile`; host-only stub installs are unsupported. | Keep the libnntile path the only storage/executor path; do not reintroduce a stub build. |
