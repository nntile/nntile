...
CI builds `torch_nntile` 0.1.0 wheels via the **`torch_nntile wheels`** workflow
...
auto-flush are **off by default**. Set
``NNTILE_ENABLE_LOCAL_COMPILER=1`` to lower TensorGraph → TileGraph on your
own machine. Without that exact value they raise. Shared-node
...
`bind_data` in torch_nntile. Legacy `execute()` is compile+run, and
``.to("cpu")`` auto-flushes pending work, only when the local-compiler flag is
set.
...
| `Runtime::compile` + `execute` + `wait` | `compile_graph()` / `run()` / `wait()` (legacy `execute()` = compile+run). Local lower (`compile_graph()`, `execute()`, `.to("cpu")` auto-flush) needs `NNTILE_ENABLE_LOCAL_COMPILER=1`. |
