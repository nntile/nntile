 ...
 *
 * @file src/kernel/bias_outer/cpu.cc
...
void cpu(Index m, Index n, Index k, T alpha, const T *src, T *dst)
    noexcept
//! Bias along outer axes on CPU
/*! For a provided m-by-k-by-n output tensor dst apply bias along the first and
 * the third axes from an input tensor src of shape (k):
 *      dst[:, i, :] += alpha * src[i]
 *
