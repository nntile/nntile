 ...
 *
 * @file src/kernel/cuda/sumnorm.cu
...
template<typename T>
static __global__
void sumnorm_kernel(Index m, Index n, Index k, Index mk, const T *src,
        T *sumnorm)
{
...
template
void sumnorm<fp32_t>(cudaStream_t stream, Index m, Index n, Index k,
        const fp32_t *src, fp32_t *sumnorm)
    noexcept;

template
void sumnorm<fp64_t>(cudaStream_t stream, Index m, Index n, Index k,
