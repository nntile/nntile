        ...
        const T *grad, T *p)
{
    ...
    if(i < num_elems)
    {
        ...
        Y lr_val{lr}, trust_ratio_val{trust_ratio}, weight_norm_val{weight_norm};
        Y grad_norm_val{grad_norm}, weight_decay_val{weight_decay};
        ...
        Y local_lr = (grad_norm_val > 0) ? lr_val * weight_norm_val / grad_norm_val : lr_val;
 ...
 * */
{
    ...
    (cuda_kernel<T>)<<<blocks, threads, 0, stream>>>(num_elems,
            lr, trust_ratio, weight_norm, grad_norm, weight_decay,
            grad, p);
}
