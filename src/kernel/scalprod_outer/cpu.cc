 ...
 *
 * @file src/kernel/scalprod_outer/cpu.cc
 * Scalar product along outer axes of buffers on CPU
 *
 * @version 1.0.0
...
void cpu(Index m, Index n, Index k, T alpha, const T *src1, const T *src2,
        T beta, T *dst)
    noexcept
//! Scalar product along outer axes
/*! For two provided m-by-k-by-n input arrays src1 and src2 compute scalar
 * products of slices along the first and the third axis with m and n elements
 * respectively, resulting in output array dst of shape (k). If beta is
 * non-zero, then values of array dst are updated
 ...
 *      dst[i] = beta*dst[i] + alpha*sum(src1[:,i,:]*src2[:,i,:])
 *      
 * @param[in] m: Size of the first mode of src1 and src2
 * @param[in] n: Size of the last mode of src1, and src2
 * @param[in] k: Size of the middle mode of src1, src2 and dst arrays
 ...
 * @param[inout] dst: Output contiguous array of shape (k), that accumulates
 *      scalar products of src1 and src2 along outer axes.
 * */
{
    constexpr T zero = 0;
    ...
    for(Index i2 = 0; i2 < k; ++i2)
    {
        ...
        for(Index i1 = 0; i1 < n; ++i1)
        {
            ...
            const T *src1_slice = src1 + (i1*k+i2)*m;
            const T *src2_slice = src2 + (i1*k+i2)*m;
            // Cycle over slices of inputs
            for(Index i0 = 0; i0 < m; ++i0)
            {
                // Read value from sources
                T val1 = src1_slice[i0];
                T val2 = src2_slice[i0];
