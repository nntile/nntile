...
template<typename T>
static void cpu_sum_ssq_single_axis(void *buffers[], void *cl_args)
{
    // Get sizes
    Index m, n, k;
...
void norm_sum_ssq_async(const Tile<T> &src, const Tile<T> &sum_ssq,
        Index axis)
{
    static struct starpu_codelet codelet_sum_ssq_single_axis =
    {
        .cpu_funcs = {cpu_sum_ssq_single_axis<T>},
        .nbuffers = 2,
        .modes = {STARPU_R, STARPU_W}
    };
    // Check inputs
    ...
    // Insert task
    starpu_task_insert(&codelet_sum_ssq_single_axis,
