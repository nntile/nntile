 ...
 *
 * @file src/tile/lamb_step.cc
 * LAMB step for Tile<T>
 *
 * @version 1.1.0
 * */

#include "nntile/tile/lamb_step.hh"
#include "nntile/starpu/lamb_step.hh"

namespace nntile::tile
{

//! Asynchronous version of tile-wise fused LAMB step
                     ...
                     const Tile<T> &p)
{
    ...
    {
        throw std::runtime_error("Shapes of second_moment and parameters are not equal");
    }
    // Submit task
    starpu::lamb_step.submit<std::tuple<T>>(num_iter, p.nelems, beta_1, beta_2, eps, lr, weight_decay,
                                 min_trust, max_trust, grad, first_moment, second_moment, p);
}

//! Blocking version of tile-wise fused LAMB step
/*! * @param[in] num_iters: current iteration number
 * @param[in] beta_1: parameter for moving average of first moments
 * @param[in] beta_2: parameter for moving average of second moments
 * @param[in] eps: small scalar to avoid division by zero
