...
template
void mask_scalar_async<fp32_t>(const Tile<bool_t> &mask, fp32_t val,
        const Tile<fp32_t> &A);

template
void mask_scalar_async<fp64_t>(const Tile<bool_t> &mask, fp64_t val,
        const Tile<fp64_t> &A);

// Explicit instantiation
template
void mask_scalar<fp32_t>(const Tile<bool_t> &mask, fp32_t val,
        const Tile<fp32_t> &A);

template
void mask_scalar<fp64_t>(const Tile<bool_t> &mask, fp64_t val,
        const Tile<fp64_t> &A);
