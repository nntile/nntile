...
template
void nrm2_async<fp32_t>(scal_t alpha, const Tile<fp32_t> &src, scal_t beta,
        const Tile<fp32_t> &dst, const Tile<fp32_t> &tmp);

template
void nrm2_async<fp64_t>(scal_t alpha, const Tile<fp64_t> &src, scal_t beta,
        const Tile<fp64_t> &dst, const Tile<fp64_t> &tmp);

// Explicit instantiation
template
void nrm2<fp32_t>(scal_t alpha, const Tile<fp32_t> &src, scal_t beta,
        const Tile<fp32_t> &dst, const Tile<fp32_t> &tmp);

template
void nrm2<fp64_t>(scal_t alpha, const Tile<fp64_t> &src, scal_t beta,
        const Tile<fp64_t> &dst, const Tile<fp64_t> &tmp);
