...
template<typename T>
void add_slice_async(scal_t alpha, const Tile<T> &src, scal_t beta, const Tile<T> &dst,
        Index axis)
...
template<typename T>
void add_slice(scal_t alpha, const Tile<T> &src, scal_t beta, const Tile<T> &dst,
               Index axis)
//! Tile<T> addition of a tensor and a broadcasted slice
...
template
void add_slice_async<fp32_t>(scal_t alpha, const Tile<fp32_t> &src,
        scal_t beta, const Tile<fp32_t> &dst, Index axis);

template
void add_slice_async<fp64_t>(scal_t alpha, const Tile<fp64_t> &src,
        scal_t beta, const Tile<fp64_t> &dst, Index axis);

template
void add_slice_async<fp32_fast_tf32_t>(scal_t alpha, const Tile<fp32_fast_tf32_t> &src,
        scal_t beta, const Tile<fp32_fast_tf32_t> &dst, Index axis);

// Explicit instantiation of template
template
void add_slice<fp32_t>(scal_t alpha, const Tile<fp32_t> &src, scal_t beta,
        const Tile<fp32_t> &dst, Index axis);

template
void add_slice<fp32_fast_tf32_t>(scal_t alpha, const Tile<fp32_fast_tf32_t> &src, scal_t beta,
        const Tile<fp32_fast_tf32_t> &dst, Index axis);

template
void add_slice<fp64_t>(scal_t alpha, const Tile<fp64_t> &src, scal_t beta,
