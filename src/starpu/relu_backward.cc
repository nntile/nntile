...
namespace nntile::starpu::relu_backward
{
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::relu_backward::cpu<T>(nelems, x, dy, dx);
#endif // STARPU_SIMGRID
}
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::relu_backward::cuda<T>(stream, nelems, x, dy, dx);
#endif // STARPU_SIMGRID
}
...
void init()
{
    codelet_fp32.init("nntile_relu_backward_fp32",
            nullptr,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_fp64.init("nntile_relu_backward_fp64",
            nullptr,
            {cpu<fp64_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp64_t>}
...
//            0);
    ...
    if(task)
    {
        ...
        *nelems_ = nelems;
        ...
        if(ret != 0)
        {
            throw std::runtime_error("Error in relu_backward MPI task "
                    "submission");
        }
    }
