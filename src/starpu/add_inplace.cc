    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::add_inplace::cpu<T>(args->nelems, args->alpha, src, args->beta, dst);
#endif // STARPU_SIMGRID
}
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::add_inplace::cuda<T>(stream, args->nelems, args->alpha, src,
            args->beta, dst);
#endif // STARPU_SIMGRID
}
...
void init()
{
    codelet_fp32.init("nntile_add_inplace_fp32",
            footprint,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_bf16.init("nntile_add_inplace_bf16",
            footprint,
            {cpu<bf16_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<bf16_t>}
    ...
    codelet_fp32_fast_tf32.init("nntile_add_inplace_fp32_fast_tf32",
            footprint,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_fp32_fast_bf16.init("nntile_add_inplace_fp32_fast_bf16",
            footprint,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
...
template
void submit<fp32_fast_bf16_t>(Index nelems, Scalar alpha, Handle src,
        Scalar beta, Handle dst);
