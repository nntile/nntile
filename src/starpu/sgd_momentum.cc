...
namespace nntile::starpu::sgd_momentum
{

//! Apply sgd_momentum along middle axis of StarPU buffer on CPU
template<typename T>
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::sgd_momentum::cpu<T>(nelems, data);
#endif // STARPU_SIMGRID
}

#ifdef NNTILE_USE_CUDA
//! Apply sgd_momentum along middle axis of StarPU buffer on CUDA
template<typename T>
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::sgd_momentum::cuda<T>(stream, nelems, data);
#endif // STARPU_SIMGRID
}
...
void init()
{
    codelet_fp32.init("nntile_sgd_momentum_fp32",
            nullptr,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_fp64.init("nntile_sgd_momentum_fp64",
            nullptr,
            {cpu<fp64_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp64_t>}
    ...
    codelet_fp32_fast_tf32.init("nntile_sgd_momentum_fp32_fast_tf32",
            nullptr,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
...
void submit(Index nelems, Handle data)
{
    ...
    if(ret != 0)
    {
        throw std::runtime_error("Error in sgd_momentum task submission");
    }
}
