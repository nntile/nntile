 ...
 * */
 ...
 #include "nntile/kernel/muon_step.hh"
 #endif // STARPU_SIMGRID
 #include "nntile/starpu/muon_step.hh"
 #include <cstdlib>
 
 //! StarPU wrappers for one step of Muon optimizer
 namespace nntile::starpu::muon_step
 {
 
 //! Apply Muon step on StarPU buffers on CPU
 template<typename T>
     ...
     noexcept
 {
     ...
     // Launch kernel
     kernel::muon_step::cpu<T>(args->num_iter, args->num_elems, args->beta_1, args->beta_2,
                               args->eps, args->lr, args->weight_decay, grad, first_moments, second_moments, p);
 #endif // STARPU_SIMGRID
 }
 
 #ifdef NNTILE_USE_CUDA
 //! Apply Muon step operation on StarPU buffer on CUDA
 template<typename T>
     ...
     noexcept
 {
     ...
     // Launch kernel
     kernel::muon_step::cuda<T>(stream, args->num_iter, args->num_elems, args->beta_1, args->beta_2,
                               args->eps, args->lr, args->weight_decay, grad, first_moments, second_moments, p);
 #endif // STARPU_SIMGRID
 }
 ...
 void init()
 {
     codelet_fp32.init("nntile_muon_step_fp32",
             nullptr,
             {cpu<fp32_t>},
 #ifdef NNTILE_USE_CUDA
             {cuda<fp32_t>}
     ...
     codelet_bf16.init("nntile_muon_step_bf16",
             nullptr,
             {cpu<bf16_t>},
 #ifdef NNTILE_USE_CUDA
             {cuda<bf16_t>}
     ...
     codelet_fp32_fast_tf32.init("nntile_muon_step_fp32_fast_tf32",
             nullptr,
             {cpu<fp32_t>},
 #ifdef NNTILE_USE_CUDA
             {cuda<fp32_t>}
     ...
     codelet_fp32_fast_fp16.init("nntile_muon_step_fp32_fast_fp16",
             nullptr,
             {cpu<fp32_t>},
 #ifdef NNTILE_USE_CUDA
             {cuda<fp32_t>}
     ...
     codelet_fp32_fast_bf16.init("nntile_muon_step_fp32_fast_bf16",
             nullptr,
             {cpu<fp32_t>},
 #ifdef NNTILE_USE_CUDA
             {cuda<fp32_t>}
     ...
     codelet_fp64.init("nntile_muon_step_fp64",
             nullptr,
             {cpu<fp64_t>},
 #ifdef NNTILE_USE_CUDA
             {cuda<fp64_t>}
             ...
             Handle second_moment, Handle p)
 {
     ...
     if(ret != 0)
     {
         throw std::runtime_error("Error in muon_step task submission");
     }
 }
