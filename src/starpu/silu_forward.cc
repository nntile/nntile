    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::silu_forward::cpu<T>(nelems, src, dst);
#endif // STARPU_SIMGRID
}
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::silu_forward::cuda<T>(stream, nelems, src, dst);
#endif // STARPU_SIMGRID
}
...
void init()
{
    codelet_fp32.init("nntile_silu_forward_fp32",
            nullptr,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_fp32_fast_tf32.init("nntile_silu_forward_fp32_fast_tf32",
            nullptr,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_fp64.init("nntile_silu_forward_fp64",
            nullptr,
            {cpu<fp64_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp64_t>}
    ...
    codelet_bf16.init("nntile_silu_forward_bf16",
            nullptr,
            {cpu<bf16_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<bf16_t>}
