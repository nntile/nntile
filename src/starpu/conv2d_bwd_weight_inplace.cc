...
//! StarPU wrappers for conv2d_bwd_weight_inplace operation
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::conv2d_bwd_weight_inplace::cpu<T>(args->src1_m, args->src1_n,
            args->src1_channels, args->batch, args->src2_m, args->src2_n,
            args->stride_m, args->stride_n, args->src2_channels,
            args->offset_m, args->offset_n, args->alpha, src1, src2,
            args->dst_m, args->dst_n, args->dilation_m, args->dilation_n,
            args->beta, dst);
#endif // STARPU_SIMGRID
}

#ifdef NNTILE_USE_CUDA
//! StarPU wrapper for kernel::conv2d_bwd_weight_inplace::cuda<T>
template <typename T>
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::conv2d_bwd_weight_inplace::cuda<T>(stream, args->src1_m,
            args->src1_n, args->src1_channels, args->batch, args->src2_m,
            args->src2_n, args->stride_m, args->stride_n, args->src2_channels,
            args->offset_m, args->offset_n, args->alpha, src1, src2,
            args->dst_m, args->dst_n, args->dilation_m, args->dilation_n,
            args->beta, dst);
#endif // STARPU_SIMGRID
...
void init()
{
    codelet_bf16.init("nntile_conv2d_bwd_weight_inplace_bf16",
            footprint,
            {cpu<bf16_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<bf16_t>}
    ...
    codelet_fp32.init("nntile_conv2d_bwd_weight_inplace_fp32",
            footprint,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_fp32_fast_tf32.init(
            "nntile_conv2d_bwd_weight_inplace_fp32_fast_tf32",
            footprint,
    ...
    codelet_fp64.init("nntile_conv2d_bwd_weight_inplace_fp64",
            footprint,
            {cpu<fp64_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp64_t>}
...
//! Insert conv2d_bwd_weight_inplace task into StarPU pool of tasks
...
void submit<bf16_t>(Index src1_m, Index src1_n, Index src1_channels,
        ...
        Scalar alpha, Handle src1, Handle src2, Index dst_m, Index dst_n,
        Index dilation_m, Index dilation_n, Scalar beta, Handle dst);
