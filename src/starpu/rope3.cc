 ...
 *
 * @file src/starpu/rope3.cc
 * StarPU wrappers for for ROtary Positional Embedding
 *
 * @version 1.0.0
 * @author Gleb Karpov
 * @date 2024-06-16
...
#include "nntile/starpu/rope3.hh"
#include "nntile/kernel/rope3.hh"
#include <cstdlib>
...
namespace starpu
{
//! StarPU wrappers for rope3 operation
namespace rope3
{
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::add_slice3::cpu<T>(args->m, args->n, args->b, args->s,
        sin, cos, src, dst);
}
...
uint32_t footprint(struct starpu_task *task)
{
    ...
    hash = starpu_hash_crc32c_be_n(&args->b, sizeof(args->b), hash);
    hash = starpu_hash_crc32c_be_n(&args->s, sizeof(args->s), hash);
    return hash;
}
...
template<typename T>
void submit(Index m, Index n, Index b, Index s, T alpha, Handle sin, Handle cos, 
        Handle src, Handle dst)
//! Insert rope3 task into StarPU pool of tasks
 ...
 * */
{
    ...
    if(ret != 0)
    {
        throw std::runtime_error("Error in rope3 task submission");
    }
}

// Explicit instantiation
template
void submit<fp32_t>(Index m, Index n, Index b, Index s, Handle sin,
        Handle cos, Handle src, Handle dst);

template
void submit<fp64_t>(Index m, Index n, Index b, Index s, Handle sin,
        Handle cos, Handle src, Handle dst);
