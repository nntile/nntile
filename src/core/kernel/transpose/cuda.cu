...
#include "nntile/core/kernel/transpose/cuda.hh"
#include <algorithm>
#include "nntile/core/kernel/cuda.hh"
#include "nntile/core/kernel/scale/cuda.hh"

namespace nntile::core::kernel::transpose
{
 ...
 * */
{
    ...
    if(m < n)
    {
        if(m == 1)
        {
            scale::cuda<T>(stream, m*n, alpha, src, dst);
        }
        else if(m < 4)
        {
...
} // namespace nntile::core::kernel::tranpose
