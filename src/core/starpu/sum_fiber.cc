...
SumFiber<std::tuple<T>>::SumFiber():
    codelet("nntile_sum_fiber", footprint, cpu_funcs, cuda_funcs)
{
...
template<>
void SumFiber<std::tuple<fp32_fast_tf32_t>>::cuda(void *buffers[], void *cl_args)
    noexcept
{
    // Fall back to FP32
...
template<>
void SumFiber<std::tuple<fp32_fast_fp16_t>>::cuda(void *buffers[], void *cl_args)
    noexcept
{
    // Fall back to FP32
    SumFiber<std::tuple<fp32_t>>::cuda(buffers, cl_args);
...
template<>
void SumFiber<std::tuple<fp32_fast_bf16_t>>::cuda(void *buffers[], void *cl_args)
    noexcept
{
    // Fall back to FP32
    SumFiber<std::tuple<fp32_t>>::cuda(buffers, cl_args);
...
template<typename T>
uint32_t SumFiber<std::tuple<T>>::footprint(struct starpu_task *task)
{
    // Get arguments
...
template<typename T>
void SumFiber<std::tuple<T>>::submit(Index m, Index n, Index k, Index batch,
        Scalar alpha, Handle src,
...
template class SumFiber<std::tuple<nntile::core::fp64_t>>;
template class SumFiber<std::tuple<nntile::core::fp32_t>>;
template class SumFiber<std::tuple<nntile::core::fp32_fast_tf32_t>>;
template class SumFiber<std::tuple<nntile::core::fp32_fast_fp16_t>>;
template class SumFiber<std::tuple<nntile::core::fp32_fast_bf16_t>>;
template class SumFiber<std::tuple<nntile::core::bf16_t>>;
template class SumFiber<std::tuple<nntile::core::fp16_t>>;
