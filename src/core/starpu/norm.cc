...
#include "nntile/core/starpu/norm.hh"
...
Norm<std::tuple<T>>::Norm():
    codelet("nntile_norm", footprint, cpu_funcs, cuda_funcs)
{
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::norm::cuda<T>(stream, args->nelems, args->alpha, src, args->beta, dst);
#endif // STARPU_SIMGRID
}
...
template<typename T>
uint32_t Norm<std::tuple<T>>::footprint(struct starpu_task *task)
{
    // Get arguments
...
template class Norm<std::tuple<nntile::core::fp64_t>>;
template class Norm<std::tuple<nntile::core::fp32_t>>;
template class Norm<std::tuple<nntile::core::fp32_fast_tf32_t>>;
template class Norm<std::tuple<nntile::core::fp32_fast_fp16_t>>;
template class Norm<std::tuple<nntile::core::fp32_fast_bf16_t>>;
template class Norm<std::tuple<nntile::core::bf16_t>>;
template class Norm<std::tuple<nntile::core::fp16_t>>;
