 ...
 *
 * @file src/starpu/conv2d_inplace.cc
...
template<typename T>
void Conv2dInplace<std::tuple<T>>::cpu(void *buffers[], void *cl_args)
    noexcept
{
    ...
    // Get interfaces
    VariableInterface **interfaces = reinterpret_cast<VariableInterface **>(buffers);
    const T *src1 = interfaces[0]->get_ptr<T>();
    const T *src2 = interfaces[1]->get_ptr<T>();
    T *dst = interfaces[2]->get_ptr<T>();
    // Launch kernel
    kernel::conv2d_inplace::cpu<T>(
        args->src1_m,
        args->src1_n,
        args->src1_channels,
        args->batch,
...
template<typename T>
uint32_t Conv2dInplace<std::tuple<T>>::footprint(struct starpu_task *task)
{
    // Get arguments
    args_t *args = reinterpret_cast<args_t *>(task->cl_arg);
    // Apply hash over all args
    uint32_t hash = 0;
...
template<typename T>
void Conv2dInplace<std::tuple<T>>::submit(Index src1_m, Index src1_n, Index src1_channels, Index batch,
        Index src2_m, Index src2_n, Index dilation_m, Index dilation_n,
        Index dst_channels, Index offset_m, Index offset_n, Scalar alpha,
        Handle src1, Handle src2, Index dst_m, Index dst_n, Index stride_m,
        Index stride_n, Scalar beta, Handle dst)
{
    // Codelet arguments
    ...
    if(ret != 0)
    {
        throw std::runtime_error("Error in conv2d_inplace task submission");
    }
}
...
template class Conv2dInplace<std::tuple<nntile::core::fp64_t>>;
template class Conv2dInplace<std::tuple<nntile::core::fp32_t>>;
template class Conv2dInplace<std::tuple<nntile::core::fp32_fast_tf32_t>>;
template class Conv2dInplace<std::tuple<nntile::core::fp32_fast_fp16_t>>;
template class Conv2dInplace<std::tuple<nntile::core::fp32_fast_bf16_t>>;
template class Conv2dInplace<std::tuple<nntile::core::bf16_t>>;
