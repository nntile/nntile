...
void subtract_indexed_outputs_async<fp32_fast_tf32_t>(scal_t val,
        const Tensor<Index> &labels, const Tensor<fp32_fast_tf32_t> &dst);
...
// Explicit instantiation
template
void subtract_indexed_outputs<fp32_t>(scal_t val, const Tensor<Index> &labels,
        const Tensor<fp32_t> &dst);

template
void subtract_indexed_outputs<fp32_fast_tf32_t>(scal_t val, const Tensor<Index> &labels,
        const Tensor<fp32_fast_tf32_t> &dst);

template
void subtract_indexed_outputs<fp64_t>(scal_t val, const Tensor<Index> &labels,
