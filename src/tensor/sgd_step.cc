                    ...
                    const Tensor<T> &p)
{
    ...
    for(Index i = 0; i < p.grid.nelems; ++i)
    {
        ...
        if(mpi_rank == p_tile_rank)
        {
            ...
            starpu::sgd_step.submit<std::tuple<T>>(traits.nelems, momentum, lr, weight_decay,
...
void sgd_step(Scalar momentum, Scalar lr, Scalar weight_decay,
               ...
               const Tensor<T> &p)
{
    sgd_step_async<T>(momentum, lr, weight_decay, grad, velocity, p);
...
template
void sgd_step_async<fp32_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<fp32_t> &grad, const Tensor<fp32_t> &velocity,
                   const Tensor<fp32_t> &p);

template
void sgd_step_async<fp32_fast_tf32_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<fp32_fast_tf32_t> &grad, const Tensor<fp32_fast_tf32_t> &velocity,
                   const Tensor<fp32_fast_tf32_t> &p);

template
void sgd_step_async<fp32_fast_fp16_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
               const Tensor<fp32_fast_fp16_t> &grad, const Tensor<fp32_fast_fp16_t> &velocity,
               const Tensor<fp32_fast_fp16_t> &p);

template
void sgd_step_async<fp32_fast_bf16_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
               const Tensor<fp32_fast_bf16_t> &grad, const Tensor<fp32_fast_bf16_t> &velocity,
               const Tensor<fp32_fast_bf16_t> &p);

template
void sgd_step_async<fp64_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<fp64_t> &grad, const Tensor<fp64_t> &velocity,
                   const Tensor<fp64_t> &p);

template
void sgd_step_async<bf16_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<bf16_t> &grad, const Tensor<bf16_t> &velocity,
                   const Tensor<bf16_t> &p);

template
void sgd_step_async<fp16_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<fp16_t> &grad, const Tensor<fp16_t> &velocity,
                   const Tensor<fp16_t> &p);

// Explicit instantiation
template
void sgd_step<fp32_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<fp32_t> &grad, const Tensor<fp32_t> &velocity,
                   const Tensor<fp32_t> &p);

template
void sgd_step<fp32_fast_tf32_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<fp32_fast_tf32_t> &grad, const Tensor<fp32_fast_tf32_t> &velocity,
                   const Tensor<fp32_fast_tf32_t> &p);

template
void sgd_step<fp32_fast_fp16_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
               const Tensor<fp32_fast_fp16_t> &grad, const Tensor<fp32_fast_fp16_t> &velocity,
               const Tensor<fp32_fast_fp16_t> &p);

template
void sgd_step<fp32_fast_bf16_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
               const Tensor<fp32_fast_bf16_t> &grad, const Tensor<fp32_fast_bf16_t> &velocity,
               const Tensor<fp32_fast_bf16_t> &p);

template
void sgd_step<fp64_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<fp64_t> &grad, const Tensor<fp64_t> &velocity,
                   const Tensor<fp64_t> &p);

template
void sgd_step<bf16_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<bf16_t> &grad, const Tensor<bf16_t> &velocity,
                   const Tensor<bf16_t> &p);

template
void sgd_step<fp16_t>(Scalar momentum, Scalar lr, Scalar weight_decay,
    const Tensor<fp16_t> &grad, const Tensor<fp16_t> &velocity,
                   const Tensor<fp16_t> &p);
