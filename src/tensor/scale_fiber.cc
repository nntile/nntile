 ...
 *
 * @file src/tensor/scale_fiber.cc
 ...
 * */
{
    ...
    for(Index i = 0; i < dst.grid.nelems; ++i)
    {
        ...
        if(mpi_rank == dst_tile_rank)
        {
            ...
            starpu::scale_fiber.submit<std::tuple<T>>(m, n, k, batch, alpha,
...
template
void scale_fiber_async<fp32_t>(Scalar alpha, const Tensor<fp32_t> &src,
        const Tensor<fp32_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber_async<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src,
        const Tensor<fp32_fast_tf32_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber_async<fp32_fast_fp16_t>(Scalar alpha, const Tensor<fp32_fast_fp16_t> &src,
        const Tensor<fp32_fast_fp16_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber_async<fp32_fast_bf16_t>(Scalar alpha, const Tensor<fp32_fast_bf16_t> &src,
        const Tensor<fp32_fast_bf16_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber_async<fp64_t>(Scalar alpha, const Tensor<fp64_t> &src,
        const Tensor<fp64_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber_async<bf16_t>(Scalar alpha, const Tensor<bf16_t> &src,
        const Tensor<bf16_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber_async<fp16_t>(Scalar alpha, const Tensor<fp16_t> &src,
        const Tensor<fp16_t> &dst, Index axis, Index batch_ndim);

// Explicit instantiation of template
template
void scale_fiber<fp32_t>(Scalar alpha, const Tensor<fp32_t> &src,
        const Tensor<fp32_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src,
        const Tensor<fp32_fast_tf32_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber<fp32_fast_fp16_t>(Scalar alpha, const Tensor<fp32_fast_fp16_t> &src,
        const Tensor<fp32_fast_fp16_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber<fp32_fast_bf16_t>(Scalar alpha, const Tensor<fp32_fast_bf16_t> &src,
        const Tensor<fp32_fast_bf16_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber<fp64_t>(Scalar alpha, const Tensor<fp64_t> &src,
        const Tensor<fp64_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber<bf16_t>(Scalar alpha, const Tensor<bf16_t> &src,
        const Tensor<bf16_t> &dst, Index axis, Index batch_ndim);

template
void scale_fiber<fp16_t>(Scalar alpha, const Tensor<fp16_t> &src,
        const Tensor<fp16_t> &dst, Index axis, Index batch_ndim);
