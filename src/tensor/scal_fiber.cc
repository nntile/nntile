...
template<typename T>
void scal_fiber_async(Scalar alpha, const Tensor<T> &src, const Tensor<T> &dst,
...
template<typename T>
void scal_fiber(Scalar alpha, const Tensor<T> &src, const Tensor<T> &dst,
        Index axis, Index batch_ndim)
//! Tensor<T> addition of a tensor and a broadcasted fiber
 ...
 * */
{
    scal_fiber_async<T>(alpha, src, dst, axis, batch_ndim);
...
template
void scal_fiber_async<fp32_t>(Scalar alpha, const Tensor<fp32_t> &src,
        const Tensor<fp32_t> &dst, Index axis, Index batch_ndim);

template
void scal_fiber_async<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src,
        const Tensor<fp32_fast_tf32_t> &dst, Index axis, Index batch_ndim);

template
void scal_fiber_async<fp64_t>(Scalar alpha, const Tensor<fp64_t> &src,
        const Tensor<fp64_t> &dst, Index axis, Index batch_ndim);

template
void scal_fiber_async<bf16_t>(Scalar alpha, const Tensor<bf16_t> &src,
        const Tensor<bf16_t> &dst, Index axis, Index batch_ndim);

// Explicit instantiation of template
template
void scal_fiber<fp32_t>(Scalar alpha, const Tensor<fp32_t> &src,
        const Tensor<fp32_t> &dst, Index axis, Index batch_ndim);

template
void scal_fiber<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src,
        const Tensor<fp32_fast_tf32_t> &dst, Index axis, Index batch_ndim);

template
void scal_fiber<fp64_t>(Scalar alpha, const Tensor<fp64_t> &src,
        const Tensor<fp64_t> &dst, Index axis, Index batch_ndim);
