 ...
 *
 * @file src/tensor/scal_fiber.cc
 * Tensor wrappers for addition of a tensor and a broadcasted fiber
 *
 * @version 1.1.0
 * */

#include "nntile/tensor/scal_fiber.hh"
...
template<typename T>
void scal_fiber_async(Scalar alpha, const Tensor<T> &src1, Scalar beta,
 ...
 * */
{
    ...
    // Apply per-tile scal_fiber asynchronously as needed
    ...
    for(Index i = 0; i < dst.grid.nelems; ++i)
    {
        ...
        if(mpi_rank == dst_tile_rank)
        {
            ...
            starpu::scal_fiber::submit<T>(m, n, k, batch, alpha,
...
template<typename T>
void scal_fiber(Scalar alpha, const Tensor<T> &src1, Scalar beta,  const Tensor<T> &src2,
        const Tensor<T> &dst, Index axis, Index batch_ndim)
//! Tensor<T> addition of a tensor and a broadcasted fiber
/*! Blocking version of scal_fiber_async<T>.
 * Reshapes input tensor and fiber into 3-dimensional and 1-dimensional arrays
 * and performs the following operations:
 *      dst[i,l,j,b] = beta*src2[i,l,j,b] + alpha*src1[l,b]
 * @param[in] alpha: Scalar factor for src1
 ...
 * */
{
    scal_fiber_async<T>(alpha, src1, beta, src2, dst, axis, batch_ndim);
...
template
void scal_fiber_async<fp32_t>(Scalar alpha, const Tensor<fp32_t> &src1,
        Scalar beta, const Tensor<fp32_t> &src2, const Tensor<fp32_t> &dst,
        Index axis, Index batch_ndim);

template
void scal_fiber_async<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src1,
        Scalar beta, const Tensor<fp32_fast_tf32_t> &src2, const Tensor<fp32_fast_tf32_t> &dst,
        Index axis, Index batch_ndim);

template
void scal_fiber_async<fp64_t>(Scalar alpha, const Tensor<fp64_t> &src1,
        Scalar beta, const Tensor<fp64_t> &src2, const Tensor<fp64_t> &dst,
        Index axis, Index batch_ndim);

template
void scal_fiber_async<bf16_t>(Scalar alpha, const Tensor<bf16_t> &src1,
        Scalar beta, const Tensor<bf16_t> &src2, const Tensor<bf16_t> &dst,
        Index axis, Index batch_ndim);

// Explicit instantiation of template
template
void scal_fiber<fp32_t>(Scalar alpha, const Tensor<fp32_t> &src1,
        Scalar beta, const Tensor<fp32_t> &src2, const Tensor<fp32_t> &dst,
        Index axis, Index batch_ndim);

template
void scal_fiber<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src1,
        Scalar beta, const Tensor<fp32_fast_tf32_t> &src2, const Tensor<fp32_fast_tf32_t> &dst,
        Index axis, Index batch_ndim);

template
void scal_fiber<fp64_t>(Scalar alpha, const Tensor<fp64_t> &src1,
        Scalar beta, const Tensor<fp64_t> &src2, const Tensor<fp64_t> &dst,
        Index axis, Index batch_ndim);

template
void scal_fiber<bf16_t>(Scalar alpha, const Tensor<bf16_t> &src1,
        Scalar beta, const Tensor<bf16_t> &src2, const Tensor<bf16_t> &dst,
        Index axis, Index batch_ndim);
