...
template<typename T>
void add_slice_async(Scalar alpha, const Tensor<T> &src1, Scalar beta,
 ...
 * */
{
    ...
    for(Index i = 0; i < src1.grid.nelems; ++i)
    {
        ...
        for(Index j = 0; j < dst.grid.shape[axis]; ++j)
        {
            ...
            auto src2_tile = src2.get_tile(dst_tile_offset);
            tile::add_slice_async<T>(alpha, src1_tile, beta, src2_tile,
                    dst_tile, axis);
...
template<typename T>
void add_slice(Scalar alpha, const Tensor<T> &src1, Scalar beta, const Tensor<T> &src2,
...
template
void add_slice_async<fp32_t>(Scalar alpha, const Tensor<fp32_t> &src1,
        Scalar beta, const Tensor<fp32_t> &src2, const Tensor<fp32_t> &dst,
        Index axis);

template
void add_slice_async<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src1,
...
template
void add_slice_async<fp16_t>(Scalar alpha, const Tensor<fp16_t> &src1, Scalar beta,
...
template
void add_slice_async<fp32_fast_bf16_t>(Scalar alpha, const Tensor<fp32_fast_bf16_t> &src1, Scalar beta,
...
template
void add_slice<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src1, Scalar beta,
...
template
void add_slice<fp32_fast_bf16_t>(Scalar alpha, const Tensor<fp32_fast_bf16_t> &src1, Scalar beta,
        const Tensor<fp32_fast_bf16_t> &src2, const Tensor<fp32_fast_bf16_t> &dst, Index axis);
...
template
void add_slice<fp16_t>(Scalar alpha, const Tensor<fp16_t> &src1, Scalar beta,
        const Tensor<fp16_t> &src2, const Tensor<fp16_t> &dst, Index axis);
