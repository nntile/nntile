 ...
 *
 * @file src/tensor/muon_step.cc
 * Fuse muon step operation for Tensor<T>
 *
 * @version 1.1.0
 * */

 #include "nntile/tensor/muon_step.hh"
 #include "nntile/starpu/muon_step.hh"
 
 namespace nntile::tensor
 {
 
 //! Asynchronous tensor-wise fuse muon step
 template<typename T>
 void muon_step_async(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
                     ...
                     const Tensor<T> &p)
 {
     ...
     for(Index i = 0; i < p.grid.nelems; ++i)
     {
         ...
         if(mpi_rank == p_tile_rank)
         {
             ...
             starpu::muon_step::submit<T>(num_iter, traits.nelems, beta_1, beta_2, eps, lr, weight_decay,
 ...
 template<typename T>
 void muon_step(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
                const Tensor<T> &grad, const Tensor<T> &first_moment, const Tensor<T> &second_moment,
                const Tensor<T> &p)
 {
     muon_step_async<T>(num_iter, beta_1, beta_2, eps, lr, weight_decay, grad, first_moment, second_moment, p);
 ...
 template
 void muon_step_async<fp32_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
     const Tensor<fp32_t> &grad, const Tensor<fp32_t> &first_moment, const Tensor<fp32_t> &second_moment,
                    const Tensor<fp32_t> &p);
 
 template
 void muon_step_async<fp32_fast_tf32_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
 ...
 template
 void muon_step_async<fp32_fast_fp16_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
 ...
 template
 void muon_step_async<fp32_fast_bf16_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
 ...
 template
 void muon_step_async<fp64_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
     const Tensor<fp64_t> &grad, const Tensor<fp64_t> &first_moment, const Tensor<fp64_t> &second_moment,
                    const Tensor<fp64_t> &p);
 
 template
 void muon_step_async<bf16_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
     const Tensor<bf16_t> &grad, const Tensor<bf16_t> &first_moment, const Tensor<bf16_t> &second_moment,
                    const Tensor<bf16_t> &p);
 
 // Explicit instantiation
 template
 void muon_step<fp32_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
     const Tensor<fp32_t> &grad, const Tensor<fp32_t> &first_moment, const Tensor<fp32_t> &second_moment,
                    const Tensor<fp32_t> &p);
 
 template
 void muon_step<fp32_fast_tf32_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
     const Tensor<fp32_fast_tf32_t> &grad, const Tensor<fp32_fast_tf32_t> &first_moment, const Tensor<fp32_fast_tf32_t> &second_moment,
                    const Tensor<fp32_fast_tf32_t> &p);
 
 template
 void muon_step<fp32_fast_fp16_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
     const Tensor<fp32_fast_fp16_t> &grad, const Tensor<fp32_fast_fp16_t> &first_moment, const Tensor<fp32_fast_fp16_t> &second_moment,
                    const Tensor<fp32_fast_fp16_t> &p);
 
 template
 void muon_step<fp32_fast_bf16_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
     const Tensor<fp32_fast_bf16_t> &grad, const Tensor<fp32_fast_bf16_t> &first_moment, const Tensor<fp32_fast_bf16_t> &second_moment,
     const Tensor<fp32_fast_bf16_t> &p);
 
 template
 void muon_step<fp64_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
     const Tensor<fp64_t> &grad, const Tensor<fp64_t> &first_moment, const Tensor<fp64_t> &second_moment,
                    const Tensor<fp64_t> &p);
 
 template
 void muon_step<bf16_t>(Index num_iter, Scalar beta_1, Scalar beta_2, Scalar eps, Scalar lr, Scalar weight_decay,
     const Tensor<bf16_t> &grad, const Tensor<bf16_t> &first_moment, const Tensor<bf16_t> &second_moment,
                    const Tensor<bf16_t> &p);
