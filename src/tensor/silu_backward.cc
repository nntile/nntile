 ...
 *
 * @file src/tensor/silu_backward.cc
 * Backward SiLU operation for Tensor<T>
 *
 * @version 1.1.0
 * */
...
//! Asynchronous tensor-wise backward silu operation
//
// @param[inout] A: Tensor for the element-wise backward silu operation
template<typename T>
void silu_backward_async(const Tensor<T> &x, const Tensor<T> &dy,
        const Tensor<T> &dx)
{
    // Check shapes
    if(x.shape != dy.shape)
    ...
    for(Index i = 0; i < x.grid.nelems; ++i)
    {
        ...
        if(mpi_rank == exec_rank)
        {
            ...
            starpu::silu_backward.submit<std::tuple<T>>(x_tile_traits.nelems,
...
//! Blocking version of tensor-wise backward silu operation
//
// @param[inout] A: Tensor for the element-wise backward silu operation
template<typename T>
void silu_backward(const Tensor<T> &x, const Tensor<T> &dy,
        const Tensor<T> &dx)
{
...
template
void silu_backward_async<fp32_t>(const Tensor<fp32_t> &x,
        const Tensor<fp32_t> &dy, const Tensor<fp32_t> &dx);

template
void silu_backward_async<fp16_t>(const Tensor<fp16_t> &x,
        const Tensor<fp16_t> &dy, const Tensor<fp16_t> &dx);

template
void silu_backward_async<bf16_t>(const Tensor<bf16_t> &x,
        const Tensor<bf16_t> &dy, const Tensor<bf16_t> &dx);

template
void silu_backward_async<fp32_fast_tf32_t>(const Tensor<fp32_fast_tf32_t> &x,
...
// Explicit instantiation
template
void silu_backward<fp32_t>(const Tensor<fp32_t> &x,
        const Tensor<fp32_t> &dy, const Tensor<fp32_t> &dx);
...
template
void silu_backward<fp16_t>(const Tensor<fp16_t> &x,
        const Tensor<fp16_t> &dy, const Tensor<fp16_t> &dx);

template
void silu_backward<bf16_t>(const Tensor<bf16_t> &x,
        const Tensor<bf16_t> &dy, const Tensor<bf16_t> &dx);
