...
//    }
    ...
    for(Index i = 0; i < maxsumexp.grid.nelems; ++i)
    {
        ...
        std::vector<Index> tmp_tile_index(maxsumexp_tile_index),
            ...
            mask_tile_handle(2);
        auto q_tile_handle = Q.get_tile_handle(k_tile_index);
        mask_tile_handle[1] = maxsumexp_tile_index[1];
        // Launch kernel for each appropriate source tile
        for(Index j = 0; j < tmp.grid.shape[0]; ++j)
        {
            tmp_tile_index[0] = j;
            k_tile_index[1] = j;
            mask_tile_index[0] = j;
            ...
            // Insert tasks
            starpu::gemm::submit<T, T>(TransOp::Trans, TransOp::NoTrans,
            ...
            starpu::mask_scalar::submit<T>(n_seq_tile, n_seq_tile,
                    mask_tile_handle, -std::numeric_limits<T>::infinity(),
                    tmp_tile_handle);
            starpu::maxsumexp::submit<T>(1, n_seq_tile,
                    n_seq_tile*n_batch_tile*n_head_tile, tmp_tile_handle,
                    maxsumexp_tile_handle, redux=0);
        }
    }
}
