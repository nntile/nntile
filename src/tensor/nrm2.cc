...
template
void nrm2_async<fp32_t>(fp32_t alpha, const Tensor<fp32_t> &src, fp32_t beta,
        const Tensor<fp32_t> &dst, const Tensor<fp32_t> &tmp);

template
void nrm2_async<fp64_t>(fp64_t alpha, const Tensor<fp64_t> &src, fp64_t beta,
        const Tensor<fp64_t> &dst, const Tensor<fp64_t> &tmp);

// Explicit instantiation
template
void nrm2<fp32_t>(fp32_t alpha, const Tensor<fp32_t> &src, fp32_t beta,
        const Tensor<fp32_t> &dst, const Tensor<fp32_t> &tmp);

template
void nrm2<fp64_t>(fp64_t alpha, const Tensor<fp64_t> &src, fp64_t beta,
        const Tensor<fp64_t> &dst, const Tensor<fp64_t> &tmp);
