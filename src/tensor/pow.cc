...
template<typename T>
void pow_async(scal_t alpha, scal_t exp, const Tensor<T> &A)
{
...
// @param[inout] A: Tensor for the element-wise power operation
template<typename T>
void pow(scal_t alpha, scal_t exp, const Tensor<T> &A)
{
...
template
void pow_async<fp32_t>(scal_t alpha, scal_t exp, const Tensor<fp32_t> &A);

template
void pow_async<fp64_t>(scal_t alpha, scal_t exp, const Tensor<fp64_t> &A);

// Explicit instantiation
template
void pow<fp32_t>(scal_t alpha, scal_t exp, const Tensor<fp32_t> &A);

template
void pow<fp64_t>(scal_t alpha, scal_t exp, const Tensor<fp64_t> &A);
