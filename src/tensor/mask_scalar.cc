...
template
void mask_scalar_async<fp32_fast_bf16_t>(const Tensor<bool_t> &mask, Scalar val,
        const Tensor<fp32_fast_bf16_t> &A, Index batch_ndim);
...
template
void mask_scalar<fp32_fast_bf16_t>(const Tensor<bool_t> &mask, Scalar val,
        const Tensor<fp32_fast_bf16_t> &A, Index batch_ndim);
