...
//! Multiply along fibers in-place: y = alpha * x * y + beta * y
    ...
    Scalar beta)
{
    ...
    {
        throw std::invalid_argument(
            "multiply_fiber_inplace: tensors must have the same shape");
    }

    OpAttrs attrs = MultiplyFiberAttrs{axis, alpha, beta};
    x.graph().add_op(
