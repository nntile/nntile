...
template<typename T>
void run_adam_step(CompiledGraph& graph, const AdamStepAttrs& attrs,
...
void execute_randn(CompiledGraph& graph, const OpExecutionInfo& op_info)
{
    ...
    switch(dtype)
    {
        ...
        case DataType::FP64:
            ...
            break;
        case DataType::FP16:
            run_randn<nntile::fp16_t>(graph, attrs, x_name);
            break;
        case DataType::BF16:
...
void execute_sgd_step(CompiledGraph& graph, const OpExecutionInfo& op_info)
{
    ...
    const std::string& p_name = op_info.input_names[2];  // p is both input and output
...
void execute_adam_step(CompiledGraph& graph, const OpExecutionInfo& op_info)
{
    ...
    const std::string& p_name = op_info.input_names[3];  // p is both input and output
    DataType dtype = graph.get_dtype(grad_name);

    switch(dtype)
    {
        case DataType::FP32:
            run_adam_step<nntile::fp32_t>(graph, attrs, grad_name, first_moment_name, second_moment_name, p_name);
            break;
        case DataType::FP32_FAST_TF32:
            run_adam_step<nntile::fp32_fast_tf32_t>(graph, attrs, grad_name, first_moment_name, second_moment_name, p_name);
            break;
        case DataType::FP32_FAST_FP16:
            run_adam_step<nntile::fp32_fast_fp16_t>(graph, attrs, grad_name, first_moment_name, second_moment_name, p_name);
            break;
        case DataType::FP32_FAST_BF16:
            run_adam_step<nntile::fp32_fast_bf16_t>(graph, attrs, grad_name, first_moment_name, second_moment_name, p_name);
            break;
        case DataType::FP64:
            run_adam_step<nntile::fp64_t>(graph, attrs, grad_name, first_moment_name, second_moment_name, p_name);
            break;
        case DataType::FP16:
            run_adam_step<nntile::fp16_t>(graph, attrs, grad_name, first_moment_name, second_moment_name, p_name);
            break;
        case DataType::BF16:
            run_adam_step<nntile::bf16_t>(graph, attrs, grad_name, first_moment_name, second_moment_name, p_name);
            break;
        case DataType::INT64:
        case DataType::INT32:
        case DataType::BOOL:
            throw std::runtime_error(
                std::string(dtype_to_string(dtype)) +
                " data type not supported for adam_step operation");
        default:
            throw std::runtime_error("Unsupported data type for adam_step");
    }
}

//! Execute adamw_step operation
