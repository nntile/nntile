...
NNGraph::TensorNode* NNMultiplyOp::forward(const std::string& output_name)
{
    ...
    TensorGraph::TensorNode* z_data = graph::tensor::multiply(
        x->data(), y->data(), output_name, alpha);
...
void NNMultiplyOp::backward() const
{
    ...
    if(x != nullptr && x->requires_grad())
    {
        ...
        TensorGraph::TensorNode* grad_x_buf = graph::tensor::multiply(
            grad_out->data(), y->data(),
            out->name() + "_grad_x_buf", alpha);
    ...
    if(y != nullptr && y->requires_grad())
    {
        ...
        Scalar grad_beta = is_first ? grad_overwrite : grad_accumulate;
        TensorGraph::TensorNode* grad_y_buf = graph::tensor::multiply(
            grad_out->data(), x->data(),
            out->name() + "_grad_y_buf", alpha);
