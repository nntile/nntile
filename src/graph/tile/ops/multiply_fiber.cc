...
template<typename T>
void run(
    Runtime& rt, Scalar a, TileGraph::TileNode* t1, TileGraph::TileNode* t2, TileGraph::TileNode* d, Index ax)
{
    nntile::core::tile::multiply_fiber<T>(a, rt.get_tile<T>(t1), rt.get_tile<T>(t2), rt.get_tile<T>(d), ax);
}
} // namespace
...
void TileMultiplyFiberOp::execute(Runtime& runtime) const
{
    ...
    {
        case DataType::FP32:
            run<nntile::core::fp32_t>(runtime, alpha, s1, s2, dst, axis);
            break;
        case DataType::FP32_FAST_TF32:
            run<nntile::core::fp32_fast_tf32_t>(runtime, alpha, s1, s2, dst, axis);
            break;
        case DataType::FP32_FAST_FP16:
            run<nntile::core::fp32_fast_fp16_t>(runtime, alpha, s1, s2, dst, axis);
            break;
        case DataType::FP32_FAST_BF16:
            run<nntile::core::fp32_fast_bf16_t>(runtime, alpha, s1, s2, dst, axis);
            break;
        case DataType::FP64:
            run<nntile::core::fp64_t>(runtime, alpha, s1, s2, dst, axis);
            break;
        case DataType::FP16:
            run<nntile::core::fp16_t>(runtime, alpha, s1, s2, dst, axis);
            break;
        case DataType::BF16:
            run<nntile::core::bf16_t>(runtime, alpha, s1, s2, dst, axis);
            break;
        case DataType::INT64:
        case DataType::BOOL:
