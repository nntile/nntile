...
def grayscale_image_patching(image, patch_size):
    ...
    for i in range(n_patches):
        ...
        vect_patch = image[0, x * patch_size: (x+1) * patch_size , y * patch_size: (y+1) * patch_size].flatten()
        patched_batch[i, :] = vect_patch
    return patched_batch


def cifar_data_loader_to_nntile(data_set, label_set, batch_input, batch_output, trns, batch_size, minibatch_size, patch_size, next_tag):
    ...
    for i in range(n_batches):
        ...
        for j in range(n_minibatches):
            for k in range(minibatch_size):
                tmp_data_tensor[:, k, :] = color_image_patching(trns(data_set[i * batch_size + j * minibatch_size + k, :, :, :]), patch_size)
...
def mnist_data_loader_to_nntile(data_set, label_set, batch_input, batch_output, trns, batch_size, minibatch_size, patch_size, next_tag):
    ...
    for i in range(n_batches):
        ...
        for j in range(n_minibatches):
            for k in range(minibatch_size):
                tmp_data_tensor[:, k, :] = grayscale_image_patching(trns(data_set[i * batch_size + j * minibatch_size + k, :, :]), patch_size)
                tmp_label_tensor[k] = label_set[i * batch_size + j * minibatch_size + k]
            x = nntile.tensor.Tensor_fp32(x_traits, x_distr, next_tag)
            next_tag = x.next_tag
            x.from_array(np.asfortranarray(tmp_data_tensor.numpy()))
...
def cifar_data_loader_to_tensor(data_set, label_set, trns, batch_size, minibatch_size, patch_size):
    ...
    for i in range(n_batches):
        for j in range(n_minibatches):
            for k in range(minibatch_size):
                train_tensor[i, j, :, k, :] = color_image_patching(trns(data_set[i * batch_size + j * minibatch_size + k, :, :, :]), patch_size)
                label_tensor[i, j, k] = label_set[i * batch_size + j * minibatch_size + k]
    return train_tensor, label_tensor


def mnist_data_loader_to_tensor(data_set, label_set, trns, batch_size, minibatch_size, patch_size):
    total_len, h, w = data_set.shape
    n_batches = total_len // batch_size
    n_minibatches = int(batch_size / minibatch_size)
    ...
    for i in range(n_batches):
        for j in range(n_minibatches):
            for k in range(minibatch_size):
                train_tensor[i, j, :, k, :] = grayscale_image_patching(trns(data_set[i * batch_size + j * minibatch_size + k, :, :]), patch_size)
                label_tensor[i, j, k] = label_set[i * batch_size + j * minibatch_size + k]
    return train_tensor, label_tensor
