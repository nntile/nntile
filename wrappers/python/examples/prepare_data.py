...
if args.tokenizer == "llama_1.3b":
    from transformers import LlamaTokenizerFast
    tokenizer = LlamaTokenizerFast.from_pretrained("kimihailv/llama-1.3b",
...
filename = Path(args.dataset_path) / args.dataset / 'train.bin'
if not (Path(args.dataset_path) / args.dataset).exists():
    Path.mkdir(Path(args.dataset_path) / args.dataset)
dtype = np.uint16
