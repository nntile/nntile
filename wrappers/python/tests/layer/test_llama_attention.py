...
TEST_PARAMS = \
[ \
    LlamaAttentionTestParams(n_emb=128, n_emb_tile=32, n_seq=64, \
        n_seq_tile=16, n_batch=4, n_batch_tile=1, n_head=16, n_head_tile=8, \
        n_head_kv=4, dtype=np.dtypes.Float32DType(), bias=True), \
    LlamaAttentionTestParams(n_emb=128, n_emb_tile=32, n_seq=64, \
        n_seq_tile=16, n_batch=4, n_batch_tile=1, n_head=16, n_head_tile=8, \
        n_head_kv=4, dtype=np.dtypes.Float32DType(), bias=False), \
    LlamaAttentionTestParams(n_emb=128, n_emb_tile=128, n_seq=64, \
...
class TestLlamaAttention:
    ...
    def test_forward_backward(self, starpu_simple, torch_rng, \
        ...
        nntile_layer.y.unregister()
        torch.testing.assert_close(y, y_nntile)
        torch.testing.assert_close(torch_layer.q_proj.weight, \
                torch_layer_other.q_proj.weight)
        torch.testing.assert_close(torch_layer.k_proj.weight, \
                torch_layer_other.k_proj.weight)
        torch.testing.assert_close(torch_layer.v_proj.weight, \
                torch_layer_other.v_proj.weight)
        torch.testing.assert_close(torch_layer.o_proj.weight, \
                torch_layer_other.o_proj.weight)
        if params.bias:
            torch.testing.assert_close(torch_layer.q_proj.bias, \
                    torch_layer_other.q_proj.bias)
            torch.testing.assert_close(torch_layer.k_proj.bias, \
                    torch_layer_other.k_proj.bias)
            torch.testing.assert_close(torch_layer.v_proj.bias, \
                    torch_layer_other.v_proj.bias)
            torch.testing.assert_close(torch_layer.o_proj.bias, \
                    torch_layer_other.o_proj.bias)
