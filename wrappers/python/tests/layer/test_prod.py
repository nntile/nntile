...
#
# @file wrappers/python/tests/layer/test_prod.py
# Test for nntile.layer.prod
#
# @version 1.0.0
...
class ToyFC_Torch(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super().__init__()
        self.lin1 = nn.Linear(input_dim, hidden_dim, bias=False)
        self.act = nn.ReLU()
        self.lin2 = nn.Linear(input_dim, hidden_dim, bias=False)
...
torch_model = ToyFC_Torch(n, hidden_dim)
...
class ToyFC(BaseModel):
    ...
    def __init__(self, x: TensorMoments, hidden_dim: int, next_tag: int):
        ...
        new_layer, next_tag = Linear.generate_simple(
                    x, "L", notrans, 1, [hidden_dim],
                    [hidden_dim], next_tag, bias=False)
        layers.append(new_layer)
        activations.extend(new_layer.activations_output)
        # Prod operation
        new_layer, next_tag = Prod.generate_simple(activations[-2],
                                                   activations[-1],
                                                   next_tag)
        layers.append(new_layer)
        activations.extend(new_layer.activations_output)
    ...
    def from_torch(torch_model, input_moment, hidden_dim, next_tag: int):
        '''
        torch_mlp is PyTorch MLP where all intermediate dimensions are
        the same and no biases in linear layers
        '''
        print("Call from torch static method")
        nntile_model = ToyFC(input_moment, hidden_dim, next_tag)
        for p, p_torch in zip(nntile_model.parameters,
...
nntile_model, next_tag = ToyFC.from_torch(torch_model, nntile_input_moment,
                                                         hidden_dim, next_tag)
...
for i, (p_nntile, p_torch) in enumerate(zip(nntile_model.parameters,
    ...
    rel_error = np.linalg.norm(p_np - p_torch_np.T, "fro") / \
