...
def helper_l(dtype: np.dtype):
    ...
    np_Y2 = np.zeros(layer.y.value.shape, dtype=np.float32, order="F")
    layer.y.value.to_array(np_Y2)
    fro_loss, next_tag = nntile.loss.Frob.generate_simple(layer.y, next_tag)
    np_zero = np.zeros(layer.y.value.shape, dtype=np.float32, order="F")
    fro_loss.y.from_array(np_zero)
    fro_loss.calc_async()
    print("Loss computed")

    y_grad_np = np.zeros(layer.y.grad.shape, order="F", dtype=np.float32)
    layer.y.grad.to_array(y_grad_np)
    print(y_grad_np)
