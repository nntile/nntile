...
def generate_inputs(params: GPTNeoTestParams,
    ...
    torch_model = GPTNeoModelTorch(torch_config)
...
def test_bench_gptneo_forward_async(context_cuda, benchmark_model):
    params = single_tile
    dtype = 'fp32'
    attn_pattern = ["local"]
    pattern_mult = 1
...
@pytest.mark.benchmark
def test_bench_gptneo_backward_async(context_cuda, benchmark_model):
    params = single_tile
    dtype = 'fp32'
    attn_pattern = ["local"]
    pattern_mult = 1
