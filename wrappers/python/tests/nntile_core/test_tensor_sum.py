...
#
# @file wrappers/python/tests/nntile_core/test_tensor_sum.py
# Test for tensor::sum<T> Python wrapper
#
# @version 1.1.0
...
single_tile = SumTestParams(
    shape=[5],
    shape_tile=[5]
)

multiple_tiles = SumTestParams(
    shape=[10, 20],
    shape_tile=[5, 10]
)
...
def get_ref_value(alpha, src, beta, dst_init):
    # Compute sum of all elements in src
    return alpha * np.sum(src.flatten()) + beta * dst_init
...
def test_sum_async(context, dtype, params, alpha, beta):
    ...
    # actual calculations
    sum_func[dtype](alpha, src, beta, dst)
