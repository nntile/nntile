...
scale_slice_async = {
    np.float32: nntile.nntile_core.tensor.scale_slice_async_fp32,
    np.float64: nntile.nntile_core.tensor.scale_slice_async_fp64
}
...
@pytest.mark.parametrize('dtype', [np.float32, np.float64])
def test_scale_slice_async(context, dtype):
    # Describe single-tile tensor
    dst_shape = [2, 3, 4]
    src_shape = []
    ndim = len(dst_shape)
    ...
    for i in range(ndim):
        ...
        scale_slice_async[dtype](alpha, src[i], dst, i)
        nntile.starpu.wait_for_all()
        dst.to_array(np_dst)
        # Test add_slice_inplace with beta=0 for reference
        add_slice_inplace[dtype](alpha, src[i], 0.0, dst_ref, i)
