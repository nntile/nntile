...
# Public entry point for StarPU **graph batch capture** (recording_begin / recording_end).
# Works with the SGOC loadable policy (``STARPU_SCHED=sgoc`` and
# ``STARPU_SCHED_LIB`` pointing at ``libgraph_sgoc_sched.so`` from starpu-sched ``new_sched``).
# The DSO exports ``starpu_graph_sched_graph_recording_begin`` / ``end``.
# For other schedulers (e.g. dmdasd), calls are no-ops so training can always bracket batches.
#
# @version 1.1.0
...
def _env_matches_sgoc_scheduler_dso() -> bool:
    """True when env points at the SGOC scheduler DSO for recording begin/end."""
    sched = os.environ.get("STARPU_SCHED", "").strip()
    lib = os.environ.get("STARPU_SCHED_LIB", "").strip()
    if not lib:
        return False
    resolved = os.path.abspath(lib)
    return sched == EXPECTED_SGOC_SCHEDULER_NAME and _sched_lib_path_matches_sgoc(
        resolved
    )


# True when graph_recording_begin/end will call into STARPU_SCHED_LIB (SGOC DSO).
...
if not GRAPH_CAPTURE_SCHED_ACTIVE:
    ...
    if _sched == EXPECTED_SGOC_SCHEDULER_NAME and _lib:
        if not _sched_lib_path_matches_sgoc(os.path.abspath(_lib)):
            warnings.warn(
                f"STARPU_SCHED is {EXPECTED_SGOC_SCHEDULER_NAME!r} but STARPU_SCHED_LIB "
                f"is not {EXPECTED_SGOC_SCHED_LIB_BASENAME}*: {_lib!r}; "
                "nntile.graph_capture_sched.GRAPH_CAPTURE_SCHED_ACTIVE is False",
                RuntimeWarning,
                stacklevel=2,
            )
    elif _lib and _sched_lib_path_matches_sgoc(os.path.abspath(_lib)):
        warnings.warn(
            f"STARPU_SCHED_LIB looks like SGOC ({_lib!r}) but "
            f"STARPU_SCHED is {_sched!r}, not {EXPECTED_SGOC_SCHEDULER_NAME!r}; "
            "nntile.graph_capture_sched.GRAPH_CAPTURE_SCHED_ACTIVE is False",
            RuntimeWarning,
            stacklevel=2,
        )
