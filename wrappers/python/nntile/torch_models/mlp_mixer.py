...
def image_patching_rgb(image_batch, patch_size):
    ...
    for batch_iter in range(b):
        for i in range(n_patches):
            ...
            for clr in range(c):
                vect_patch = image_batch[batch_iter, x * patch_size: (x+1) * patch_size , y * patch_size: (y+1) * patch_size, clr].flatten()
...
def image_patching(image_batch, patch_size):
    ...
    if h % patch_size != 0 or w % patch_size != 0:
            raise ValueError("patch size should be divisor of image height and width")
    n_patches = int(h * w / (patch_size ** 2))
    ...
    for i in range(n_patches):
        ...
        y = i % n_y
        for j in range(b):
            patched_batch[i, j, :] = image_batch[x * patch_size: (x+1) * patch_size, j , y * patch_size: (y+1) * patch_size].reshape(1, 1, patch_size ** 2)
...
class MlpMixer(nn.Module):
    def __init__(self, channel_dim: int, init_patch_dim: int, patch_dim: int, num_mixer_layers: int, n_classes: int):
        ...
        mixer_layer_blocks = [Mixer(self.channel_dim,self.projected_patch_dim) for _ in range(self.num_mixer_layers)]
        self.mixer_sequence =  nn.Sequential(nn.Linear(self.init_patch_dim, self.projected_patch_dim, bias = False),
                                              *mixer_layer_blocks)
        self.classification = nn.Linear(self.projected_patch_dim, n_classes, bias = False)
