...
class RobertaForMaskedLM(BaseModel):
    ...
    def from_pretrained(
        ...
        """Load a HuggingFace RobertaForMaskedLM checkpoint and convert
        to NNTile. Output at self.activations[-1] is per-token vocab
        logits, shape (vocab_size, seq_len, batch_size)."""
        torch_model = RobertaForMaskedLM_torch.from_pretrained(
            model_name, cache_dir=cache_dir, local_files_only=False,
        )
        torch_model.eval()
