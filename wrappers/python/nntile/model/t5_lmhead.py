...
class T5ClassificationHead(BaseModel):
    ...
    def from_torch(
        ...
        # First dense layer
        t5_head_nntile.dense.w.value.from_array(
            torch_head.dense.weight.cpu().detach().numpy()
        )
        t5_head_nntile.dense.b.value.from_array(
            torch_head.dense.bias.cpu().detach().numpy()
        )

        # Output projection layer
        t5_head_nntile.out_proj.w.value.from_array(
            torch_head.out_proj.weight.cpu().detach().numpy()
        )
        t5_head_nntile.out_proj.b.value.from_array(
    ...
    def to_torch(self):
        ...
        torch_config = T5ConfigTorch(
            d_model=self.config.d_model, num_labels=self.num_labels
        )
        ...
        torch_head.dense.weight.data = torch.tensor(
            to_numpy(self.dense.w.value), requires_grad=True
        )
        torch_head.dense.bias.data = torch.tensor(
            to_numpy(self.dense.b.value), requires_grad=True
        )
        torch_head.out_proj.weight.data = torch.tensor(
            to_numpy(self.out_proj.w.value), requires_grad=True
        )
        torch_head.out_proj.bias.data = torch.tensor(
            to_numpy(self.out_proj.b.value), requires_grad=True
        )
    ...
    def to_torch_with_grads(self):
        ...
        torch_head.out_proj.weight.grad = torch.tensor(
            nntc.to_numpy(self.out_proj.w.grad)
        )
        torch_head.out_proj.bias.grad = torch.tensor(
            nntc.to_numpy(self.out_proj.b.grad)
        )
