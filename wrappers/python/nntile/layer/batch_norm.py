...
class BatchNorm2d(BaseLayer):
    def __init__(
        ...
        self.inv_std = empty(self.x.value.shape[2:3], dtype=self.dtype)
        ...
        self.tmp_buff_channels = empty(self.x.value.shape[2:3], dtype=self.dtype)
    ...
    def generate_simple(cls, x, eps: float = 1e-05, redux=False):
        ...
        bias = TensorMoments(
            empty([n_channels], dtype=type(x.value)),
            empty([n_channels], dtype=type(x.value)),
            grad_required=True,
        )
    ...
    def _normalize_forward(self):
        ...
        sum_fiber_async(1.0 / self.numel_in_channel, self.x.value, 0.0, x_mean,
                2, 0)
        self.x.value.wont_use()

        # X_res = X - mean
        add_fiber_async(-1.0, x_mean, 1.0, self.x_normalized, 2, 0)
        ...
        # X_res = X_res/std
        prod_fiber_async(self.inv_std, 1.0, self.x_normalized, 2)
    ...
    def _learnable_transform_forward(self):
        ...
        prod_fiber_async(self.weight.value, 1.0, self.y.value, 2)
        self.weight.value.wont_use()
        add_fiber_async(1.0, self.bias.value, 1.0, self.y.value, 2, 0)
    ...
    def _compute_grad_normalized_input_over_x(self, grad_nnt):
        ...
        sum_fiber_async(1.0 / self.numel_in_channel, grad_nnt, 0.0, grad_mean,
                2, 0)

        # grad computing over mean
        add_fiber_async(-1.0, grad_mean, 1.0, grad_nnt, 2, 0)
    ...
    def _compute_grad_inv_std_over_x(self, grad_nnt):
        ...
        sum_fiber_async(1.0, grad_nnt, 0.0, inv_denom_grad, 2, 0)
        ...
        x_normalized_grad = grad_nnt  # empty(self.x.value.shape)
        mean_grad = self.tmp_buff_channels  #  empty([self.n_channels])
        ...
        prod_fiber_async(xvar_grad, 1.0, x_normalized_grad, 2)
    ...
    def _normalize_backward(self):
        ...
        prod_fiber_async(inv_denominator_ref, 1.0, nominator_grad, 2)
    ...
    def _learnable_transform_backward(self):
        # bias_grad = grad
        sum_fiber_async(1.0, self.y.grad, 0.0, self.bias.grad, 2, 0)
        ...
        prod_fiber_async(self.weight.value, 1.0, self.y.grad, 2)
        self.weight.value.wont_use()
