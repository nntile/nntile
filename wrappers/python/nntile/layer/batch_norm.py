...
class BatchNorm2d(BaseLayer):
    def __init__(self, x: TensorMoments, weight: Tensor, bias: Tensor,
        ...
        self.std_tmp_2dim = zeros(
            self.x.shape[1:2] + self.x.shape[3:], dtype=type(x.value)
        )
    ...
    def generate_simple(cls, x, eps: float = 1e-05, redux=False):
        ...
        return BatchNorm2d(
            x, weight, bias, x_res, x_mean, inv_std, eps=eps, redux=redux
        )
    ...
    def backward_async(self):
        ...
        inv_denominator_grad_x = self._compute_grad_inv_std_over_x(
            self.x, inv_denominator_grad
        )
