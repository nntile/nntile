...
class LlamaAttention(BaseLayer):
    ...
    def __init__(self, x_q: TensorMoments, x_k: TensorMoments, \
            ...
            k_transposed: TensorMoments, k: TensorMoments, k_rep: TensorMoments, \
            v_transposed: TensorMoments, v: TensorMoments, v_rep: TensorMoments, \
    ...
    def backward_async(self):
        ...
        if self.a.grad_required:
            ...
            gemm_async(1.0, trans, self.v_rep.value, notrans, \
                        self.b.grad, 0.0, self.a.grad, 1, 3, redux=self.redux)
        ...
        if self.v_rep.grad_required:
            ...
            gemm_async(1.0, notrans, self.b.grad, trans, \
                        self.a.value, 0.0, self.v_rep.grad, 1, 3, \
                        redux=self.redux)
        # dB can be deleted
        ...
        if self.k_rep.grad_required:
            # dK_rep = 1.0/sqrt(head_size) * einsum('jmlbi,kmlbi->jklbi', Q, dA)
            gemm_async(1.0/self.head_size**0.5, notrans, self.q.value, \
                        trans, self.a.grad, 0.0, self.k_rep.grad, 1, 3, \
                        redux=self.redux)
        # Q can be deleted
        self.q.value.invalidate_submit()
        if self.q.grad_required:
            # dQ = 1.0/sqrt(head_size) * einsum('jklbi,kmlbi->jmlbi', K_rep, dA)
            gemm_async(1.0/self.head_size**0.5, notrans, self.k_rep.value, \
                        notrans, self.a.grad, 0.0, self.q.grad, 1, 3, \
                        redux=self.redux)
        ...
        if self.x_q.grad_required:
            ...
            gemm_async(1.0, trans, self.w_q.value, notrans, \
                        self.q_transposed.grad, 1.0, self.x_q.grad, 3, 0, \
                        redux=self.redux)
