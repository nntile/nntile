...
def test_repeat_1d_matches_cpu():
    ...
    x = x_cpu.to("nntile")
    y = x.repeat(2)
    assert torch.allclose(y.cpu(), x_cpu.repeat(2))
...
def test_repeat_1d_padded_repeats_matches_cpu():
    ...
    x = x_cpu.to("nntile")
    y = x.repeat(2, 3)
    assert torch.allclose(y.cpu(), x_cpu.repeat(2, 3))
...
def test_repeat_identity_matches_cpu():
    ...
    x = x_cpu.to("nntile")
    y = x.repeat(1, 1)
    assert torch.allclose(y.cpu(), x_cpu.repeat(1, 1))
...
def test_repeat_leading_pad_matches_cpu():
    ...
    x = x_cpu.to("nntile")
    y = x.repeat(1, 2, 1)
    assert torch.allclose(y.cpu(), x_cpu.repeat(1, 2, 1))
