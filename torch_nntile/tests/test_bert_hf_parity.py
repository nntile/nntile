...
# BERT layer + full-model parity vs HuggingFace BertForMaskedLM.
...
def _make_models(hf_cfg: HfBertConfig):
    torch.manual_seed(0)
    hf = BertForMaskedLM(hf_cfg).eval().float()
    local_cfg = bert_config_from_hf(hf_cfg)
    minimal = BertMlm(local_cfg).eval().float()
    load_hf_into_bert_mlm(minimal, hf)
...
def test_bert_intermediate_forward_matches_hf(tiny_hf_config):
    torch.manual_seed(2)
    hf_inter = HfIntermediate(tiny_hf_config).eval().float()
    local = BertIntermediate(bert_config_from_hf(tiny_hf_config)).eval().float()
...
def test_bert_attention_forward_matches_hf(tiny_hf_config):
    torch.manual_seed(3)
...
def test_bert_layer_forward_matches_hf(tiny_hf_config):
    torch.manual_seed(4)
...
def test_bert_mlm_forward_matches_hf(tiny_hf_config):
...
def test_bert_mlm_backward_matches_hf(tiny_hf_config):
    hf, minimal = _make_models(tiny_hf_config)
