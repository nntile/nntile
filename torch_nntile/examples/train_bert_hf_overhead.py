...
#
# @file torch_nntile/examples/train_bert_hf_overhead.py
# Train stock HuggingFace BertForMaskedLM (cpu, cuda, or nntile).

"""Train HuggingFace BERT on a tiny synthetic token stream.
...
Attention: stock HF BERT uses **eager** ``BertSelfAttention`` (manual
...
Examples::
    ...
    python torch_nntile/examples/train_bert_hf_overhead.py train \\
        --device cuda --disable-tf32 --seed 42 \\
        --config torch_nntile/examples/overhead_bert/bert_xs.json \\
        --seq-len 768 --batch-size 1 --max-sequences 10 --epochs 1 \\
        --output-dir /tmp/bert_overhead_s_cuda

Overhead ladder: ``torch_nntile/examples/overhead_bert/``.
"""
...
def _default_config_path() -> Path:
    return Path(__file__).resolve().parent / "overhead_bert/bert_xs.json"


def _attn_implementation_for_device(device: str) -> str:
    """Stock HF BERT: eager attention for parity with nntile."""
    del device
    return "eager"
...
def ensure_seq_len_fits_positions(config: BertConfig, seq_len: int) -> None:
    if seq_len < 1:
...
def build_model(config: BertConfig, seed: int) -> BertForMaskedLM:
    set_seed(seed)
...
def load_train_state(
    ...
    if args.checkpoint:
        ...
        config = BertConfig.from_dict(ckpt["config"])
        config._attn_implementation = attn_impl
        model = BertForMaskedLM(config).float()
...
def mlm_loss(model: BertForMaskedLM, batch: BatchDict) -> torch.Tensor:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train = sub.add_parser(
        "train",
        help="Train BERT HF on a tiny synthetic token stream",
    )
    train.add_argument(
        "--device",
        required=True,
