...
#
# @file torch_nntile/examples/train_t5_hf_overhead.py
# Train stock HuggingFace T5ForConditionalGeneration (cpu, cuda, or nntile).

"""Train HuggingFace T5 on a tiny synthetic encoder-decoder token stream.
...
Attention: stock HF T5 uses **eager** attention (manual ``matmul`` / ``softmax``;
no ``sdpa`` backend in transformers 4.52). Both ``cuda`` and ``nntile`` use
``attn_implementation="eager"``. ``use_cache=False`` for training.
``--disable-tf32`` only disables TF32 GEMM / cuDNN matmul on CUDA.
...
flight when the clock starts. Loss uses stock T5 ``labels`` CE via
``t5_ce_loss`` (same on CUDA and nntile). On nntile,
...
Examples::
    ...
    python torch_nntile/examples/train_t5_hf_overhead.py train \\
        --device cuda --disable-tf32 --seed 42 \\
        --config torch_nntile/examples/overhead_t5/t5_xs.json \\
        --seq-len 768 --batch-size 1 --max-sequences 10 --epochs 1 \\
        --output-dir /tmp/t5_overhead_s_cuda

Overhead ladder: ``torch_nntile/examples/overhead_t5/``.
"""
...
from hf_tiny_train_common import make_encoder_decoder_batch, t5_ce_loss
from transformers import T5Config, T5ForConditionalGeneration


def _default_config_path() -> Path:
    return Path(__file__).resolve().parent / "overhead_t5/t5_xs.json"


def _attn_implementation_for_device(device: str) -> str:
    """Stock HF T5: eager attention for parity with nntile."""
    del device
    return "eager"
...
def load_t5_config(
    ...
    *,
    ...
    fields.setdefault("dropout_rate", 0.0)
    fields["use_cache"] = False
    config = T5Config(**fields)
...
def ensure_seq_len_fits_config(config: T5Config, seq_len: int) -> None:
    if seq_len < 1:
        raise SystemExit(f"--seq-len must be >= 1, got {seq_len}")
    max_dist = int(config.relative_attention_max_distance)
    if seq_len > max_dist:
        raise SystemExit(
            f"--seq-len={seq_len} exceeds config "
            f"relative_attention_max_distance={max_dist}."
        )
...
def build_train_batches(
    ...
    ensure_seq_len_fits_config(config, args.seq_len)
...
def build_model(config: T5Config, seed: int) -> T5ForConditionalGeneration:
    set_seed(seed)
...
def count_batch_elems(
    ...
    for epoch_data in epoch_batches:
        for batch in epoch_data:
            ...
            n_inputs += int(batch["decoder_input_ids"].numel())
...
def load_train_state(
    ...
    if args.checkpoint:
        ...
        config = T5Config.from_dict(ckpt["config"])
        config._attn_implementation = attn_impl
        model = T5ForConditionalGeneration(config).float()
...
def t5_loss(model: T5ForConditionalGeneration, batch: BatchDict) -> torch.Tensor:
    """Encoder-decoder CE via stock T5 ``labels`` path."""
    return t5_ce_loss(model, batch)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train = sub.add_parser(
        "train",
        help="Train T5 HF on a tiny synthetic encoder-decoder token stream",
    )
    train.add_argument(
        "--device",
        required=True,
    ...
    train.add_argument(
        ...
        default=32,
        help=(
            "MLM sequence length (input_ids and labels)"
        ),
    )
    ...
    train.add_argument(
        ...
        default=64,
        help=(
            "Number of MLM training steps (default 64)"
        ),
    )
    train.add_argument(
        "--no-shuffle",
