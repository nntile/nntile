...
# Train stock HuggingFace GPTNeoForCausalLM (cpu, cuda, or nntile).

"""Train HuggingFace GPT-Neo on a tiny synthetic token stream.
...
Attention: stock HF GPT-Neo uses **eager** ``GPTNeoSelfAttention`` (manual
...
flight when the clock starts. Stock HuggingFace GPT-Neo builds
...
Examples::
    ...
    python torch_nntile/examples/train_gpt_neo_hf.py train \\
        ...
        --config torch_nntile/examples/overhead_gpt_neo/gpt_neo_s.json \\
        --seq-len 1024 --batch-size 1 --max-sequences 10 --epochs 1 \\
        --output-dir /tmp/gpt_neo_overhead_s_cuda

Overhead ladder: ``torch_nntile/examples/overhead_gpt_neo/``.
"""
...
def _attn_implementation_for_device(device: str) -> str:
    """Stock HF GPT-Neo: eager attention (no sdpa class in transformers 4.52)."""
    del device
    return "eager"


def load_gpt_neo_config(
    path: Path,
    *,
...
def ensure_seq_len_fits_positions(config: GPTNeoConfig, seq_len: int) -> None:
...
def build_model(config: GPTNeoConfig, seed: int) -> GPTNeoForCausalLM:
    set_seed(seed)
    config.use_cache = False
...
def load_train_state(
    ...
    if args.checkpoint:
        ...
        config = GPTNeoConfig.from_dict(ckpt["config"])
        config._attn_implementation = attn_impl
        config.use_cache = False
        model = GPTNeoForCausalLM(config).float()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train = sub.add_parser(
        "train",
        help="Train GPT-Neo HF on a tiny synthetic token stream",
    )
    train.add_argument(
        "--device",
        required=True,
