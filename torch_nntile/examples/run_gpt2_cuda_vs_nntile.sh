...
#
# @file torch_nntile/examples/run_gpt2_cuda_vs_nntile.sh
# Train HF GPT-2 on CUDA (train_gpt2_hf.py) and NNTile GPT2LMHead
# (train_gpt2.py) from the same seed, then compare HF-format checkpoints.
#
...
#   ./torch_nntile/examples/run_gpt2_cuda_vs_nntile.sh
#
# Optional environment variables:
#   SEED            Init seed (default: 42)
#   EPOCHS          Epochs per device run (default: 2)
#   OUTPUT_ROOT     Run directory (default: /tmp/gpt2_cuda_vs_nntile)
...
TRAIN_HF_PY="${SCRIPT_DIR}/train_gpt2_hf.py"
TRAIN_NNTILE_PY="${SCRIPT_DIR}/train_gpt2.py"
CONFIG_JSON="${SCRIPT_DIR}/gpt2_hf_tiny_config.json"

SEED="${SEED:-42}"
EPOCHS="${EPOCHS:-2}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/tmp/gpt2_cuda_vs_nntile}"
SEQ_LEN="${SEQ_LEN:-32}"
...
echo "=== GPT-2: HF CUDA vs NNTile GPT2LMHead (seed=${SEED}) ==="
...
echo "--- Train HF GPT-2 device=cuda from scratch ---"
"${PYTHON}" "${TRAIN_HF_PY}" train \
    --device cuda \
    --disable-tf32 \
    --output-dir "${CUDA_DIR}" \
    "${COMMON_ARGS[@]}"
...
echo "--- Train NNTile GPT2LMHead from scratch (same seed) ---"
# shellcheck disable=SC2086
"${PYTHON}" "${TRAIN_NNTILE_PY}" train \
    --output-dir "${NNTILE_DIR}" \
    "${COMMON_ARGS[@]}" \
    ${EXTRA_NNTILE_ARGS}
echo ""

echo "--- Compare HF-format checkpoints (relative Frobenius norms) ---"
"${PYTHON}" "${TRAIN_NNTILE_PY}" compare \
    --checkpoint-a "${CUDA_CKPT}" \
    --checkpoint-b "${NNTILE_CKPT}"

echo ""
echo "Done."
echo "  CUDA (HF) checkpoint:    ${CUDA_CKPT}"
echo "  nntile (minimal) checkpoint: ${NNTILE_CKPT}"
