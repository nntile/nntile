...
# Train stock HuggingFace GPTNeoXForCausalLM (cpu, cuda, or nntile).

"""Train HuggingFace GPT-NeoX on a tiny synthetic token stream.
...
flight when the clock starts. Stock HuggingFace GPT-NeoX builds
...
Examples::
    ...
    python torch_nntile/examples/train_gpt_neox_hf.py train \
        --device cpu --seed 42 --config .../gpt_neox_hf_tiny_config.json \
        --output-dir runs/cpu --epochs 2

    # From scratch on CUDA
    python torch_nntile/examples/train_gpt_neox_hf.py train \
        --device cuda --seed 42 --config .../gpt_neox_hf_tiny_config.json \
        --output-dir runs/cuda --epochs 2

    # From scratch on nntile (separate process)
    python torch_nntile/examples/train_gpt_neox_hf.py train \
        --device nntile --seed 42 --config .../gpt_neox_hf_tiny_config.json \
...
Overhead ladder: ``torch_nntile/examples/overhead_gpt_neox/``.
"""
...
def load_gpt_neox_config(
    ...
    *,
    ...
    for key in ("attention_dropout", "hidden_dropout", "classifier_dropout"):
        fields.setdefault(key, 0.0)
    fields.setdefault("rotary_pct", 0.5)
    fields.setdefault("rotary_emb_base", 10000)
    fields.setdefault("tie_word_embeddings", False)
    config = GPTNeoXConfig(**fields)
...
def ensure_seq_len_fits_positions(config: GPTNeoXConfig, seq_len: int) -> None:
...
def build_model(config: GPTNeoXConfig, seed: int) -> GPTNeoXForCausalLM:
    set_seed(seed)
    config.use_cache = False
...
def fix_empty_buffers_for_nntile(model: torch.nn.Module) -> None:
    """Replace 0-element buffers (e.g. ``rotary_pct=0`` ``inv_freq``) before H2D.

    NNTile TensorGraph rejects non-positive dimension sizes; HF still builds
    valid models with empty rotary tables when rotation is disabled.
    """
...
def load_train_state(
    ...
    if args.checkpoint:
        ...
        config = GPTNeoXConfig.from_dict(ckpt["config"])
        config._attn_implementation = attn_impl
        config.use_cache = False
        model = GPTNeoXForCausalLM(config).float()
...
def train_nntile(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        with torch.no_grad():
            ...
            fix_empty_buffers_for_nntile(cpu_model)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train = sub.add_parser(
        "train",
        help="Train GPT-NeoX HF on a tiny synthetic token stream",
    )
    train.add_argument(
        "--device",
        required=True,
    ...
    train.add_argument(
        ...
        help="GPT-NeoX JSON config path",
    )
