...
#
# @file torch_nntile/examples/train_roberta_hf_overhead.py
# Train stock HuggingFace RobertaForMaskedLM (cpu, cuda, or nntile).

"""Train HuggingFace RoBERTa on a tiny synthetic token stream.
...
Attention: stock HF RoBERTa uses **eager** ``RobertaSelfAttention`` (manual
``matmul`` / ``softmax``; no ``sdpa`` backend in transformers 4.52). Both
``cuda`` and ``nntile`` use ``attn_implementation="eager"``.
...
flight when the clock starts. Loss uses stock ``F.cross_entropy`` on MLM logits with ``ignore_index=-100``
(same on CUDA and nntile). On nntile,
...
Examples::
    ...
    python torch_nntile/examples/train_roberta_hf_overhead.py train \\
        --device cuda --disable-tf32 --seed 42 \\
        --config torch_nntile/examples/overhead_roberta/roberta_xs.json \\
        --seq-len 768 --batch-size 1 --max-sequences 10 --epochs 1 \\
        --output-dir /tmp/roberta_overhead_s_cuda

Overhead ladder: ``torch_nntile/examples/overhead_roberta/``.
"""
...
from hf_tiny_train_common import make_mlm_batch, mlm_ce_loss
from transformers import RobertaConfig, RobertaForMaskedLM


def _default_config_path() -> Path:
    return Path(__file__).resolve().parent / "overhead_roberta/roberta_xs.json"


def _attn_implementation_for_device(device: str) -> str:
    """Stock HF RoBERTa: eager attention for parity with nntile."""
    del device
    return "eager"


def load_roberta_config(
    path: Path,
    *,
    ...
    fields.setdefault("hidden_dropout_prob", 0.0)
    fields.setdefault("attention_probs_dropout_prob", 0.0)
    config = RobertaConfig(**fields)
...
def ensure_seq_len_fits_positions(config: RobertaConfig, seq_len: int) -> None:
    if seq_len < 1:
    ...
    if seq_len > n_positions:
        raise SystemExit(
            ...
            f"config max_position_embeddings={n_positions}."
        )
...
def build_model(config: RobertaConfig, seed: int) -> RobertaForMaskedLM:
    set_seed(seed)
...
def load_train_state(
    ...
    if args.checkpoint:
        ...
        model = RobertaForMaskedLM(config).float()
...
def mlm_loss(model: RobertaForMaskedLM, batch: BatchDict) -> torch.Tensor:
    """Masked LM CE via stock ``F.cross_entropy`` (``ignore_index=-100``)."""
    return mlm_ce_loss(model, batch)
...
def train_torch(args: argparse.Namespace) -> int:
    ...
    loss = mlm_loss(model, last_batch)
    loss.backward()
    optimizer.step()
    del loss
    optimizer.zero_grad(set_to_none=True)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train = sub.add_parser(
        "train",
        help="Train RoBERTa HF on a tiny synthetic token stream",
    )
    train.add_argument(
        "--device",
        required=True,
