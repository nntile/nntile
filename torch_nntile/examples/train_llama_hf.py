...
# Train stock HuggingFace LlamaForCausalLM (cpu, cuda, or nntile).

"""Train HuggingFace Llama on a tiny synthetic token stream.
...
flight when the clock starts. Stock HuggingFace Llama builds
...
Examples::
    ...
    python torch_nntile/examples/train_llama_hf.py train \
        --device cpu --seed 42 --config .../llama_hf_tiny_config.json \
        --output-dir runs/cpu --epochs 2

    # From scratch on CUDA
    python torch_nntile/examples/train_llama_hf.py train \
        --device cuda --seed 42 --config .../llama_hf_tiny_config.json \
        --output-dir runs/cuda --epochs 2

    # From scratch on nntile (separate process)
    python torch_nntile/examples/train_llama_hf.py train \
        --device nntile --seed 42 --config .../llama_hf_tiny_config.json \
...
Overhead ladder: ``torch_nntile/examples/overhead_llama/``.
"""
...
def load_llama_config(
    ...
    *,
    ...
    fields.setdefault("attention_dropout", 0.0)
    fields.setdefault("hidden_act", "silu")
    fields.setdefault("rms_norm_eps", 1e-6)
    fields.setdefault("rope_theta", 10000.0)
    fields.setdefault("tie_word_embeddings", False)
    n_heads = fields.get("num_attention_heads")
    if n_heads is not None:
        fields.setdefault("num_key_value_heads", n_heads)
    config = LlamaConfig(**fields)
...
def ensure_seq_len_fits_positions(config: LlamaConfig, seq_len: int) -> None:
    ...
    if seq_len > n_positions:
        raise SystemExit(
            ...
            f"config max_position_embeddings={n_positions}. Use --seq-len <= "
            f"{n_positions}."
        )
...
def build_model(config: LlamaConfig, seed: int) -> LlamaForCausalLM:
    set_seed(seed)
    config.use_cache = False
...
def patch_hf_llama_rms_norm_for_nntile() -> None:
    """Patch HF ``LlamaRMSNorm`` so ``weight * hidden`` uses ``expand_as``.

    Stock HF multiplies ``weight * hidden_states`` with broadcast; nntile
    ``mul.Tensor`` requires same shapes. Expanding the weight first matches
    PyTorch numerics without ``rms_norm_forward`` in the extension build.
    """
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        weight = self.weight
        while weight.dim() < hidden_states.dim():
            weight = weight.unsqueeze(0)
        weight = weight.expand_as(hidden_states)
        return weight * hidden_states.to(input_dtype)

    LlamaRMSNorm.forward = forward  # type: ignore[method-assign]
    print("HF LlamaRMSNorm: expand weight before mul (nntile)", flush=True)
...
def load_train_state(
    ...
    if args.checkpoint:
        ...
        config = LlamaConfig.from_dict(ckpt["config"])
...
def train_nntile(args: argparse.Namespace) -> int:
    ...
    patch_hf_llama_rms_norm_for_nntile()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train = sub.add_parser(
        "train",
        help="Train Llama HF on a tiny synthetic token stream",
    )
    train.add_argument(
        "--device",
        required=True,
