...
| final loss | {GPT2_REF['long_loss']:.6f} | **{neo_long_loss:.6f}** | Neo drifts vs HF(cuda) on 10-step S |
...
{NOTATION_HF}
{prelim_block}Ten-step stock HuggingFace **GPTNeoForCausalLM** on **HF(cuda)** vs **HF(nntile)**.
...
**{repeats} repeats** per configuration (mean ± stdev). Post **`float32×bool`
`at::mul_out`** fix (eager causal mask). Includes **S HF(nntile) {long_steps}-step**
steady-state run per repeat.
...
def main() -> int:
    ...
    if write_long_plots(
        ...
        csv_path=csv_path,
        svg_path=svg_path,
        title=f"GPT-Neo S HF(nntile) host overhead per iteration ({long_steps} steps)",
    ):
