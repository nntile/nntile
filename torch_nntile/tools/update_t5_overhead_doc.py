...
def render_doc(
    ...
    if loss_notes:
        ...
        loss_section = (
            "T5 CE loss matches HF(cuda) vs HF(nntile) to printed 1e-4 at all ladder sizes "
...
{NOTATION_HF}
Ten-step stock HuggingFace **T5ForConditionalGeneration** on **HF(cuda)** vs **HF(nntile)**.
...
def main() -> int:
    ...
    if write_long_plots(
        ...
        csv_path=csv_path,
        svg_path=svg_path,
        title=f"T5 S HF(nntile) host overhead per iteration ({long_steps} steps)",
    ):
